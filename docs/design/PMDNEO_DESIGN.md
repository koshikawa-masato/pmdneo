...
[`docs/design/analysis_m_data_structure.md`](analysis_m_data_structure.md)、 v2 + v3 部分追記済)。 Phase 2 ではこの仕様基盤を Z80 driver 実装に
落とし込む。

### §2-3-2 実装範囲

- **driver フルスクラッチ実装(FM/SSG 部分)**:
  - `.m` 解釈ループ(V4.8s 互換 dispatch table 構造、 擬似コードは
