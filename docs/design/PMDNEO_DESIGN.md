...
PMDNEO の実装方針(YM2610B 仕様 + chip 互換性活用)を早期に実証する。

[★ 再定義中] PMD V4.8s 公式 Z80 binary は存在しないことが判明したため
(PMD86.COM / PMDB2.COM は PC-98 用 x86)、 当初の「既存 PMD V4.8s driver Z80
binary 流用」 path は成立しない。 Phase 1 の具体構成は user 判断待ち
(Q32 議論):

- (a) PMD V3.9 (PC-88 Z80 binary、 pmd88v48s vendor の pmd2) を流用
  → ただし「PMD88 互換捨てる」 方針と矛盾する可能性
- (b) Phase 1 を ROM ビルドフロー検証のみに縮小、 driver の MAME 再生確認は
  Phase 2 以降
- (c) Phase 1 を skip、 Phase 2 から直接開始
- (d) Phase 1 = `.m` フォーマット解析 + 仕様文書化、 driver は Phase 2 から
- (e) その他
...
PMDNEO 専用フルスクラッチ driver(YM2610B 仕様、 OPNB 専用設計)を実装、 既存
PMDDotNET 出力(V4.8s 互換 `.m`)を解釈して NEOGEO 上で FM/SSG 楽曲を再生
できる状態にする。 Phase 3 以降の ADPCM-A / ADPCM-B 拡張のための driver
基盤を確立する。
...
- 自作 driver で V4.8s 互換 `.m` の楽曲が MAME 上の NEOGEO で再生可能
- driver は OPNB 専用設計、 PMD88 V3.9 / V4.8s driver からの流用ゼロ
- Phase 3 で ADPCM-A 拡張する準備が整った driver 基盤

### §2-3-4 規模感

数ヶ月レベル。 PMD.ASM 10864 行 x86 アセンブリを仕様書として読みながら、
NEOGEO Z80 サブ CPU 用に書き起こす移植作業 + OPNB 適合(K/R / A,D は
chip 側で無視される前提) が中心。 当初「数週間」 と見積もったが、 PMD V4.8s
ソースの規模を考慮して再評価した。
...
- Phase 2: そのまま使用(改良なし、 driver 側で `.m` を解釈)
- Phase 3: PMDDotNET にない ADPCM-A 6ch + `.PNE` 形式 + `#PNEFile` コマンドを
  新規追加実装、 出力を OPNB 拡張版 `.mn` に切替
- Phase 4: ADPCM-B 対応 + ROM 出力部分(NEOGEO 用)を完成、 `.mn` 仕様確定
