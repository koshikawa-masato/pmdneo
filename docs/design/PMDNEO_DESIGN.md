...
- 解析対象:
  ...
  - 音色データ領域
  - 末尾付加情報(`#コマンド` の反映等)
- 成果物: `docs/spec/m_format.md`(新規) として仕様書化
...
- `docs/spec/m_format.md` に次の項目が文書化されている:
  - `.m` の binary 構造 全体図
  - dispatch table の opcode → handler 対応表
  - 各 opcode の byte 列フォーマット(byte 数、 引数解釈、 引数範囲)
  - part offset table の構造
  - 音色データ / `#コマンド` 反映領域の説明
- Phase 2 で driver を実装する際、 `m_format.md` だけ読めば dispatch ループ
  が書ける詳細度
