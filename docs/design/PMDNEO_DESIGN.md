...
# §3 Phase 2 着手前 設計書 3 種 (Phase 1 (δ) 解析の応用)

Phase 2 driver 着手前に「設計書ファースト」 原則 (= R2 sprint 偽完了の
反省、 「実装に飛び込む前に仕様を文書で固定」)に従って 3 つの設計書を
`docs/design/` 配下に整備した (計 2296 行)。 各設計書は前の設計書を前提
にしているため、 1 → 2 → 3 の順で読む必要がある。

## §3-1 設計書 1: `.mn` binary layout 仕様

[`mn_binary_layout.md`](mn_binary_layout.md) (673 行)

`.mn` (PMDNEO 独自拡張子) の binary layout 仕様。 OPNA 互換 layout を温存
しつつ、 ADPCM-A 6ch 用 part offset table と `.PNE` filename を後方拡張
領域に並べる構造を確定。

主要決定事項:

- m_start bit 2 = PMDNEO mode flag (= 拡張領域あり/なし判別)
- m_buf[26..27] = extended_data_adr (PMDNEO 専用、 後方拡張領域への offset)
- ADPCM-A 6 part offset table は prgdat 領域の後ろに配置
- `.PNE` filename ptr は `.mn` 末尾に NUL-terminated string で embed
- part letter A-K + L-Q (新規 ADPCM-A 6ch)
- 既存 OPNA `.m` を PMDNEO driver で再生可能 (FM/SSG/ADPCM-B 鳴る、 K/R
  無音、 誤動作なし)

## §3-2 設計書 2: PPZ → ADPCM-A 翻訳 mapping

[`ppz_to_adpcma_mapping.md`](ppz_to_adpcma_mapping.md) (818 行)

PMD V4.8s の PPZ8 拡張機構を ADPCM-A 6ch 用に翻訳するための逐条対応表。
PMDPPZ + PMDDotNET の PPZ8 module を read-only 参照テンプレートとして
書き換える指針。

主要決定事項:

- 関数呼出規約: A = opcode、 DE = arg1、 BC = arg2、 HL = ptr 戻り値
- 8ch → 6ch 縮約: part10a-f を残し part10g/h を完全削除
- 新規 `cmdtbla` を新設 (jump1 上書きせず互換性温存)
- Loop / SrcFreq: 完全 no-op + `.PNE` 生成側で展開
- workarea: OPNA layout 後方拡張領域に直接配置 (part 番号順)
- PMD ファミリ「未対応 cmd スルー」 思想の継承

棚卸結果:

- PMD.ASM 全 `if ppz` 29 箇所 (rename only 2 / 削除 4 / redesign 23)
- PPZDRV.ASM 全 76 label (主要 entry point 17 個に集約)
- PMDDotNET PPZChannelWork 17 field → ADPCMAChannelWork 5 field 簡略化
- chip 駆動 opcode 14 個 → YM2610/B ADPCM-A register (port B 0x00-0x2D 帯)
  操作 mapping

## §3-3 設計書 3: Phase 2 driver 実装計画

[`phase2_driver_plan.md`](phase2_driver_plan.md) (805 行)

Phase 2 driver の src/driver/ 配下に何をどう起こすかの実装計画。 設計書
1 + 2 を前提とする。

主要決定事項:

- assembler: **sdasz80** (ngdevkit 慣習)
- driver source 構造: 8 file 構成
  - `PMDNEO.ASM` (build top)
  - `PMD_Z80.ASM` (PMD.ASM の Z80 化 base)
  - `ADPCMB_DRV.ASM` (Phase 2 完成)
  - `ADPCMA_DRV.ASM` (Phase 3 用 stub、 Phase 2 では空)
  - `KR_STUB.INC` (K/R no-op stub)
  - `IRQ.INC` (TIMER-B IRQ handler)
  - `REGMAP.INC` (YM2610/B register 定数)
  - `WORKAREA.INC` (per-part workarea offset 定数)
- ngdevkit 結合: nullsound 系統 sound command framework 流用 + driver
  core 独自実装
- ADPCM-B sample = `.PPC` 流用 (PMD V4.8s 既存 format 踏襲)
  - `.P86` は WebApp 側で `.PPC` に変換して取込
  - `.PNE` (ADPCM-A) と `.PPC` (ADPCM-B) を同じ V-ROM bank に並べて焼き込み
  - driver は chip register 経由で ADPCM-A / ADPCM-B に振り分け
- K/R workarea: OPNA 完全互換でフルサイズ確保
- audio gate test: Step 1 (SSG) → Step 2 (FM) → Step 3 (ADPCM-B) →
  Step 4 (K/R no-op + si trace) → Step 5 (統合長尺)
- Phase 2 内 sub-phase: SubA (skelton) → SubB (SSG) → SubC (FM) → SubD
  (K/R) → SubE (ADPCM-B) → SubF (統合)
- 想定期間: 1.5-2.5 ヶ月

## §3-4 設計書 1-3 完了の意義

3 設計書完了時点 (= 計 2296 行) で:

- `.mn` binary format が byte 単位で確定
- PPZ → ADPCM-A 翻訳指針が file 単位 / routine 単位で確定
- Phase 2 driver 実装の day-1 作業 (= dummy ROM ビルドが通る driver
  source skeleton 起こし) に着手可能な詳細度
- user judgment 全 13 件解決済 (= 「記憶は AI に、 判断は自分が握る」
  CLAUDE.md 原則準拠)

これで Phase 2 driver 実装着手の準備が完成。 以後は src/driver/
skeleton 起こし (= 別 plan) → Phase 2 driver 実装 (SubA-SubF を順次実行)
に進む。

# §4 互換性維持戦略

(章4 以降は次の壁打ちセッションで詰める。 ただし設計書 1 §0-3 + §8 で
OPNA バイナリ互換性方針 + PMD ファミリ「未対応 cmd スルー」 思想の継承
が確定済 = §4 で詰める範囲は限定的)
...
[章 1-2 完成、 §3 完成 (Phase 2 着手前 設計書 3 種への index)、 §4-§6
は次の壁打ちセッションで詰める。 Phase 2 driver 実装の準備は揃った]
