# `.PNE` バイナリ layout 仕様書
    ...
    参照: ADR-0021 (= Step 7 sprint 起票)
    状態: draft v0.1 (= α-1 段階、 provenance 調査 + format 骨格固定)
    注: path B (= §6 / §11) は **α-1 時点の推奨**であり、 converter I/O contract
        確定 + ADR-0021 §決定 5 補正提出 によって **α-2 で正式確定**する。
        本書 α-1 段階では layout + provenance 調査 + path 候補整理 までを責務とする。
...
## 6. 既存 ngdevkit 経路との接続 (= path B 採用根拠)

### 6-0. 本章の位置付け (= α-1 / α-2 責務境界)

本章で示す path B は **α-1 時点の推奨** であり、 正式採用は **α-2 (= converter I/O contract sprint)** で converter input / output 仕様を詰めた上で ADR-0021 §決定 5 補正提出と同時に確定する。

α-1 と α-2 の責務分担:

| sub-sprint | 責務 | 本書での扱い |
|---|---|---|
| **α-1 (= 本書 commit)** | `.PNE` binary layout 固定 + provenance 調査 + path 候補整理 | §3-§5 (= format 骨格) / §2 (= provenance) / §6 (= path 候補) |
| **α-2 (= 次 commit)** | converter I/O contract 確定 + path B 正式採用判断 + ADR-0021 §決定 5 補正提出 | 本書では未確定 (= 「α-2 で確定」 と明記)、 §11 で補正候補提示 |

α-1 段階では path B が「他の候補 (= path A / C) より整合性が高い」 ことを示すに留め、 path B の細部 (= converter input/output signature、 error handling、 file naming convention 等) は α-2 で詰める。

### 6-1. path B 経路図

```
.PNE file (= source of truth、 楽曲制作者が `#PNEFile` で参照)
  ↓ converter (= step 7-β で実装、 `scripts/pne-to-ngdevkit.py` 等)
samples-map.yaml + .adpcma binary 群 (= ngdevkit 入力)
  ↓ vromtool.py (= ngdevkit native、 完全不変)
samples.inc (= driver consumable defines)
  ↓ assembler
driver build (= standalone_test.s 完全不変)
```

### 6-2. converter responsibility (= α-2 で詳細確定)

`.PNE` → ngdevkit 入力 への converter の責務:

1. `.PNE` magic / version 検証
2. slot table parse、 各 slot の name / raw data offset / size 抽出
3. raw ADPCM-A binary を slot 名で `.adpcma` file に書き出す (= 一時 dir or `assets/sounds/adpcma/` 同等位置)
4. `samples-map.yaml` を slot 順で生成
5. ADPCM-B (= beat 等) は `.PNE` scope-out のため、 既存 entry をそのまま温存 (= 別 mechanism で管理)

### 6-3. ADPCM-B との同居問題

現状の `samples-map.yaml` は ADPCM-A 6 + ADPCM-B 1 (= beat) を同居している (= §2-2 確認済)。 `.PNE` は ADPCM-A 専用のため、 converter は:

- ADPCM-A 部分 (= 6 slot) のみ `.PNE` 由来で再生成
- ADPCM-B 部分 (= beat) は既存 samples-map.yaml entry を温存 (= 手書き or 別 mechanism)
...
## 11. ADR-0021 §決定 5 補正候補 (= α-2 で正式確定予定)

α-1 調査で発見した「現状の `samples.inc` は vromtool.py 経由で生成済」 という事実から、 ADR-0021 §決定 5 は以下のように補正される **見込み**。 ただし本書 α-1 段階では「補正候補の提示」 までを責務とし、 **正式採用の確定は α-2** (= converter I/O contract sprint 内で ADR-0021 修正案を user に提出 + 承認) で行う。

**現 §決定 5 (= path A 想定)**:
> `.PNE` → 直接 `samples.inc` 生成 + driver source の sample data 部を生成 include へ移行

**補正案 (= path B、 α-2 で正式判断)**:
> `.PNE` → converter で `samples-map.yaml` + `.adpcma` binary 群を生成 → ngdevkit vromtool.py が引き続き `samples.inc` を生成 → driver build へ
>
> driver source / build pipeline / vromtool.py 経路はすべて完全不変。 改修対象は converter (= 新規) と build script (= `.PNE` → `samples-map.yaml` 変換 step の追加) のみ。

この補正により ADR-0021 の「driver 不変」 「ROM byte-identical primary gate」 がより強く保証される (= 既存経路と byte 単位で同じ生成物になる)。

---

[draft v0.1 — provenance 調査 (§2) + format 骨格 (§3-§7) + path B 採用根拠 (§6) + α-2 補正候補 (§11) 完成。 α-2 で converter I/O contract 詰めて ADR-0021 §決定 5 補正提出]
