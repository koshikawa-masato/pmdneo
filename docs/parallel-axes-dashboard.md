...
| **G (= ADPCM 動的 sample 供給)** | `wip-axis-g-ppc-parser-adr-0048-filing` + `wip-axis-g-alpha-ppc-format-archaeology` + `wip-axis-g-beta-ppc-parser-spike` + `wip-axis-g-gamma-integration-design` + `wip-axis-g-adr-0048-vromtool-finding-revision` (= 全 MERGED) | 0048 | **ADR-0048 Draft 起票済 + α/β/γ 完了 + 35th session γ revision (= vromtool finding 反映 + 案 C 部分 runtime parse 採用)** (= PR #39/#41/#43/#45 MERGED、 Annex A 8 section literal + spike script 6/6 PASS + §決定 8 V-ROM mapping mapping-B 確定 + 案 C 経路確定 + ppc-to-ngdevkit.py 新規 generator scope literal + stale literal 全削除、 doc + spike script sprint)、 次 = sub-sprint δ runtime selection proof (= 案 C 経路 driver touch 最小、 既存 ADR-0043 routine 不可触) | bfc2e5e (= γ revision 116 ins + 19 del) | α 3 round + β 1 round + γ 3 round + **γ revision 3 round chain (= approve/revise/approve)** (= 35th session vromtool finding 反映、 must-fix 10 件 + nice-to-have 3 件 全反映、 factual drift 完全解消、 δ 案 C 経路着手 GO) | n/a (= doc + spike script sprint、 user 介入は escalate or 最終確認 PR merge のみ、 δ で driver touch 開始予定 = byte-identical regression + audio gate audition 議題) |
...
| 0048 | **軸 G (= ADPCM 動的 sample 供給)** | **起票済 + α/β/γ 完了 + γ revision (= 9d091c7 + 80fd219 + bd9401a + 12fcf69 + bfc2e5e、 PR #37/#39/#41/#43/#45 MERGED、 Draft)** | `.PPC` parser + driver runtime selection + asset converter 接続 (= ADR-0043 §決定 2 literal 後続 ADR)、 5 段 α/β/γ/δ/ε 構成、 doc-only filing + α format archaeology + β parser/validator proof spike 6/6 PASS + γ integration design + V-ROM mapping mapping-B 確定 + selection key δ defer + 35th session γ revision (= vromtool finding 反映、 案 C 部分 runtime parse 採用、 ppc-to-ngdevkit.py 新規 generator scope literal) 完了、 次 sub-sprint δ runtime selection proof (= 案 C 経路 driver touch 最小 + 既存 ADR-0043 routine 不可触) |
...
| **δ** (= runtime selection proof、 **案 C 部分 runtime parse 経路**) | **次** | - | 案 C 経路 = **(1) sample data**: `.PPC` から ADPCM-B raw byte 抽出 → 既存 vromtool 経路 (= yaml + adpcm_b passthrough) で V-ROM 焼き込み + **(2) directory binary**: ROM 内別領域 embed (= `PPC_DIRECTORY_BASE` symbol) + **(3) driver runtime**: `pmdneo_select_adpcmb_ppc_pointer` 新規 routine (= 既存 ADR-0043 routine 不可触) で directory 引き + mapping-B `v_rom_word = ppc_word + PPC_VROM_BASE_OFFSET_WORD` + **(4) selection key**: 候補-1 (= sample_table_id bit7 set/clear で source 分岐) + **(5) 新規 generator**: `scripts/ppc-to-ngdevkit.py` (= vromtool 外側、 `.PPC` → directory binary embed + sample data 既存 vromtool 経路へ渡す) + **(6) 実 `.PPC` minimum fixture** 1-2 entry 生成 + **verify gate** (= ymfm-trace primary gate + driver byte-identical + 既存 ADR-0043 fixture regression + 新規 .PPC fixture reg 0x12-0x15 literal assert + MAME 起動確認 audio gate)、 driver touch sprint = byte-identical regression + audio gate audition 議題 |
| ε | 未着手 | - | integration + audition gate (= 必要時 user audition)、 ADR-0043 ADPCM-B production-ready 保護 |
