...
| **G (= ADPCM 動的 sample 供給)** | `wip-axis-g-ppc-parser-adr-0048-filing` + `wip-axis-g-alpha-ppc-format-archaeology` + `wip-axis-g-beta-ppc-parser-spike` + `wip-axis-g-gamma-integration-design` (= 全 MERGED) | 0048 | **ADR-0048 Draft 起票済 + α/β/γ 完了** (= PR #39/#41/#43 MERGED、 Annex A 8 section literal + spike script 6/6 PASS + §決定 8 V-ROM mapping mapping-B 確定 + integration design literal、 doc + spike script sprint)、 次 = sub-sprint δ runtime selection proof (= 最小 driver touch、 既存 ADR-0043 routine 不可触) | 12fcf69 (= γ 完了 151 ins + 7 del) | α 3 round + β 1 round + **γ 3 round chain (= revise/revise/approve)** (= must-fix 4 件 + nice-to-have 2 件 全反映、 factual drift 完全解消、 δ 着手 GO) | n/a (= doc + spike script sprint、 user 介入は escalate or 最終確認 PR merge のみ、 δ で driver touch 開始予定 = byte-identical regression + audio gate audition 議題) |
...
| 0048 | **軸 G (= ADPCM 動的 sample 供給)** | **起票済 + α/β/γ 完了 (= 9d091c7 + 80fd219 + bd9401a + 12fcf69、 PR #37/#39/#41/#43 MERGED、 Draft)** | `.PPC` parser + driver runtime selection + asset converter 接続 (= ADR-0043 §決定 2 literal 後続 ADR)、 5 段 α/β/γ/δ/ε 構成、 doc-only filing + α format archaeology + β parser/validator proof spike 6/6 PASS + γ integration design + V-ROM mapping mapping-B 確定 + selection key δ defer 完了、 次 sub-sprint δ runtime selection proof (= 最小 driver touch + 既存 ADR-0043 routine 不可触) |
...
| **δ** (= runtime selection proof) | **次** | - | mapping-B 実装 (= driver source `standalone_test.s` 新規 routine 追加 only、 既存 ADR-0043 routine 不可触) + selection key 候補-1 確定 (= sample_table_id bit7 set/clear で source 分岐) + vromtool.py 拡張 (= `.PPC` file 取り込み + samples.inc symbol emit + offset 計算) + 実 `.PPC` minimum fixture 1-2 entry 生成 + verify gate (= ymfm-trace primary gate + driver byte-identical + 既存 ADR-0043 fixture regression + 新規 .PPC fixture reg 0x12-0x15 literal assert)、 最小 driver touch |
