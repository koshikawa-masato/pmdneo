...
| **G (= ADPCM 動的 sample 供給)** | `wip-axis-g-ppc-parser-adr-0048-filing` (= MERGED) + 後続 sub-sprint α-ε branch | 0048 | **ADR-0048 Draft 起票済** (= PR #37 MERGED 9e429ac、 5 段 α/β/γ/δ/ε 構成、 doc-only filing 完了)、 次 = sub-sprint α format archaeology | 9d091c7 (= ADR-0048 起票 246 行) | layer 2 round 1 approve (= 33rd session 起点 sprint 方針) + **layer 2 round 2 approve** (= ADR-0048 起票 doc-only filing review、 must-fix 0 件、 next step α GO) | n/a (= doc-only filing、 user 介入は escalate or 最終確認 PR merge のみ) |
...
| 0048 | **軸 G (= ADPCM 動的 sample 供給)** | **起票済 (= 9d091c7、 PR #37 MERGED、 Draft)** | `.PPC` parser + driver runtime selection + asset converter 接続 (= ADR-0043 §決定 2 literal 後続 ADR)、 5 段 α/β/γ/δ/ε 構成 (= Codex layer 2 推奨)、 33rd session 候補 3 採用、 doc-only filing 完了 (= 246 行)、 次 sub-sprint α format archaeology 着手 |
...
| **α** (= format archaeology + fixture contract) | **次** | - | vendor/PMDDotNET/PMDDotNETDriver/PCMDRV.cs (= 1063 行) grep + byte-level parser spec literal + 最小 fixture (= 1-2 entry) 期待値 + PMDNEO YM2610(B) V-ROM 直結 mapping 設計 ADR-0048 Annex A 化、 doc-only sprint (= driver / runtime / vendor source touch なし) |
| β/γ/δ/ε | 未着手 | - | β parser / validator proof spike (= driver touch なし) → γ integration design (= 設計のみ) → δ runtime selection proof (= 最小 driver touch、 既存 routine 不可触) → ε integration + audition gate (= 必要時 user audition)、 ADR-0043 ADPCM-B production-ready 保護 |
