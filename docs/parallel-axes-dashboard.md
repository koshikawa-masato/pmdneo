...
| **G (= ADPCM 動的 sample 供給)** | `wip-axis-g-ppc-parser-adr-0048-filing` + `wip-axis-g-alpha-ppc-format-archaeology` (= 全 MERGED) | 0048 | **ADR-0048 Draft 起票済 + α 完了** (= PR #39 MERGED 9b52af3、 Annex A 8 section literal + §決定 2 役割分離 + §決定 4 commit 汚染防止、 doc-only sprint)、 次 = sub-sprint β parser / validator proof spike | 80fd219 (= α 完了 220 ins + 32 del) | round 1 approve + round 2 approve + **layer 2 round 3 approve** (= α revise chain round 1 revise → round 2 revise → round 3 approve、 must-fix 5+3 件 + nice-to-have 3+1 件 全反映、 β 着手 GO) | n/a (= doc-only sprint、 user 介入は escalate or 最終確認 PR merge のみ) |
...
| 0048 | **軸 G (= ADPCM 動的 sample 供給)** | **起票済 + α 完了 (= 9d091c7 + 80fd219、 PR #37/#39 MERGED、 Draft)** | `.PPC` parser + driver runtime selection + asset converter 接続 (= ADR-0043 §決定 2 literal 後続 ADR)、 5 段 α/β/γ/δ/ε 構成、 doc-only filing + α format archaeology + Annex A 8 section literal + 役割分離 + commit 汚染防止 完了、 次 sub-sprint β parser / validator proof spike + minimum fixture 生成 |
...
| **β** (= parser / validator proof spike + minimum fixture 生成) | **次** | - | scripts/ppc-parser-spike.py 等 spike script 新規 (= byte-level emit + reject 条件 5 件 literal 検証)、 既存 `vendor/PMDDotNET/` 配下 .PPC sample 探索 + 不在時 byte-level emit で minimum valid .PPC 1-2 entry 生成、 driver / runtime / vendor 完全不変 |
| γ/δ/ε | 未着手 | - | γ integration design (= 設計のみ、 V-ROM mapping 確定) → δ runtime selection proof (= 最小 driver touch、 既存 ADR-0043 routine 不可触) → ε integration + audition gate (= 必要時 user audition)、 ADR-0043 ADPCM-B production-ready 保護 |
