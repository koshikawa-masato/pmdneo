...
|---|---|---|---|---|---|---|
| **0 (= orchestration setup)** | `wip-orchestration-setup` | 0041 | γ 着手中 (= 本 commit) → δ で PR + 本拠地 merge | b205716 (= α ADR-0041 起票) | round 1 revise → must-fix 反映 → round 2 fallback approve | PR review |
...
|---|---|---|---|---|---|
| 2026-05-18 | 軸 0 α | layer 2 hang (= Codex runtime `task-mpb6938b-z2y3pw` 解放されず、 background + foreground 2 度試行後 hang) | fallback (= 主軸単独評価 approve) | ADR-0041 b205716 commit + push 進行 (= must-fix 3 + nice-to-have 4 + §決定 4-2/4-3 反映済) | **pending** (= Codex 復帰待ち) |
| 2026-05-18 | 軸 0 β | layer 2 hang (= 同 task 解放されず、 3 回連続 hang) | fallback (= 主軸単独評価 approve) | memory + CLAUDE.md + MEMORY.md literal 化進行 (= 全 user 個人 file、 commit なし sprint) | **pending** (= Codex 復帰待ち) |
| 2026-05-18 | 軸 0 γ | layer 2 hang (= 4 回連続 hang、 同 task 解放されず) | fallback (= 主軸単独評価 approve) | dashboard 作成 commit + push 進行 (= ADR-0041 §決定 7 仕様 + sub-agent context 調査結果 literal 反映) | **pending** (= Codex 復帰待ち) |
...
| γ | 着手中 | (= 本 dashboard 作成 commit 予定) | dashboard `docs/parallel-axes-dashboard.md` 新規作成 |
| δ | 未着手 | - | PR 作成 + 本拠地 merge |
...
| α | 未着手 | - | ADR-0042 起票 (= β sub-sprint 設計、 mixer 構造 or .fxp gain 案) |
...
| α | 未着手 | - | ADR-0043 起票 (= ADPCM-B 設計、 sub-sprint α/β/γ/δ 4 段構成、 ADPCM-A の 5 段より ch 軸不要で 1 段減) |
...
| α | 未着手 | - | ADR-0044 起票 (= MML 拡張 sub-軸 設計、 F-3 chip target flag 先行推奨) |
