...
| **C (= ADPCM-B 軸)** | `wip-axis-c-adpcmb` (= ADR α + 実装 α) + `wip-axis-c-beta-multi-sample` (= 実装 β) + `wip-axis-c-gamma-1-actual-silence` (= 実装 γ-1、 32nd session 新 branch)、 PR #18/#22/#28 全 MERGED + γ-1 PR pending | 0043 | **実装 sub-sprint γ-1 完了** (= 本 commit、 actual silence sample 投入 + vromtool encode 経路 + Codex layer 2 must-fix 3 件解消 + verify gate 両 gate 全 PASS) | (= 本 commit hash) | layer 2 計 3 round (= round 1 escalate + round 2 revise + round 3 revise→主軸 plan v2 固定実装) Codex rescue 化 default 永続化適用 | γ-2 sample_table_id integration + δ statement audio gate (= ADPCM-A 連動 + 越川氏 audition) |
...
| **実装 sub-sprint γ-1** | **完了 (= 本 commit)、 verify gate 両 gate 全 PASS** | (= 本 commit hash) | actual silence sample 投入 (= project-owned `assets/pne/silence.wav` sha256 c726d333... + yaml + vromtool encode 経路) + driver placeholder → silence rename + literal addr 撤去 + `src/test-fixtures/axis-c/` 新規 fixture + verify script、 verify gate 両 gate 全 PASS = step4 voice 0 byte-identical 維持 + γ-1 voice 1 differential (= samples.inc emit + reg 0x12-0x15 SILENCE_* 一致 + beat addr differ)、 主軸単独実装 + Codex layer 2 計 3 round (= escalate → revise → revise→plan v2 反映)、 ADR-0041 §決定 4-2 Codex rescue 化 default 永続化 適用 (= user 介入なし non-stop 進行) |
| γ-2/δ | γ-2 sub-sprint 起動可 | - | γ-2 sample_table_id integration (= 0xFD32 lookup + ADPCM-A 連動 切替 proof) → δ statement audio gate (= 越川氏 audition、 永久 user scope) |
