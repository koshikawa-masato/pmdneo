...
| **A (= sample provenance β)** | `wip-axis-a-sample-provenance` (= α + β-1 commit、 PR #17/#20 MERGED) | 0042 | **β-1 完了 MERGED** (= 63d3b09 PR #20、 §決定 2 採用案 A literal) | 63d3b09 (= β-1 90 行 diff) | layer 1 round 2 approve + layer 2 採用案 A approve | β-2 spike `audible-level-sweep-spike.py` 新規作成予定 |
| **C (= ADPCM-B 軸)** | `wip-axis-c-adpcmb` (= ADR α + 実装 sub-sprint α commit、 PR #18/#22 MERGED) | 0043 | **実装 sub-sprint α 完了 MERGED** (= 588a11c PR #22、 pmdneo_select_adpcmb_sample_pointer 新規 + adpcmb_keyon selector 経路化 + verify gate 全 PASS、 主軸 fallback 経由 guard 9 件機能ゼロ越境) | 588a11c (= 111 ins + 6 del、 driver + ADR doc-only) | ADR α session 019e3b56-... round 3 approve + 実装 sub-sprint α layer 1 round 2 approve + layer 2 計 4 round 全 approve (= 配置 + GO + 規律 + verify 経路) | β sub-sprint multi-sample fixture + voice index table 化 + range check 起動可 |
...
| β/γ/δ | β sub-sprint 起動可 | - | β multi-sample fixture 比較 (= voice index table 化 + A register range check + sentinel 経路統合) → γ runtime sample selection arch → δ integration + audit_gate |
