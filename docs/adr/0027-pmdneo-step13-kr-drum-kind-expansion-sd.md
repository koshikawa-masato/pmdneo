...
- 関連設計書: `docs/design/PMDNEO_DESIGN.md` §1-8-3 (= `.PNE` 仕様骨子)、 `docs/manual/PMDMML_MAN_V48s_utf8.txt` (= PMD V4.8s K part / R command syntax 仕様、 drum 識別文字 b/s/c/h/t/r 6 種定義)
...
- **drum 種拡張軸 = bit 1 SD のみ accept** (= 全 5 drum 一気は scope-out、 c/h/t/r 残り 4 種 future) (= 軸 1)
...
- (e2) (= **採用**): SD 単独拡張 (= bit 1 のみ accept、 c/h/t/r 残り 4 種 future)
...
Step 13 で routine 内部の implementation は拡張される (= bit 1 分岐追加) が、 routine entry / 引数 / 戻り値 ABI は不変。 future の drum 種拡張 (= c/h/t/r 4 種) でも同じ entry addr を保持することを Step 13 で先取り保証する。
...
- 残り 4 種 (= `c` = CYM / `h` = HH / `t` = TOM / `r` = RIM) は future sub-sprint で段階追加 (= Step 14 候補)
...
- **bit 2 = 1**: CYM trigger → **silent ignore** (= 未対応 cmd スルー思想、 ADR-0026 §決定 11 踏襲)
- **bit 3 = 1**: HH trigger → **silent ignore**
- **bit 4 = 1**: TOM trigger → **silent ignore**
- **bit 5 = 1**: RIM trigger → **silent ignore**
...
- future sprint で c/h/t/r を **段階追加**
...
- 別 mapping table 構造 (= bitmap bit position → sample addr pointer の lookup table) は 4+ drum で検討 (= future sprint c/h/t/r 追加時に再評価)
...
2. c/h/t/r 残り 4 drum 種拡張 → future sub-sprint
...
「dispatch path は drum 種拡張で増やさない」 contract が Step 13 で **bit 0 BD + bit 1 SD の 2 drum 状況下で routine entry addr が変化しない** ことで literal 実装的に保証される。 future drum 種拡張 (= c/h/t/r) でも同じ entry addr を保持することを Step 13 で先取り保証する。
...
## Annex A: PMDDotNET SD emit 調査結果 (= α 着手後 literal 反映)

### A-1: PMDDotNET SD emit 記号 (= `\s` の前提確認)

(α 着手時に literal 反映予定)

- mc.cs rhysel 配列の `s` 識別文字対応確認
- rs00 emit `0xEB <bitmap>` の bit 1 出力確認
- fixture 命名 (= `k-sr-only.mml` / `r-melody-sr-only.mml`) と PMDDotNET 実 emit syntax の整合

### A-2: PMDDotNET bitmap OR emit 動作 (= 同 K part 行 `\b\s` 連続記述時)

(α 着手時に literal 反映予定)

- 同 K part 行 `\b\s` 連続記述で:
  - case A: bitmap OR `0x03` を 1 byte emit (= `0xEB 0x03 ...`)
  - case B: 2 個の `0xEB` + `0x01` + `0xEB` + `0x02` を emit (= 連続 trigger)
- どちらの動作かを literal 確認
- Step 13 では BD+SD 同時打ちは fixture proof scope-out だが、 driver の bitmap accept range 設計に影響あり

### A-3: PMD V4.8s manual SD 記号

(α 着手時に literal 反映予定)

- manual `docs/manual/PMDMML_MAN_V48s_utf8.txt` の SD 記号定義確認
- K part / R command での SD 記号統一性確認

### A-4: SD sample (`adpcma_sample_sd`) 既存 embed 確認

(α 着手時に literal 反映予定)

- ADR-0025 step5b で確認された `adpcma_sample_sd` の addr / size literal
- driver source 内 reference 確認

### A-5: bit 1 分岐追加の literal asm 候補 (= β 着手前)

(α 着手時に literal 反映予定)

- `pmdneo_rhythm_event_trigger` routine 内部の bit 1 分岐挿入位置候補
- explicit if/jr 流儀での asm literal 案

### A-6: fail-safe 条件 evaluate

(α 着手時に literal 反映予定)

- 「PMDDotNET SD emit 記号が `\s` でない」 場合の fixture 命名修正方針
- 「`adpcma_sample_sd` が ADPCM-A subsystem 内に embed されていない」 場合の fallback (= 新規 embed or BD sample 流用 + freq shift)
- 両条件のいずれにも該当しない場合は β 進行可
