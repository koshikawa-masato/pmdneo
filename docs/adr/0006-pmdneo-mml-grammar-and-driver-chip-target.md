...
これにより AES+ YM2610B 対応 ROM 用 MML と公式 NEOGEO YM2610 用 MML を **同一 source で** 生成可能 (= build flag 切替のみで使い分け)。
...
1. **compile.py 拡張**:
   - `TARGET_PARTS` = `("A", "B", "C", "D", "E", "F", "G", "H", "I", "J", "K", "L", "M", "N", "O", "P", "Q")` に拡張 (= 17 part)
   - `PART_LABELS` table に A/D/K を追加
...
2. **driver (standalone_test.s) 拡張**:
   - `part_workarea` を 14 → 16 (= +A/D で 2 part 追加) または 17 (= +K) に拡張
   - A/D part dispatch を確認 (= ADR-0001 既決で 6ch dispatch あるはず)
   - K part dispatch は mute path (= cmd 見ても skip)
...
- PMDDotNET `mc.cs:3525, 1350, 1384`: voice 定義 / part letter convention 根拠
