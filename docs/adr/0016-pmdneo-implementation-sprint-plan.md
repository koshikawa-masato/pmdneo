...
- 関連: ADR-0013 (= 同 .M 2 経路比較 路線への切替)、 ADR-0014 (= ADR-0006 sprint 成果のカテゴリ別判断 + PMDPPZ 流儀発見)、 ADR-0015 (= PMDDotNET 改造 技術調査 sprint、 全 5 軸完了 Accepted)、 ADR-0017 (= develop driver snapshot + ADR-0015 前提整理 + path A 採択)、 ADR-0019 (= step 5 設計判断 6 議題集約、 Accepted)、 ADR-0020 (= step 6 audio isolation 戦略、 silent-bcef fixture 採用、 Accepted、 step 5 完了後の検証 infra 強化拡張 sprint)
...
### 起票時 (= 2026-05-12 4th session、 step 1 着手前)
...
### step 5 ε-c 完了後 (= 2026-05-12 6th session、 ADR-0019 Accepted)

step 1-5 完了で ADR-0016 全体 Accepted。 future scope-out 7 候補を `docs/design/handoff/adr-0016-step5-completion.md` に整理:

1. K/R rhythm compatibility 現役接続 (= micro-sprint)
2. `.PNE` parser driver 実装 (= 次 sprint)
3. ADPCMA_DRV.inc routine 移動 (= refactor sprint)
4. solo 化 audio gate (= optional micro-sprint) ← **ADR-0020 / step 6 で着手**
5. PMDNEO.s nullsound integration (= 大規模 sprint)
6. ADPCM-A vol/pan 拡張 (= 別設計 sprint)
7. vol scale 統一 (= adpcma_keyon_simple raw or 整理)

### step 6 完了後 (= 2026-05-13 7th session、 ADR-0020 Accepted)

solo 化 audio gate (= future scope-out #4) を **ADR-0020 step 6 として消化**:
- 6-a (= silent-bcef fixture + audio isolation verify): 完了
- 6-b (= PMDNEO_MUTE_FM driver flag): 不要判定で scope-out 留保

step 6 完了で残未消化 6 候補 (= 上記 #1/#2/#3/#5/#6/#7):

1. **`.PNE` parser driver 実装** (= 次 sprint 最有力、 Phase 3 driver 部分完了の残作業)
2. K/R rhythm compatibility 現役接続 (= micro-sprint)
3. ADPCMA_DRV.inc routine refactor 移動
4. PMDNEO.s + nullsound integration (= 大規模 sprint)
5. ADPCM-A vol/pan 拡張
6. vol scale 統一

詳細は `docs/design/handoff/adr-0016-step6-completion.md` 参照。
