...
| β | **完了** (= PR #28 MERGED、 主軸 fallback regime + sub-agent 5 連続 isolation fail) | dfc9774 |
| γ-1 | **完了** (= PR #30 MERGED 766fca4) | 766fca4 |
| **γ-2** | **完了** (= 本 commit、 hybrid 経路 + sample_table_id integration proof + Codex layer 2 must-fix 4 件解消、 ADPCM-A 独立性 trace は γ-3 へ defer = unexpected finding) | (= 本 commit hash) |
| γ-3 | 未着手 (= 候補新規追加) | - (= ADPCM-A 独立性 trace、 J + L-Q 同時 fixture、 PMDDotNET /B mode J emit 修正は scope-out 維持) |
