...
| **β** | multi-sample fixture 比較 (= J ch only で sample A / sample B fixture、 build-time `samples.inc` 拡張) | start/stop addr 差分 (= reg 0x12/0x13/0x14/0x15) を register trace で確認、 delta-N / vol / pan は固定で同一、 verify script 自動 PASS |
