...
- 状態: **Accepted** (= 2026-05-21 39th session 軸 B 実装 sprint 2 = δ-2 PartWork/SRAM placement 完了。 全 sub-sprint α/β/γ 完走 = PR #80/#81/#82、 §決定 5 verify gate 6 件 = `src/test-fixtures/axis-b/verify-axis-b-sram-placement.sh` 全 PASS。 ground truth = ADR-0045 Annex I-2 / §J-4-2、 ADR-0045 §J-4-2 literal 後続実装 ADR。 **軸 B 実装 sprint chain の 実装 2 = δ-2 = v2 SRAM sub-region foundation の完了** (= δ-3 F-2-B / δ-4 軸 C/G/rhythm 接続点 が依存)。 軸 B 全体は未完了 (= 「軸 B 完成」 表現不使用、 残 = 実装 3-4 = δ-3/δ-4)。 Draft → Accepted 移行は §決定 9 = Codex layer 2 approve 経由 (= 39th session 完全自走 model、 user 判断を Codex layer 2 へ委譲、 Annex D-4 + 改訂履歴 γ 行参照))
- 著作権者: 越川将人
- 関連 ADR:
...
## Annex D: γ 実装 completion record (= verify script 体系化 + ADR-0053 completion + Draft→Accepted)

### D-1: γ deliverable

軸 B 実装 sprint 2 γ = verify script 体系化 + ADR-0053 completion + Draft → Accepted 移行 (= 39th session、 PR #82)。 driver touch なし (= verify script + ADR + dashboard のみ、 §決定 1 γ 行 literal)。

| deliverable | 内容 |
|---|---|
| `src/test-fixtures/axis-b/verify-axis-b-sram-placement.sh` | §決定 5 の verify gate 6 件を体系化した新規 verify script。 production build (= 静的 gate 1-5、 `.lst` の `.equ` resolve 値抽出) + `verify-axis-b-v2-entry.sh` (= gate 6 baseline) |
| ADR-0053 completion | Annex D + sub-sprint chain γ 行 + 改訂履歴 + 状態行 Draft → Accepted |

### D-2: verify-axis-b-sram-placement.sh = 6 gate 構成

| gate | 種別 | 検証 |
|---|---|---|
| gate 1 | static | sub-region 非重複 + 合計 = driver_state 64 + PartWork 256 + reserved 327 = 647 byte (= region 境界定数の差分算術) |
| gate 2 | static | region 境界定数 = `pmdneo_v2_driver_state_base` 0xFD39 / `pmdneo_v2_partwork_base` 0xFD79 / `pmdneo_v2_reserved_base` 0xFE79 が §決定 2 案 A と一致 |
| gate 3 | static | 既配置 3 field (= `pmdneo_v2_fade_level` 0xFD39 / `pmdneo_v2_ssg_mixer` 0xFD3A / `pmdneo_v2_entry_marker` 0xFD3B) placement 不変 + 全て driver_state region [0xFD39,0xFD79) 内 |
| gate 4 | static | 既存 SRAM layout 0xF820-0xFD38 不変 (= `part_workarea` 0xF820 + `PART_COUNT` 20 / `PART_WORKAREA_SIZE` 64 / `driver_pne_filename_buf` 0xFD20 / `driver_pne_filename_adr_word` 0xFD30 / `driver_pne_sample_table_id` 0xFD32 / 軸 G scratch 6 symbol 0xFD33-0xFD38 = `ppc_scratch_start_lsb/msb` + `ppc_scratch_stop_lsb/msb` + `audition_frame_counter_lsb/msb` = ADR-0006/0022/0023/0048 layout shift なし) |
| gate 5 | static | 命名規約 = v2 region [0xFD39,0xFFC0) に resolve する `.equ` symbol が全て `pmdneo_v2_` prefix |
| gate 6 | baseline | `verify-axis-b-v2-entry.sh` 7 gate (= 内部で `verify-fadeout-semantics.sh` 16 + `verify-mute-semantics.sh` 7 + baseline 9 script + `verify-ssg-tone-enable.sh` 15 gate を transitively) 全 PASS |

### D-3: γ 検証結果

- `verify-axis-b-sram-placement.sh` 6 gate 全 PASS (= 39th session 実行確認、 EXIT 0)
  - gate 1 = driver_state 64 + PartWork 256 + reserved 327 = 647
  - gate 2 = region 境界定数 0xFD39/0xFD79/0xFE79
  - gate 3 = 既配置 3 field 0xFD39/0xFD3A/0xFD3B、 driver_state region 内
  - gate 4 = 既存 SRAM layout 0xF820-0xFD38 不変 (= part_workarea 0xF820 + PART_COUNT/SIZE + driver_pne_filename_buf/adr_word/sample_table_id 0xFD20/0xFD30/0xFD32 + 軸 G scratch 6 symbol 0xFD33-0xFD38)
  - gate 5 = v2 region の `.equ` symbol 全て `pmdneo_v2_` prefix
  - gate 6 = `verify-axis-b-v2-entry.sh` 7 gate 全 PASS
- driver touch なし (= verify-axis-b-sram-placement.sh 新規 + ADR-0053 + dashboard のみ)
- Codex layer 2 = γ 実装 review approve

### D-4: Draft → Accepted 移行

全 sub-sprint α/β/γ 完走 + §決定 5 verify gate 6 件 (= `verify-axis-b-sram-placement.sh`) 全 PASS をもって ADR-0053 を **Draft → Accepted** へ移行する。

§決定 9 で本 sprint 2 は ADR-0041 §決定 4-2 Codex rescue 化 + 39th session 完全自走 model を継承し、 Accepted 移行判断を Codex layer 2 へ投入する規律とした。 39th session で user が「完全自律で user 報告は最終完走時のみ + それまで Codex rescue 経由 GO」 と明示しており、 Draft → Accepted 移行の根拠 = **user 判断を Codex layer 2 へ委譲 + γ 内 Codex approve 経由確定** (= memory `feedback_codex_layer2_implementation_review_delegation.md` 39th session 完全自走 model、 ADR-0052 ε の Draft→Accepted と同形式)。

ADR-0053 Accepted = **軸 B 実装 sprint 2 (= δ-2 PartWork/SRAM placement) の完了** であり、 軸 B 全体の完了ではない (= 「軸 B 完成」 表現不使用)。 軸 B 実装 sprint chain の残 = 実装 3-4 (= δ-3 F-2-B integration / δ-4 軸 C/G/rhythm 接続点)。
...
**進捗 (= α/β/γ 完走 = δ-2 完了)**: α で設計書 (= 本 ADR-0053) を起票し、 区画の範囲・順序・既存 3 変数の分類・検証方法を文書で固定した。 β でドライバ (= `standalone_test.s`) に区画の境界定数 3 つ (= driver_state / PartWork / reserved の各先頭アドレス) を追加した (= ドライバの機械語は変わらない = ビルド結果のバイト列は β 前と同一)。 γ で検証スクリプト (= `verify-axis-b-sram-placement.sh`、 区画の非重複・命名規約・既存 layout 不変を機械的に確認する 6 項目) を整備し、 全 6 項目 PASS を確認した。
...
**次**: 全 sub-sprint α/β/γ 完走 + 検証 6 項目 PASS をもって ADR-0053 は Draft → Accepted へ移行 (= δ-2 完了)。 軸 B 実装 sprint chain の次は実装 3 = δ-3 (= F-2-B ch3 4-op individual mode integration)。

## sub-sprint chain 進捗

| sub | 状態 | PR | Codex layer 2 review |
|---|---|---|---|
| α (= ADR-0053 起票) | **完了** (= 39th session、 PR #80) | PR #80 | 起票 plan review approve (= 論点 1 案 A / 論点 2 3 段、 escalate なし、 規律 5 観点 PASS) + 起票 review revise (= must-fix 1 = sprint 番号訂正) → 修正 → approve |
| β (= driver region 境界定数 + comment block) | **完了** (= 39th session、 PR #81) | PR #81 | β 実装 review approve |
| γ (= verify script 体系化 + completion + Accepted 判断) | **完了** (= 39th session、 PR #82) | PR #82 | γ 実装 review approve |
...
| 2026-05-21 | γ 実装完了 + Draft → Accepted (= 39th session、 PR #82) | verify script 体系化 + ADR-0053 completion + Draft → Accepted 移行。 §決定 5 の verify gate 6 件を `src/test-fixtures/axis-b/verify-axis-b-sram-placement.sh` に体系化 (= production build .lst の `.equ` resolve 値抽出による静的 gate 1-5 = sub-region 非重複/region 境界定数/既配置 3 field placement/既存 layout 不変/命名規約 + verify-axis-b-v2-entry.sh baseline gate 6)。 Annex D 追記 (= γ completion + 6 gate 構成 + 検証結果 + Draft→Accepted 移行根拠) + sub-sprint chain γ 完了 reflect + 状態行 Draft → Accepted。 検証 = verify-axis-b-sram-placement.sh 6 gate 全 PASS (EXIT 0)。 driver touch なし (= verify script 新規 + ADR-0053 + dashboard のみ)。 Codex layer 2 = γ 実装 review approve。 Draft → Accepted 移行は §決定 9 = 完全自走 model で user 判断を Codex layer 2 へ委譲し γ 内 Codex approve 経由確定。 ADR-0053 Accepted = 軸 B 実装 sprint 2 = δ-2 完了、 軸 B 全体は未完了 (= 「軸 B 完成」 表現不使用、 残 = 実装 3-4 = δ-3/δ-4) |
