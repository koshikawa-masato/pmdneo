...
| gate 4 | static | 既存 SRAM layout 0xFD20-0xFD38 不変 (= `driver_pne_filename_adr_word` 0xFD30 / `driver_pne_sample_table_id` 0xFD32 / `ppc_scratch_start_lsb` 0xFD33 / `audition_frame_counter_lsb` 0xFD37 = ADR-0022/0023/0048 layout shift なし) |
...
- `verify-axis-b-sram-placement.sh` 6 gate 全 PASS (= 39th session 実行確認、 EXIT 0)
  ...
  - gate 4 = 既存 SRAM layout 0xFD30/0xFD32/0xFD33/0xFD37 不変
...
**進捗 (= α/β 完了)**: α で設計書 (= 本 ADR-0053) を起票し、 区画の範囲・順序・既存 3 変数の分類・検証方法を文書で固定した。 β でドライバ (= `standalone_test.s`) に区画の境界定数 3 つ (= driver_state / PartWork / reserved の各先頭アドレス) を追加した。 ドライバの機械語は変わらない (= 定数はまだ命令から参照されないため、 ビルド結果のバイト列は β 前と同一)。
...
**次**: γ sub-sprint で検証スクリプト (= 区画の非重複・命名規約・既存 layout 不変を機械的に確認) を整備し、 ADR-0053 を Draft → Accepted へ移行する。
