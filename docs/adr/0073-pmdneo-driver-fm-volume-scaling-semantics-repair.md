...
- 状態: **Accepted** (= 2026-05-27 43rd session ε 完走 milestone = ADR-0073 sub-sprint α/β/γ/ε 全完了 + γ runtime functional verify ALL PASS = δ-5 primary success metric wav RMS = -29.59 dBFS (= ADR-0072 ε -76 dBFS quiet 根本治療達成、 +46.41 dB 改善) + 4 build matrix B1-B4 ALL PASS + .lst predicate 6 件 ALL PASS + δ-1〜δ-7 ALL PASS、 driver patch 確定 = 修正 routine 5 件 (= `pmdneo_fm_voice_set_default` cache 初期化 + `pmdneo5_init_part` FM part 限定 default vol 108 + `comv` byte 直接 store + `comat` voice ptr cache 保存 + `fm_volume_hook` dispatch) + 新規 routine `fm_volume_hook_pmddotnet` (= ~180 byte PMD V4.8s `volset_slot` 数式 + per-alg carrier mask 経路 port + per-op helper 4 件) + 新規 data `fm_carrier_table` 8 byte (= PMD V4.8s `PMD.ASM:7958-7964` literal port) + 新規 SRAM equ 7 件 (= 0xFD62-0xFD6D 12 byte voice ptr cache、 PMDDOTNET 経路限定)、 patch byte 量 ~241 byte 増 net (= flag-on path)、 全 guarded change `.if PMDNEO_USE_PMDDOTNET ... .endif` 配下限定 = flag-off byte-identical 維持 (= active baseline `b15883fe...` 維持 confirmed)、 baseline 訂正 (= γ-1-1 § user 明示 mandate option (a) 採用 = 「actual baseline への doc 訂正」、 `b15883fe...` を canonical active baseline、 `457a237c...` は ADR-0071 ε era transient record として immutable carry)。 ε Accepted 後解禁 wording「PMDNEO driver FM volume scaling semantics repair 完了」 (= 併記必須 7 件 mandatory = (i) metric pass であり aesthetic approve ではない + (ii) (d) audition gate 達成ではない + (iii) roadmap ⑥ audition 完了ではない + (iv) production-ready 全体達成ではない + (v) 軸 B 完成ではない + (vi) 軸 G 完成ではない + (vii) 本番 cmd 切替完了ではない)、 main agent autonomous で完走 (= ADR-0041 §決定 4-2 Codex Rescue 化 default + §決定 4-3 retrospective Codex review pattern、 計 Codex layer 2 review 8 round chain = sprint β plan review 4 round chain approve + sprint γ impl-review 2 round chain approve + sprint α/ε plan review 各 1 round approve)。 sub-sprint chain α (= 5 並走 sub-agent investigation + Annex α 6 sub-section) + β (= plan v1→v2→v3 iteration + Codex Rescue plan review 4 round chain approve + 軽微 fix-up) + γ (= driver patch implementation + Codex Rescue impl-review 2 round chain approve + γ runtime functional verify ALL PASS) + ε (= 本 sprint = Draft → Accepted + Annex ε fill + wording 解禁) 全完了。 atomic 1 セット規律 15 回目適用完走 (= PR #142+...+#157)、 退避 branch + scope-out branch + user 別作業 branches 全 untouched。 user 明示 mandate ε scope literal = 「Draft → Accepted + Annex ε fill + γ runtime functional verify ALL PASS 正式記録 + ADR-0072 ε -76 dBFS quiet 根本治療達成記録 + wording 解禁 + 併記必須 7 件明記 + dashboard update + Codex Rescue review + main agent merge + branch 削除 atomic + memory update」 + 禁止 mandate literal = 「user audition 進めない + ADR-0065 δ session 自動開始しない + production-ready / 軸 B 完成 / 本番 cmd 切替 肯定しない」)
- 起票日: 2026-05-27
...
> **supersede pointer (= ADR-0073 sprint γ 着手時、 2026-05-27 43rd session)**: 本 §決定 3 + plan v1/v2/v3 内「`457a237c...`」 active baseline reference は ADR-0071 ε Accepted 直後の transient build artifact による誤記録。 main agent 直接 verify (= `make clean` + `PMDNEO_USE_PMDDOTNET=0 scripts/build-poc.sh` + sha256sum) で actual build = **`b15883fe59804a201e13d0c05f083c1c3dd31fbfb1efd193b34d550d18f561e4`** 確定、 patch revert 前後で同 sha256 confirmed = patch 起因ではない (= guarded change `.if PMDNEO_USE_PMDDOTNET` flag-off byte-identical mandate 守られている) = **「actual baseline への doc 訂正」** (= 「production baseline 変更」 ではない、 user 明示 mandate option (a) 採用)。 詳細 = Annex γ-1 §。 本 §決定 3 + plan v1/v2/v3 内「`457a237c...`」 references は immutable history mandate (= memory `feedback_doc_governance_two_systems.md`) carry。
...
plan v2 β-2-8 **18 condition 完全 carry**、 #1〜#13 = ADR-0071 §決定 6 + ADR-0072 §決定 6 + ADR-0069 §決定 5 from inheritance、 #14〜#18 = ADR-0073 固有。 各 condition の trigger executable command:

#### #1〜#13 trigger pointer (= ADR-0071/0072/0069 source 完全継承、 ADR-0073 で新規 trigger logic 追加なし):

| # | trigger source | 実行可能 command |
|---|---|---|
| **#1** production sha256 mismatch | active baseline `457a237c...` byte 違い (= ADR-0071 §決定 6 #1 / ADR-0072 §決定 6 #1 / ADR-0069 §決定 5 #1) | `sha256sum vendor/ngdevkit-examples/00-template/build/rom/243-m1.m1` 比較 (= β-3-7 § gate γ-2 同経路) → 不一致時 即 halt + revert |
| **#2** sub-agent worktree base ref mismatch (= ADR-0069 §決定 5 #2、 軽微 fix-up 対象) | preflight pwd / HEAD 不一致 | sub-agent preflight 4 件 check (= memory `feedback_subagent_isolation_worktree_base_ref_mismatch.md` 9 件 guard) → fail-fast 即 restart |
| **#3** vendor 改変 (= ADR-0069 §決定 5 #3) | `vendor/` directory 変更 | `git diff --name-only vendor/` 空 confirm → 違反時 即 halt + revert |
| **#4** doc 内 typo / 誤記 (= ADR-0069 §決定 5 #4、 軽微 fix-up 対象) | wording stale | Codex Rescue review finding → 連鎖 commit fix-up (= rollback ではない) |
| **#5** ADR-0048 軸 G ε partial state placement 違反 (= ADR-0069 §決定 5 #5) | 0xFD32-0xFD38 touch | `.lst` symbol table + grep `0xFD3[2-8]` → 違反時 即 halt + revert |
| **#6** ADR-0026 §決定 3/4 K dispatch L ch 固定占有 違反 (= ADR-0069 §決定 5 #6) | `pmdneo_rhythm_event_trigger` body modify | `git diff src/driver/standalone_test.s` で該当 routine 行 untouched confirm → 違反時 即 halt + revert |
| **#7** ADR-0051 `pmdneo_ssg_tone_sync` reg 0x07 RMW owner contract 違反 (= ADR-0069 §決定 5 #7) | `pmdneo_ssg_tone_sync` body modify | `git diff` で該当行 untouched confirm → 違反時 即 halt + revert |
| **#8a** 軽微 wording fix-up (= ADR-0069 §決定 5 #8a) | minor typo carry | 連鎖 commit fix-up |
| **#8b** Codex retrospective review finding fix-up (= ADR-0069 §決定 5 #8b) | retrospective revise | 連鎖 commit fix-up |
| **#9** ADR-0058 v2 PartWork 不可触 mandate 違反 (= ADR-0069 §決定 5 #9) | PART_OFF 新規 field 追加 | `grep "^.equ.*PART_OFF" src/driver/standalone_test.s` 既存 field 数同一 confirm → 違反時 即 halt + revert |
| **#10** ADR 全体 halt = scope 変更 / design 大幅 pivot (= ADR-0069 §決定 5 #10) | sub-sprint chain 全体 revert 必要 | user 明示 GO 必須 + Draft 維持 + 全 sub-sprint rollback record |
| **#11** destructive git 操作 trigger (= ADR-0069 §決定 5 #11) | `git reset --hard` / `git push --force` / `git checkout --` 等 | user 明示なし禁止、 `git revert` のみ採用 |
| **#12** voice load failure on PMDDOTNET path (= ADR-0072 §決定 6 #12) | MAME ymfm-trace で voice register writes 0 件 | `grep -E "^[0-9]+\s+A\s+(30\|40\|50\|60\|70\|80\|B0)" /tmp/adr-0073/d1-ymfm.tsv \| wc -l` > 0 confirm → 0 件時 即 halt + revert |
| **#13** audio silent persistence after γ impl (= ADR-0072 §決定 6 #13) | δ-5 Layer 1 silent | `jq -r '.layers["1_wav_hygiene"].metrics.rms_dbfs' d5-analysis.json` extremely low → 1 retry → 再 silent なら halt |

#### #14〜#18 trigger 実行可能 command (= ADR-0073 固有):
...
## Annex γ-1: sprint γ driver patch impl + baseline 訂正記録 (= 2026-05-27 43rd session)

### γ-1-0: user 明示 GO 受領 + baseline 訂正 mandate

ADR-0073 sprint β PR2 完了 (= PR #156 MERGED at `fd9831d` + Codex Rescue plan review 4 round chain approve) 後の sprint γ 起票 = driver patch implementation。 user 明示 GO mandate literal:

- fix A/B/C 範囲限定
- `src/driver/standalone_test.s` 指定 routine 周辺のみ touch
- `.if PMDNEO_USE_PMDDOTNET ... .endif` guarded change 限定
- production sha256 維持 mandatory
- 4 build matrix B1-B4 実施
- .lst predicate 6 件確認
- δ-5 Layer 1 PASS primary success metric
- ADR-0072 voice delivery regression 防止必須
- rollback 18 condition 適用
- Codex Rescue impl-review 必須
- merge 後 branch 削除 atomic

### γ-1-1: baseline 訂正 (= `457a237c...` → `b15883fe...`、 user 明示 mandate option (a) 採用)

γ 着手時 B1 production baseline build verify で **doc 内記録と actual build mismatch 検出**:

| 軸 | sha256 |
|---|---|
| **ADR-0073 doc 内 baseline 期待値** (= plan v1/v2/v3 + §決定 3 carry) | `457a237cd696e09bc99f707d13bc8851c75faf7225eee5e0d4c7111980ca9092` |
| **actual B1 build (= patch 適用後 PMDNEO_USE_PMDDOTNET=0)** | `b15883fe59804a201e13d0c05f083c1c3dd31fbfb1efd193b34d550d18f561e4` |
| **patch revert 後 build (= 切り分け)** | `b15883fe59804a201e13d0c05f083c1c3dd31fbfb1efd193b34d550d18f561e4` (= 同じ) |
| **ADR-0048〜0070 era historical baseline** | `b15883fe59804a201e13d0c05f083c1c3dd31fbfb1efd193b34d550d18f561e4` (= 同じ) |

#### 切り分け結果 + user 判断

- patch revert 前後で同 sha256 = **ADR-0073 patch 起因ではない** (= guarded change `.if PMDNEO_USE_PMDDOTNET` flag-off byte-identical mandate 守られている)
- doc 内 `457a237c...` = ADR-0071 ε Accepted 直後 (= commit `d4ea252`) で record されたが、 ADR-0072 ε merge 経由で actual baseline は `b15883fe...` に戻った推定 (= ADR-0072 §決定 driver source 完全 no-touch confirmed と整合)
- user 明示 mandate option (a) 採用 = **actual baseline `b15883fe...` を canonical active baseline** として扱い、 ADR-0073 γ verify baseline を `b15883fe...` に訂正、 `457a237c...` は ADR-0071 ε era の transient record として扱う

#### 訂正 scope (= user 明示 mandate literal)

- ADR-0073 §決定 3 + Annex β-3 内 plan v3 baseline reference = **immutable history mandate carry** (= supersede pointer 追加で訂正、 literal 直接書換禁止)
- ADR-0073 γ verify baseline (= 本 Annex γ-1 + 4 build matrix B1-B4 期待値) = **`b15883fe...`** で進行
- dashboard `production baseline` section = **`b15883fe...` を canonical active baseline + `457a237c...` を transient reference** へ訂正
- ADR-0071/0072 historical record 内 `457a237c...` literal = **書換禁止** (= memory `feedback_doc_governance_two_systems.md` mandate carry)

#### 禁止 scope 明示 (= user mandate)

- `457a237c...` 維持のために sprint γ halt しない
- patch を疑って rollback しない
- ADR-0072 ε merge 全履歴調査に脱線しない
- 「production baseline 変更」 と誤記しない (= 今回は「actual baseline への doc 訂正」)

### γ-1-2: driver patch impl 内容 literal (= plan v3 Annex β-3-3 / β-3-4 反映)

修正 routine 4 件 + 新規 routine 1 件 + 新規 data 1 件 + 新規 SRAM equ 7 件、 全 guarded change `.if PMDNEO_USE_PMDDOTNET ... .endif` 配下限定:

| # | 対象 | line | 修正内容 | patch byte (= 推定 in flag-on) |
|---|---|---|---|---|
| 1 | SRAM equ block | line 291 直後 | `driver_pmddotnet_fm_voice_ptr_base` + ch0-ch5 計 7 件 equ (= 0xFD62-0xFD6D 12 byte 占有) | 0 byte (= equ は size 占有なし) |
| 2 | `pmdneo_fm_voice_set_default` | line 1311-1313 | guarded change additive = cache 初期化 logic (= ch_idx 0-5 限定、 default voice load 経路 cover) | ~30 byte 増 |
| 3 | `pmdneo5_init_part` | line 3481-3505 | guarded change additive = FM part 限定 default vol 108 (= `cp #PART_SSG1` 判定経路) | ~8 byte 増 |
| 4 | `comv` | line 3972-4003 | guarded change = byte 直接 store (= PMD V4.8s comv:3406-3411 semantic port) + 既存 logic `.else` 配下完全保存 | ~25 byte 減 (= flag-on path shortcut) |
| 5 | `comat` | line 4419-4439 | guarded change additive = FM ch 別 voice ptr cache 保存 logic | ~25 byte 増 |
| 6 | `fm_volume_hook` | line 4472-4529 | guarded change dispatch `jp fm_volume_hook_pmddotnet` | ~3 byte 増 |
| 7 | `fm_carrier_table` | 新規 (= line 4626 後) | 新規 data 8 byte (= PMD V4.8s `PMD.ASM:7958-7964` literal port) | 8 byte 増 |
| 8 | `fm_volume_hook_pmddotnet` | 新規 (= line 4626 後) | 新規 routine + helper 4 件 (= `_fmvhp_write_s0/s1/s2/s3` + per-op TL additive + carrier mask 経路) | ~180 byte 増 (= plan v3 ~120 byte 推定から拡大) |
| | **合計 net** | | | **~229 byte 増 net** (= plan v3 ~166 byte 推定から +63 byte) |

### γ-1-3: 4 build matrix B1-B4 verify (= `b15883fe...` baseline 訂正反映)

| build | flag | fixture | 期待 sha256 |
|---|---|---|---|
| **(B1) production baseline** | `PMDNEO_USE_PMDDOTNET=0` | no PMDDOTNET_MML | **== `b15883fe...` byte-identical** |
| **(B2) post-patch flag-off** | `PMDNEO_USE_PMDDOTNET=0` + ADR-0073 patch | no PMDDOTNET_MML | **== (B1) byte-identical** (= guarded change flag-off 完全無効化) |
| **(B3) flag-on pre-patch** | `PMDNEO_USE_PMDDOTNET=1` + ADR-0072 ε state (= 本 ADR-0073 patch 未適用) | `src/test-fixtures/adr-0073/test-volume-baseline.mml` | ADR-0072 ε baseline (= 別値、 γ impl 内 record) |
| **(B4) flag-on post-patch** | `PMDNEO_USE_PMDDOTNET=1` + ADR-0073 patch | 同上 | (B3) と diff = ADR-0073 patch byte 量のみ |

### γ-1-4: `.lst` predicate 6 件 verify (= plan v3 β-3-6 literal)

1. `fm_carrier_table` symbol assemble PASS (= 0x0610 セクション末尾配置)
2. `fm_volume_hook_pmddotnet` symbol assemble PASS
3. 新規 SRAM equ 7 件 (= `driver_pmddotnet_fm_voice_ptr_chN` × 6 + base) assemble PASS
4. 既存 `comv` / `fm_volume_hook` / `pmdneo5_init_part` / `comat` routine body の `.else` 配下 byte-identical 維持
5. `pmdneo_fm_voice_set_default` の `.else` 配下 byte-identical 維持
6. 既存 symbol table 順序不変

### γ-1-5: 新規 fixture + verify script (= plan v3 β-3-5 allowed-touch literal)

- `src/test-fixtures/adr-0073/test-volume-baseline.mml`
- `src/test-fixtures/adr-0073/test-v-shift-coverage.mml`
- `src/test-fixtures/adr-0073/expected-events.json`
- `src/test-fixtures/adr-0073/verify-volume-scaling.sh` (= fail-fast `set -euo pipefail` + 実 CLI 整合)

### γ-1-6: δ functional verify ALL PASS (= 2026-05-27 43rd session、 main agent autonomous 実行)

#### 4 build matrix 完走

| build | flag + fixture | sha256 | 結果 |
|---|---|---|---|
| B1 production baseline | `PMDNEO_USE_PMDDOTNET=0` | `b15883fe...` | **PASS** (= active baseline byte-identical) |
| B2 post-patch flag-off | 同上 | `b15883fe...` | **PASS** (= B1 byte-identical = guarded change mandate 達成) |
| B3 flag-on pre-patch | `PMDNEO_USE_PMDDOTNET=1` + ADR-0072 ε state (= `78a1087`) + `test-volume-baseline.mml` | `ab47039fdefd014ee95b91f8c0278e00702564d1c6a9d81063ce9ae051a2363a` | recorded |
| B4 flag-on post-patch | 同 flag + ADR-0073 patch + 同 fixture | `6a304e2a247c2f1c2176ed4cc83e14f7e19a58d71c71fc3787dee7a8e932986b` | recorded |
| **(B3) vs (B4) byte diff** | | | **4144 byte** (= 中間 insert reference relocation 由来 normal、 .lst overflow なし confirmed) |

#### `.lst` predicate 6 件 PASS (= γ-3 B4 flag-on)

1. `fm_carrier_table` @ 0x10D4 assemble = **PASS**
2. `fm_volume_hook_pmddotnet` @ 0x10DC assemble = **PASS**
3. SRAM equ 7 件 (= 0xFD62-0xFD6D) assemble = **PASS**
4. 既存 `comv` / `fm_volume_hook` / `pmdneo5_init_part` / `comat` routine body `.else` byte-identical = **PASS** (= B1==B2)
5. `pmdneo_fm_voice_set_default` `.else` byte-identical = **PASS**
6. 既存 symbol 9 件存在 = **PASS**

#### δ-1〜δ-7 verify (= MAME runtime trace + wav render)

| gate | scope | 結果 |
|---|---|---|
| **δ-1** | OP4 carrier TL = 0x00/0x02 (= max output 近傍) | **PASS** (= PMD V4.8s `volset_slot` 数式 v15 + voice TL=0 → ~2 計算整合) |
| **δ-2** | modulator TL voice data exact = OP1=0x11 + OP2=0x19 + OP3=0x26 | **PASS** (= fix B carrier mask 経路 = modulator untouched、 voice 音色設計保存) |
| **δ-3** | ADR-0072 voice opcode dispatch regression-free | **PASS** (= voice 001 TL exact load 観測) |
| δ-4 | 既存 18+ verify script regression-free | flag-off byte-identical mandate 達成 = production-path regression-free 保証 (= ALL run sample 別 task scope) |
| **δ-5 (= primary success metric)** | `analyze-audition-wav.py` Layer 1 wav RMS > -60 dBFS | **PASS = -29.59 dBFS** (= +46.41 dB 改善、 ADR-0072 ε -76 dBFS quiet 根本治療達成) |
| δ-6 | ADR-0050 fade factor 経路継承 | flag-off byte-identical mandate 達成で間接 confirm |
| **δ-7** | v+/v- emit coverage = `xxd pmddotnet_song.m \| grep` 0xDE/0xDD count | **PASS = 0 件** (= LR-2 scope-out OK、 #18 rollback condition 不発) |

#### audio quiet 根本治療 evidence

- ADR-0072 ε baseline: -76 dBFS RMS (= quiet、 audition material 不成立)
- **ADR-0073 γ patch 後**: **-29.59 dBFS RMS** (= audible、 Layer 1 threshold > -60 dBFS PASS)
- **改善 = +46.41 dB** = primary success metric 達成

#### metric pass vs aesthetic approve 規律遵守 (= user 明示 mandate)

本 δ-5 verify は **engineering metric pass** = wav RMS Layer 1 threshold pass。 **aesthetic approve / user audition には進まない** (= memory `feedback_metric_pass_is_not_aesthetic_pass.md` 規律下、 ADR-0065 ε δ session 別 task scope)。 metric pass ≠ audition gate 達成 (= 併記必須 mandatory carry)。

### γ-1-7: Codex Rescue impl-review 完走 (= 2 round chain approve、 PR #157 MERGED at `e6c9442`)

sprint γ Codex Rescue impl-review 2 round chain approve = round 1 (= agentId `a84e13b690892737e`、 5m 29s) revise + 1 must-fix critical (= `pmdneo_fade_scale` clobber C+DE preserve mandate) → fix-up commit `8af1677` → round 2 attempt 1 placeholder = 機械復旧 retry (= 6 回目) → round 2 retry (= agentId `a17b130fab2968049`、 2m 24s) approve。 PR #157 MERGED at `e6c9442` + atomic 1 セット規律 15 回目適用完走。 後続 = δ runtime functional verify (= 本 γ-1-6 § 完走) → ε Accepted milestone = user 明示 GO 必須。

### γ-1-7: Codex Rescue impl-review 投入 mandate (= user 明示 mandate literal)

review 重点軸 = (1) baseline 訂正の根拠 (= patch 起因ではない、 patch revert 前後同 sha256、 actual build mismatch、 doc 側 stale) を必ず含む + (2) plan v3 fix A/B/C 実装 literal 整合 + (3) allowed-touch limited + (4) 4 build matrix B1-B4 verify 結果 + (5) .lst predicate 6 件 + (6) ADR-0072 既存機能 regression 防止 + (7) 越権操作なし confirmed。

## Annex ε: ADR-0073 sprint ε Accepted milestone (= 2026-05-27 43rd session)

### ε-0: user 明示 GO 受領 + ε scope mandate

ADR-0073 sprint γ runtime functional verify ALL PASS 完走 (= 集約 commit `a41c755`) 後の sprint ε Accepted milestone 起票。 user 明示 GO mandate literal:

> 「ADR-0073 sprint ε Accepted milestone を起票してください。 これは、 PMDDOTNET MML を再生したときに音量が極端に小さくなる問題について、 driver 側の FM volume scaling 修理と runtime verify が完了したことを正式記録するための doc-only sprint です。 ADR-0073 では comv / fm_volume_hook / FM default volume の扱いを修正し、 ADR-0072 時点で -76 dBFS だった音量が -29.59 dBFS まで改善しました。 この sprint ε が完了すると、 『PMDNEO driver FM volume scaling semantics repair 完了』 という表現が使えるようになります。 ただし、 これは engineering metric pass であり、 越川氏 audition approve や production-ready 達成ではありません。」

### ε-1: ADR-0073 全 sub-sprint chain 完走 record

| sub-sprint | scope | 完了経路 | 完了判定 |
|---|---|---|---|
| **α** | root cause investigation + Annex α 6 sub-section fill + Draft 起票 | PR #155 MERGED at `91ad9be` (= Codex Rescue plan review 1 round approve) | 真の root cause 確定 = driver-side 3 件 semantic divergence (= `comv` 二重 v→V 変換 + `fm_volume_hook` voice TL replace + per-alg carrier mask 不在) + (+) default vol mismatch + 仮説 A reject + fix A/B/C 並列 record |
| **β** | plan iteration + Codex Rescue plan review chain + fix A/B/C 採用案確定 | PR #156 MERGED at `fd9831d` (= Codex Rescue plan review 4 round chain approve = round 1 revise + plan v2 → round 2 retry revise + plan v3 → round 3 revise + 軽微 fix-up → round 4 approve) | Annex β-1/β-2/β-3 fill (= plan v1→v2→v3 iteration、 plan v3 = ground truth) + fix A+B+C 統合採用 + allowed-touch literal + 4 build matrix B1-B4 verify command + .lst predicate 6 件 + δ verify gate + rollback 18 condition |
| **γ** | driver patch implementation + 4 build matrix verify + Codex Rescue impl-review | PR #157 MERGED at `e6c9442` (= Codex Rescue impl-review 2 round chain approve = round 1 revise critical bug (= `pmdneo_fade_scale` clobber preserve mandate) + fix-up → round 2 retry approve) + γ runtime functional verify 完走 (= 集約 commit `a41c755`、 main agent autonomous) | 修正 routine 5 件 + 新規 routine + 新規 data + 新規 SRAM equ 7 件 + γ runtime functional verify ALL PASS = δ-5 primary success metric wav RMS -29.59 dBFS (= +46.41 dB 改善) + 4 build matrix ALL PASS + .lst predicate 6 件 ALL PASS + δ-1〜δ-7 ALL PASS + baseline 訂正 `b15883fe...` |
| **ε (= 本 sprint)** | Draft → Accepted + Annex ε fill + wording 解禁 + 併記必須 7 件 明記 + dashboard update | **PR #158 起票 HEAD `b5a6c7e`** = Codex Rescue review 投入 (= agentId `a71c53fc52481e20c` = AXIS-EPS-1〜8 review) + main agent merge + atomic 1 セット規律 16 回目適用予定 (= PR #142+...+#157+#158) | doc-only sprint = ADR doc 状態 final + Annex ε literal + 改訂履歴 ε entry + 平易要約 ε section + dashboard 0073 行 final |

### ε-2: γ runtime functional verify ALL PASS 正式記録

| verify item | result |
|---|---|
| **4 build matrix B1-B4** | ALL PASS (= B1/B2 byte-identical `b15883fe...` + B3 `ab47039f...` + B4 `6a304e2a...` + B3vsB4 4144 byte diff normal) |
| **.lst predicate 6 件** | ALL PASS (= `fm_carrier_table` @ 0x10D4 + `fm_volume_hook_pmddotnet` @ 0x10DC + SRAM equ 7 件 + 既存 routine body `.else` byte-identical + `pmdneo_fm_voice_set_default` `.else` byte-identical + 既存 symbol 9 件存在) |
| **δ-1** OP4 carrier TL = 0x00/0x02 max output 近傍 | PASS (= PMD V4.8s `volset_slot` 数式整合) |
| **δ-2** modulator TL voice data exact = OP1=0x11+OP2=0x19+OP3=0x26 | PASS (= fix B carrier mask 経路動作、 modulator untouched) |
| **δ-3** ADR-0072 voice opcode dispatch regression-free | PASS |
| δ-4 既存 verify regression-free | 間接 confirm (= flag-off byte-identical) |
| **δ-5 (= primary success metric)** Layer 1 wav RMS > -60 dBFS | **PASS = -29.59 dBFS** = +46.41 dB 改善 |
| δ-6 ADR-0050 fade factor 経路継承 | 間接 confirm (= flag-off byte-identical) |
| **δ-7** v+/v- emit coverage = 0 件 | PASS (= LR-2 scope-out OK、 #18 rollback condition 不発) |

### ε-3: ADR-0072 ε -76 dBFS quiet 根本治療達成記録

| 軸 | RMS | 状態 |
|---|---|---|
| ADR-0072 ε baseline (= ADR-0072 §決定 7 + Annex β-5 ε milestone record) | -76 dBFS | quiet (= audition material としては不成立) |
| **ADR-0073 γ runtime functional verify 後 (= 本 ε milestone)** | **-29.59 dBFS** | **audible (= Layer 1 threshold > -60 dBFS PASS)** |
| 改善 | **+46.41 dB** | primary success metric 達成 = ADR-0072 ε quiet 根本治療達成 |

### ε-4: 「PMDNEO driver FM volume scaling semantics repair 完了」 wording 解禁 + 併記必須 7 件 mandatory carry

ε Accepted 後解禁 wording = **「PMDNEO driver FM volume scaling semantics repair 完了」**。 ただし以下 7 件併記 mandatory (= user 明示 mandate literal):

1. **metric pass であり aesthetic approve ではない** (= memory `feedback_metric_pass_is_not_aesthetic_pass.md` 規律 = engineering Layer 1 wav RMS threshold pass、 越川氏 audition approve は別軸)
2. **(d) audition gate 達成ではない** (= ADR-0065 ε δ session scope、 audition material 再設計 + 越川氏 audition session 必要)
3. **roadmap ⑥ audition 完了ではない** (= ADR-0065 全体 = audition validation gate framework + δ session 経由 + 越川氏 approve まで必要)
4. **production-ready 全体達成ではない** (= ADR-0066 候補 future = 本番 cmd 切替判断 + 越川氏 final approve まで多軸残課題)
5. **軸 B 完成ではない** (= 軸 B = Phase 2 FM/SSG driver フルスクラッチ全体、 残 sprint 1-4 等 + ADR-0070 K bitmap pair distinct + 他多数の 軸 B 実装 sprint chain)
6. **軸 G 完成ではない** (= ADR-0048 軸 G dynamic supply、 ε partial state placement 等の残課題)
7. **本番 cmd 切替完了ではない** (= ADR-0066 候補 future = `PMDNEO_USE_PMDDOTNET` flag default 切替判断、 user 明示 GO 必須)

### ε-5: 修正された問題と修正の literal

**修正対象問題** = PMDDOTNET MML を再生したときに audio 出力音量が極端に小さい問題 (= ADR-0072 ε で -76 dBFS RMS = audition material として実用不能):
- driver `comv` (= MML `v0`〜`v15` opcode handler) が二重 v→V 変換で全 v 値を v16=127 に丸めていた
- driver `fm_volume_hook` が voice TL を完全無視して全 4 op に同 TL を上書きしていた (= PMD V4.8s `volset_slot` の voice TL + ~vol additive + per-alg carrier mask 経由と根本的に異なる)
- driver `pmdneo5_init_part` default vol = 15 が fix A 適用後の byte 直接 store semantic と不整合 (= V byte semantics では 108 = PMD V4.8s 同等)

**修正内容** = `src/driver/standalone_test.s` 5 routine guarded change + 新規 routine `fm_volume_hook_pmddotnet` (= PMD V4.8s `volset_slot` 数式 line-by-line port + per-alg carrier mask 経由 + per-op TL additive `~V + voice_TL - 0x80 saturate` + carrier-only TL register write) + 新規 data `fm_carrier_table` (= ALG 8 値 → carrier slot mask、 PMD V4.8s `PMD.ASM:7958-7964` literal port) + 新規 SRAM equ 7 件 (= 0xFD62-0xFD6D voice ptr cache、 PMDDOTNET 経路限定):

1. `comv` (= line 3972-4003): byte 直接 store (= PMD V4.8s `PMD.ASM:3406-3411` semantic port、 二重 v→V 変換削除)
2. `fm_volume_hook` (= line 4472-4529): guarded change dispatch `jp fm_volume_hook_pmddotnet` (= flag-off は既存 logic 完全保存)
3. `comat` (= line 4419-4439): voice ptr cache 保存 logic 追加 (= `fm_volume_hook_pmddotnet` 再取得用、 FM ch 0-5 限定)
4. `pmdneo_fm_voice_set_default` (= line 1311-1313): init-time default voice load 経路 cache 初期化 (= `@`/`comat` を通らない経路の voice ptr cache 整備)
5. `pmdneo5_init_part` (= line 3481-3505): FM part 限定 default vol 108 override (= `cp #PART_SSG1` 判定経路、 SSG/PCM/ADPCM-A/Rhythm/FM3EXT untouched)

全 guarded change `.if PMDNEO_USE_PMDDOTNET ... .endif` 配下限定 = production default (= flag-off) では `b15883fe...` baseline byte-identical 維持。

### ε-6: 本 ε Accepted で次に進められること

| 後続 | 状態 |
|---|---|
| **ADR-0065 ε δ session 起票 dependency 解除** | dependency 解除 ready = user 明示 GO mandatory + user 介入 mandatory (= audition session 実施、 越川氏 aesthetic judgment) |
| ADR-0066 候補 (= 本番 cmd 切替 = `PMDNEO_USE_PMDDOTNET` flag default 切替判断) | ADR-0065 ε Accepted 後 future、 user 明示 GO 必須 |
| ADR-0070 候補 (= K bitmap pair distinct) | ADR-0069 γ Accepted で dependency 解除済、 user 明示 GO 必須 |
| 軸 B 残 sprint chain (= 軸 B 完成までの実装 sprint 1-4 等) | 各 user 明示 GO 必須 |

### ε-7: 禁止 mandate (= user 明示 mandate literal carry)

- **user audition に進まない** (= ADR-0065 ε δ session は user 介入 mandatory、 本 ε milestone は engineering metric pass 記録のみ)
- **ADR-0065 δ session 自動開始しない** (= user 明示 GO mandatory point、 main agent autonomous では着手しない)
- **production-ready / 軸 B 完成 / 本番 cmd 切替 肯定しない** (= 併記必須 7 件 mandatory carry、 単独 wording 禁止)

### ε-8: ADR-0073 全体 commit chain literal

| sub-sprint | commit hash | 説明 |
|---|---|---|
| α PR1 起票 | `50412b9` | ADR-0073 doc 新規起票 (= 8 決定 + Annex α 6 sub-section + Annex β-1 placeholder + 改訂履歴 + 平易要約) + dashboard 0073 行 add |
| α PR #155 merge | `91ad9be` | atomic 1 セット 13 回目適用 |
| α final maintenance | `93d9e23` | dashboard 0073 行 final update |
| β PR2 plan v1 起票 | `c5f9861` | Annex β-1 plan v1 literal fill |
| β plan v2 fix-up | `a37414e` | round 1 revise plan v2 (= fix C FM 限定 + SRAM voice ptr cache + sha256 artifact 修正) |
| β plan v3 fix-up | `7531e7d` | round 2 retry revise plan v3 (= α-2 supersede + `pmdneo_fm_voice_set_default` allowed-touch + 実 CLI 整合 + fail-fast + baseline 動的生成) |
| β round 3 軽微 fix-up | `d390da1` | dashboard stale literal + #1〜#13 trigger pointer 補強 |
| β PR #156 merge | `fd9831d` | atomic 1 セット 14 回目適用 |
| β final maintenance | `52bfcb3` | dashboard 0073 行 final update |
| γ PR3 driver patch 起票 | `fd55e4c` | 修正 routine 4 件 + 新規 routine + 新規 data + 新規 SRAM equ + 新規 fixture 3 件 + 新規 verify script + ADR doc Annex γ-1 fill + baseline 訂正 |
| γ round 1 critical bug fix-up | `8af1677` | `pmdneo_fade_scale` clobber push/pop preserve mandate |
| γ PR #157 merge | `e6c9442` | atomic 1 セット 15 回目適用 |
| γ final maintenance | `5c08c73` | dashboard 0073 行 final update |
| γ runtime functional verify | `a41c755` | 4 build matrix B1-B4 + .lst predicate 6 件 + δ-1〜δ-7 ALL PASS + δ-5 primary success metric -29.59 dBFS + fixture LF→CRLF + verify script bug fix-up |
| **ε** (= 本 sprint) | **`b5a6c7e`** (= 本 PR #158 起票 commit、 Codex Rescue review agentId `a71c53fc52481e20c` revise → 軽微 fix-up `<本 fix-up commit、 後続 PR head 更新時>` → round 2 retry approve 予定) | Draft → Accepted + Annex ε fill + wording 解禁 + 併記必須 7 件 |

### ε-9: Codex Rescue review 投入 mandate (= 本 ε sprint scope)

ε doc-only sprint = ADR doc 状態 Draft → Accepted + Annex ε fill + dashboard update。 Codex Rescue review 重点軸:
- ADR doc 状態 final transition 妥当性 (= γ runtime functional verify ALL PASS 完走 evidence)
- Annex ε literal 整合 (= sub-sprint chain α/β/γ/ε record + γ-1-6 § ALL PASS reference + ADR-0072 ε -76 dBFS quiet 根本治療 evidence)
- 「PMDNEO driver FM volume scaling semantics repair 完了」 wording 解禁 + 併記必須 7 件 mandatory carry 確認
- 禁止 mandate (= user audition / ADR-0065 δ session 自動開始 / production-ready / 軸 B 完成 / 本番 cmd 切替 肯定) 整合
- dashboard 0073 行 status final update 整合
- 越権操作なし (= 6 件 review-only mandate literal carry)

## Annex β-4 / β-5: placeholder (= sprint β future round iteration、 fill 予定なし = sprint β PR2 完走済)
...
- 2026-05-27: ADR-0073 sprint ε PR #158 round 1 Codex Rescue review (= agentId `a71c53fc52481e20c`、 elapsed 約 3m 41s) **revise** + 2 must-fix + 0 nh + 0 lr + 越権操作なし confirmed = per-axis verdict round 1 = AXIS-EPS-1 PASS (= 状態 Draft → Accepted final transition + δ-5 -29.59 dBFS + +46.41 dB 改善 + α/β/γ/ε 全完了 literal 整合) + **AXIS-EPS-2 FAIL** (= must-fix 2 = MF-ε-1 修正 routine 件数不一致 line 1679「4 件」 vs line 1723「5 routine」 + MF-ε-2 PR #NNN / HEAD placeholder 未填充 line 1680「PR #NNN 起票予定」 + line 1766「後続 PR 起票時に embed」) + AXIS-EPS-3/4/5/6/7/8 PASS、 must-fix 2 件 = MF-ε-1 修正 routine 件数 4 件 → 5 件 統一 (= ε scope literal 内、 ε-1 § + 状態行 carry literal、 γ Annex γ-1-2 immutable history は不可触) + MF-ε-2 PR #158 / HEAD `b5a6c7e` literal 填充 (= ε-1 § ε row + ε-8 commit chain table ε row 両方)、 main agent autonomous で軽微 fix-up commit (= memory `feedback_main_agent_engineering_responsibility.md` literal「doc wording = Claude Code 自分で直す」 整合 = ADR-0067/0068/0069/0071/0072 軽微 wording 整合 precedent 同形)、 修正範囲 = (1) 状態行 (= line 3) ε-1 § (= line 1679) 「修正 routine 4 件」 → 「修正 routine 5 件」 統一 + (2) ε-1 § ε row (= line 1680) 「PR #NNN 起票予定」 → 「PR #158 起票 HEAD `b5a6c7e`」 literal 填充 + (3) ε-8 § ε commit chain row (= line 1766) 「(= 後続 PR 起票時に embed)」 → 「`b5a6c7e` (= 本 PR #158 起票 commit、 Codex Rescue review agentId `a71c53fc52481e20c` revise → 軽微 fix-up → round 2 retry approve 予定)」 literal 填充、 round 2 retry 投入予定 (= 同 AXIS-EPS-1〜8 carry confirm)。
- 2026-05-27: ADR-0073 sprint ε Accepted milestone 起票 (= 本 PR4 doc-only sprint) = ADR doc 状態 Draft → Accepted + Annex ε fill 9 sub-section literal (= ε-0 user mandate + ε-1 sub-sprint chain α/β/γ/ε 完走 record + ε-2 γ runtime functional verify ALL PASS 正式記録 + ε-3 ADR-0072 ε -76 dBFS quiet 根本治療達成記録 + ε-4 「PMDNEO driver FM volume scaling semantics repair 完了」 wording 解禁 + 併記必須 7 件 mandatory carry + ε-5 修正された問題と修正の literal + ε-6 本 ε Accepted で次に進められること + ε-7 禁止 mandate user audition / ADR-0065 δ session 自動開始 / production-ready 肯定 carry + ε-8 ADR-0073 全体 commit chain literal + ε-9 Codex Rescue review 投入 mandate) + 平易要約 sprint ε context section 追加 (= 6 構造)、 起票者: 越川将人 (= 主軸 Claude Code 経由)、 user 明示 mandate「ADR-0073 sprint ε Accepted milestone 起票 = PMDDOTNET MML を再生したときに音量が極端に小さくなる問題について driver 側の FM volume scaling 修理と runtime verify が完了したことを正式記録するための doc-only sprint = ADR-0073 では comv / fm_volume_hook / FM default volume の扱いを修正し、 ADR-0072 時点で -76 dBFS だった音量が -29.59 dBFS まで改善 = sprint ε 完了で『PMDNEO driver FM volume scaling semantics repair 完了』 wording 使用可能、 ただし engineering metric pass であり越川氏 audition approve や production-ready 達成ではない」 mandate 経路。 ε scope literal = (1) Draft → Accepted + (2) Annex ε fill + (3) γ runtime functional verify ALL PASS 正式記録 + (4) ADR-0072 ε -76 dBFS quiet 根本治療達成 記録 + (5) wording 解禁 + (6) 併記必須 7 件 明記 = (i) metric pass であり aesthetic approve ではない + (ii) (d) audition gate 達成ではない + (iii) roadmap ⑥ audition 完了ではない + (iv) production-ready 全体達成ではない + (v) 軸 B 完成ではない + (vi) 軸 G 完成ではない + (vii) 本番 cmd 切替完了ではない + (7) dashboard update + (8) Codex Rescue review + (9) main agent merge + branch 削除 atomic + (10) memory update、 禁止 mandate literal = user audition 進めない + ADR-0065 δ session 自動開始しない + production-ready / 軸 B 完成 / 本番 cmd 切替 肯定しない、 後続 = Codex Rescue review 投入予定 + approve loop + main agent 経路 merge + atomic 1 セット規律 16 回目適用予定 (= PR #142+...+#157+本 ε PR)。
- 2026-05-27: ADR-0073 sprint γ runtime functional verify ALL PASS (= 集約 branch `wip-pmddotnet-opnb-extension@5c08c73` 経路、 main agent autonomous 実行 = user 明示 GO 受領「γ runtime functional verify に進む + engineering verify = main agent autonomous + user audition / ε Accepted 進めない + metric pass と aesthetic approve 混同しない + audio が出ても user audition 進めない」 mandate)、 4 build matrix B1-B4 完走 = (B1) production baseline `b15883fe...` + (B2) post-patch flag-off (B1) byte-identical + (B3) flag-on pre-patch (= ADR-0072 ε state `78a1087`) `ab47039f...` + (B4) flag-on post-patch `6a304e2a...` + (B3) vs (B4) byte diff = 4144 byte (= 中間 insert reference relocation 由来 normal、 .lst overflow なし confirmed、 0x0610 セクション overflow risk = memory `feedback_org_section_overflow_silent_bug.md` 該当なし)、 .lst predicate 6 件 ALL PASS (= `fm_carrier_table` @ 0x10D4 + `fm_volume_hook_pmddotnet` @ 0x10DC + SRAM equ 7 件 (0xFD62-0xFD6D) + 既存 routine body `.else` byte-identical + `pmdneo_fm_voice_set_default` `.else` byte-identical + 既存 symbol 9 件存在)、 δ-1〜δ-7 ALL PASS = (δ-1 OP4 carrier TL=0x00/0x02 max output 近傍 PMD V4.8s `volset_slot` 数式整合 + δ-2 modulator TL voice data exact preserve OP1=0x11 + OP2=0x19 + OP3=0x26 fix B carrier mask 経路動作 + δ-3 ADR-0072 voice opcode dispatch regression-free + δ-4 flag-off byte-identical mandate 達成 production-path regression-free 保証 + **δ-5 PRIMARY SUCCESS = analyze-audition-wav.py Layer 1 wav RMS = -29.59 dBFS > -60 dBFS threshold PASS = ADR-0072 ε -76 dBFS quiet 根本治療達成 +46.41 dB 改善** + δ-6 fade factor 経路継承 flag-off byte-identical 間接 confirm + δ-7 v+/v- emit count = 0 LR-2 scope-out OK)、 MAME runtime trace 経路 = `bash scripts/run-mame.sh --headless --trace --wavwrite --wavwrite-seconds 5` で wav (= 960048 byte = 5 秒 48kHz × 2ch × 16bit) + ymfm-trace (= 2887 行) + z80-mem-trace 取得、 fixture LF → CRLF 変換 main agent direct fix-up (= PMDDotNET parser CRLF 想定発見 = `file` command + xxd 比較で ADR-0072 fixture との encoding 差発見)、 metric pass vs aesthetic approve 規律遵守 (= memory `feedback_metric_pass_is_not_aesthetic_pass.md`)、 表記制約 = 「ADR-0073 γ driver patch implementation 完了」 + 「ADR-0073 γ runtime functional verify ALL PASS」 + 「ADR-0072 ε -76 dBFS quiet 根本治療達成」 使用可、 「PMDNEO driver FM volume scaling semantics repair 完了」 + 「(d) audition gate 達成」 + 「roadmap ⑥ audition 完了」 + 「production-ready 全体達成」 + 「軸 B 完成」 + 「本番 cmd 切替完了」 = δ/ε Accepted 後まで保留 / 各 user 明示 GO 必須、 後続 = sprint ε Accepted milestone = user 明示 GO 必須。
- 2026-05-27: ADR-0073 sprint γ PR3 round 1 Codex Rescue impl-review (= agentId `a84e13b690892737e`、 elapsed 約 5m 29s) **revise** + must-fix 1 + nh 2 + lr 2 + 越権操作なし confirmed、 per-axis verdict round 1 = AXIS-IMPL-1 PASS (= baseline 訂正根拠 5 件 + Annex γ-1 record + ADR-0071/0072 immutable carry 守られている) + **AXIS-IMPL-2 FAIL** (= must-fix 1 = `fm_volume_hook_pmddotnet` 内 `pmdneo_fade_scale` clobbers AF/BC/DE/HL (= line 2269 comment + 実装 L2271-2293 C/DE/HL 上書き)、 C (= carrier mask) + DE (= voice ptr) を `call pmdneo_fade_scale` 直後の per-slot loop で使用 = undefined behavior、 push/pop で preserve mandate) + AXIS-IMPL-3/4/5/6/7/8/9/10 PASS、 nh 2 件 = NH-1 verify-volume-scaling.sh の SCOPE-OUT wording 明示 + NH-2 ADR doc 内 `457a237c...` plan-history reference 多数 = summary 付近 reader-facing note、 lr 2 件 = LR-1 `comvshift_up/down` (= line 4074-4100) は fix A 後も PART_OFF_VOLUME 操作 (= runtime behavior δ MAME verify 待ち) + LR-2 B3/B4 + δ MAME runtime SCOPE-OUT (= audible success 未証明)、 main agent autonomous で fix-up commit (= memory `feedback_main_agent_engineering_responsibility.md` literal「engineering gate 不足 / verify bug = Claude Code 自分で直す」 整合 = critical register clobber bug は impl scope 内 main agent direct fix-up 対象) = `fm_volume_hook_pmddotnet` 内 `call pmdneo_fade_scale` 前後で `push bc` + `push de` + `pop de` + `pop bc` で C + DE preserve、 fix-up 後 γ-1〜γ-3 build verify 再実行 ALL PASS (= flag-off byte-identical `b15883fe...` 維持 confirmed + .lst predicate 4/5/6 PASS)、 round 2 投入予定 (= must-fix 1 反映 confirm + 同 AXIS carry)。
- 2026-05-27: ADR-0073 sprint γ PR3 起票 (= driver patch implementation = fix A+B+C 統合実装 + 4 build matrix B1-B4 verify + 新規 fixture 3 件 + verify script + ADR doc Annex γ-1 fill + baseline 訂正記録) = Draft、 起票者: 越川将人 (= 主軸 Claude Code 経由)、 user 明示 GO 受領 + base anchor `wip-pmddotnet-opnb-extension@52bfcb3`。 driver patch 実装範囲 (= plan v3 β-3-3/β-3-4/β-3-5 literal carry) = 修正 routine 4 件 (= `pmdneo_fm_voice_set_default` cache 初期化 ~30 byte + `pmdneo5_init_part` FM part 限定 default vol 108 ~8 byte + `comv` byte 直接 store ~25 byte 減 + `comat` voice ptr cache 保存 ~25 byte + `fm_volume_hook` dispatch ~3 byte) + 新規 routine `fm_volume_hook_pmddotnet` ~180 byte + 新規 data `fm_carrier_table` 8 byte + 新規 SRAM equ 7 件 (= 0xFD62-0xFD6D 12 byte 占有)、 patch byte 量 ~229 byte 増 net (= plan v3 ~166 byte 推定から +63 byte = `fm_volume_hook_pmddotnet` per-op unrolled loop の helper 4 件 (= `_fmvhp_write_s0/s1/s2/s3`) 含む実 byte 量)、 全 guarded change `.if PMDNEO_USE_PMDDOTNET ... .endif` 配下限定 (= flag-off byte-identical 維持)。 **baseline 訂正 (= user 明示 mandate option (a) 採用)**: γ 着手時 B1 production baseline build verify で actual = `b15883fe59804a201e13d0c05f083c1c3dd31fbfb1efd193b34d550d18f561e4` 確定 (= ADR-0048〜0070 era historical baseline と同じ)、 ADR-0073 doc 内 plan v1/v2/v3 + §決定 3 carry の期待値 `457a237c...` と mismatch 検出。 main agent 直接切り分け = patch revert 前後で同 sha256 confirmed = ADR-0073 patch 起因ではない (= guarded change `.if PMDNEO_USE_PMDDOTNET` flag-off byte-identical mandate 守られている)、 user 明示 mandate「actual baseline への doc 訂正」 option (a) 採用 = canonical active baseline = `b15883fe...` 訂正 (= dashboard production baseline section + ADR-0073 §決定 3 supersede pointer 追加 + Annex γ-1 § baseline 訂正記録)、 `457a237c...` は ADR-0071 ε era の transient record (= immutable history mandate carry)。 4 build matrix B1-B4 期待 sha256 = (B1)/(B2) `b15883fe...` byte-identical + (B3) ADR-0072 ε state + (B4) ADR-0073 patch byte 量のみ diff、 .lst predicate 6 件 (= flag-on `fm_carrier_table` + `fm_volume_hook_pmddotnet` + SRAM equ 7 件 assemble + 既存 routine body `.else` 配下 byte-identical + `pmdneo_fm_voice_set_default` `.else` 配下 byte-identical + 既存 symbol table 順序不変)、 新規 fixture 3 件 (= `test-volume-baseline.mml` voice 001 + v15 + 6 FM ch + `test-v-shift-coverage.mml` v+/v- coverage LR-2 mitigation + `expected-events.json` δ-5 Layer 1 wav RMS > -60 dBFS threshold)、 新規 verify script `verify-volume-scaling.sh` (= fail-fast `set -euo pipefail` + γ-1 B1 active baseline check + γ-2 B2 flag-off byte-identical + γ-3 .lst predicate 6 件)、 Codex Rescue impl-review 投入予定 (= user mandate「baseline 訂正の根拠を必ず review 軸に入れる」 mandatory)、 後続 = main agent 経路 merge + atomic 1 セット規律 15 回目適用予定。
- 2026-05-27: ADR-0073 sprint β PR2 round 3 Codex Rescue plan review (= agentId `aeecb6ad50e314bd6`、 elapsed 約 3m 51s) **revise** + must-fix 2 + nh 0 + lr 0 + 越権操作なし confirmed = per-axis verdict round 3 = AXIS-R1 FAIL (= dashboard 0073 row stale `0xFD 0x7A` literal、 ADR 内 β-3-3 訂正済 = PASS だが dashboard carry 漏れ) + AXIS-R2/R3/R4/R5/R6/R7 PASS + AXIS-R8 FAIL (= #1〜#13 trigger executable command literal 不在、 ADR-0073 内に pointer or command table 必要) + AXIS-R9 PASS + AXIS-R10 FAIL (= MF-new-1 反映 dashboard side carry 漏れ = R1 と同体)、 must-fix 2 件 = MF-3-1 dashboard 0073 row stale byte literal 修正 (= `0xFD 0x7A` → `0xFD 0x7D`、 sprint α PR2 doc 起票時の inadvertent typo carry) + MF-3-2 rollback condition #1〜#13 trigger pointer 補強 (= ADR-0073 内 plan v3 β-3-8 §に #1〜#13 trigger source + 実行可能 command table 追加、 ADR-0071/0072/0069 from inheritance literal pointer)、 main agent autonomous で軽微 fix-up commit (= 連鎖 commit fix-up = ADR-0072 plan v7 軽量 wording 整合 precedent 同形、 plan v4 起草不要 = plan v3 内容妥当性 confirm 済 + wording stale 修正のみ)、 round 4 投入予定 (= 同 AXIS-R1〜R10 carry confirm)。
...
### 平易要約 sprint ε context section 追加 (= 2026-05-27 43rd session、 sprint ε Accepted milestone)

#### やりたいこと

ADR-0073 全体 (= driver FM volume scaling semantics repair) の **正式完了記録** を残す。 ADR-0072 ε で audio 音量が -76 dBFS quiet (= audition material 不成立) だった問題を、 ADR-0073 で driver `comv` / `fm_volume_hook` / FM default volume の扱いを修正して、 -29.59 dBFS まで改善 (= +46.41 dB)。 これで「PMDNEO driver FM volume scaling semantics repair 完了」 と書けるようになる。

#### 前提

- ADR-0073 sprint α/β/γ 完了 + γ runtime functional verify ALL PASS 済 (= 集約 commit `a41c755`)
- δ-5 primary success metric = wav RMS -29.59 dBFS > -60 dBFS threshold PASS confirmed
- 4 build matrix B1-B4 ALL PASS + .lst predicate 6 件 ALL PASS + δ-1〜δ-7 ALL PASS
- production sha256 `b15883fe...` byte-identical 維持 confirmed (= guarded change `.if PMDNEO_USE_PMDDOTNET` flag-off mandate 達成)
- ADR-0072 voice opcode dispatch + voice_table 1-based offset + #FFFile support 既存機能 regression-free
- user 明示 mandate = ε scope = doc-only sprint = Draft → Accepted + Annex ε fill + wording 解禁 + 併記必須 7 件 + 禁止 mandate (= user audition 進めない / ADR-0065 δ session 自動開始しない / production-ready 等 肯定しない)

#### やったこと

1. ADR doc 状態 = Draft → **Accepted** (= ε milestone)
2. Annex ε literal fill 9 sub-section (= ε-0 user mandate + ε-1 全 sub-sprint chain α/β/γ/ε 完走 record + ε-2 γ runtime verify ALL PASS 正式記録 + ε-3 ADR-0072 -76 dBFS 根本治療達成 + ε-4 wording 解禁 + 併記必須 7 件 + ε-5 修正された問題と修正 literal + ε-6 次に進められること + ε-7 禁止 mandate + ε-8 commit chain literal + ε-9 Codex Rescue review mandate)
3. 改訂履歴 sprint ε entry append (= chronological order、 append only mandate 厳守)
4. 平易要約 sprint ε context section 追加 (= 6 構造)
5. dashboard 0073 行 status final update (= ε Accepted milestone 反映)
6. Codex Rescue review 投入予定 (= ε scope 整合確認 + wording 解禁 + 併記必須 7 件 mandate 確認 + 越権操作なし)

#### 結果

- **ADR-0073 sprint ε Accepted milestone 起票** = ADR-0073 全体完走 record
- 「**PMDNEO driver FM volume scaling semantics repair 完了**」 wording 使用可能 (= 併記必須 7 件 mandatory carry)
- ADR-0072 ε -76 dBFS quiet **根本治療達成** literal record (= +46.41 dB 改善 evidence)
- 残課題明示 = ADR-0065 ε δ session (= 越川氏 audition、 user 介入 mandatory) + ADR-0066/0070 候補 (= 各 user 明示 GO 必須)

#### 解釈

- 本 ε milestone は **engineering metric pass の正式記録** = wav RMS Layer 1 threshold pass = audition material 不成立問題の **engineering 側解決完了**
- aesthetic approve (= 越川氏 audition session) は **別軸 user 介入 mandatory** (= memory `feedback_metric_pass_is_not_aesthetic_pass.md` 規律 = 41st session δ session 試行 invalid 経路 origin)
- ADR-0072 「driver no-touch mandate」 を覆す pivot 判断 (= driver semantics divergence 範疇 = guarded change 経路必須) が β plan review approve + user 明示 GO 経由限定で慎重に進行した実証
- 「PMDNEO driver FM volume scaling semantics repair 完了」 wording 解禁 = ADR-0072 ε で permanent 禁止された wording の **本 ε で初めて解禁**、 ただし 7 件併記 mandatory carry

#### 次

- 本 ε PR = doc-only 起票 → Codex Rescue review → approve loop → main agent 経路 merge (= atomic 1 セット規律 16 回目適用予定 = PR #142+...+#157+本 ε PR)
- ADR-0073 全 sub-sprint chain 完走 + Accepted state 確定 (= ADR-0073 全体 future close)
- **ADR-0065 ε δ session 起票判断** = ADR-0073 ε Accepted 後 dependency 解除 ready、 ただし **user 介入 mandatory** (= 越川氏 audition session 実施、 main agent 自動開始しない)
- **ADR-0066 候補** (= 本番 cmd 切替 = `PMDNEO_USE_PMDDOTNET` flag default 切替判断) = ADR-0065 ε Accepted 後 future、 user 明示 GO 必須
- **ADR-0070 候補** (= K bitmap pair distinct) = ADR-0069 γ Accepted で dependency 解除済、 user 明示 GO 必須
- **軸 B 残 sprint chain** = 各 user 明示 GO 必須

#### 重要 insight (= ADR-0073 全体完走 retrospective)

ADR-0073 全体経緯 (= sprint α/β/γ/ε) で実証された規律:
1. **driver no-touch mandate は問題範疇による適用判断**: build wiring/tooling 範疇 = no-touch、 driver semantics divergence 範疇 = guarded change 経路必須 (= ADR-0072 ε insight の精緻化 supersede)
2. **baseline 訂正 = user judgment scope**: 「actual baseline への doc 訂正」 wording 整合厳守、 「production baseline 変更」 誤記禁止 (= γ-1-1 § literal)
3. **plan v1 → v2 → v3 iteration immutable history mandate**: Annex β-1/β-2 は immutable carry + 次 plan version supersede pointer のみ + 誤記訂正は supersede note 経由 indirect 訂正 (= ADR-0072 plan v3 precedent 同形)
4. **impl-review critical bug fix-up = main agent direct**: register clobber 等の verify bug は main agent 直接 fix 対象 (= memory `feedback_main_agent_engineering_responsibility.md` 規律下)
5. **metric pass ≠ aesthetic approve**: engineering Layer 1 threshold pass は audition gate 達成 ではない (= 併記必須 mandatory carry、 memory `feedback_metric_pass_is_not_aesthetic_pass.md` 規律 41st session δ session invalid 経路 origin)
6. **plan/impl review chain Codex Rescue + 機械復旧 rule**: placeholder response 6 回 + 4 round chain 完走、 機械復旧 rule cancel + 1 retry default 確立
7. **atomic 1 セット規律連続適用**: PR #142+#143+#144+#145+#146+#147+#148+#149+#151+#152+#153+#154+#155+#156+#157+本 ε PR = 16 回目連続適用 (= branch 運用 4 条規律 mature 化)
