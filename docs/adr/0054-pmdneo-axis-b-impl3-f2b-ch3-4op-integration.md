...
- 状態: **Draft** (= 2026-05-22 39th session 軸 B 実装 sprint 3 α、 ground truth = ADR-0045 Annex I-3 / §J-4-3、 ADR 起票 doc-only filing、 後続 β/γ/δ で driver 最小 integration → verify → completion。 ADR-0045 §J-4-3 literal 後続実装 ADR。 **軸 B 実装 sprint chain の 実装 3 = δ-3 = F-2-B ch3 4-op individual mode の入口 + register write proof**。 軸 B 全体は未完了、 「軸 B 完成」 表現不使用)
- 著作権者: 越川将人
- 関連 ADR:
...
**今回やること (= α)**: 設計書 (= 本 ADR-0054) を起票するだけ。 ドライバのコードはまだ書かない。 範囲・F-2-B の経路・検証方法・不可触対象を文書で固定する。

**次**: 本 ADR-0054 を doc-only で commit / PR / merge した後、 β でドライバに F-2-B の最小 proof path を追加し、 γ で検証スクリプトを整備、 δ で Draft → Accepted へ移行する。

## sub-sprint chain 進捗

| sub | 状態 | PR | Codex layer 2 review |
|---|---|---|---|
| α (= ADR-0054 起票) | **進行中** (= 39th session、 本 PR) | (= 本 PR) | 起票 plan review = escalate (= 論点 2 件 = F-2-B 経路 / hooks=noop scope、 user 判断 → 簡易実装 / defer 確定) + 起票 review |
| β (= minimal driver integration) | 未着手 | - | - |
