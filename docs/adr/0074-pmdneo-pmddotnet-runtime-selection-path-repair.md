...
```
[build invocation]
  build-poc.sh が PMDDOTNET_MML detect + flag=1 set
      ↓
  PMDNEO_SONG=2 を build invocation env var として set (= preflight build 専用)
      ↓
[Makefile / build.mk]
...
| **`scripts/build-poc.sh`** | PMDDOTNET_MML block 内に `TEST_MODE_PMDDOTNET_SONG_SELECT=1` 設定 + song_data.inc 動的拡張 + `PMDNEO_SONG=2 export` 追加 (= flag=1 時のみ) | 完全不変 (= PMDDOTNET_MML 未設定 or TEST_MODE_PMDDOTNET_SONG_SELECT 未設定で skip) |
