...
| **γ** | runtime selection path implementation = 確定 touch (= main.c + song_data.inc + driver いずれか or 複合) + 機械的証明 fixture 作成 + 4 build matrix verify | **user 明示 GO mandatory point** (= ADR-0058 §決定 1 衝突する可能性 = 設計判断軸、 ADR-0041 §決定 5 escalation 該当) → **2026-05-27 user 明示 GO 受領 + γ impl 完了** = candidate 4 実装 + 4 build matrix B1-B4 ALL PASS + γ-1〜γ-5 ALL PASS + .lst predicate 10 件 ALL PASS + production sha256 `b15883fe...` byte-identical 維持 + NH-1 design observation (= γ-3 § literal) | impl 完了 + 4 build matrix PASS + 「指定 fixture 由来 audio」 機械的証明 evidence 取得 | あり (= scope IN routine + build infra のみ、 guarded change or preflight build mode 配下) |
...
## Annex γ: sprint γ runtime selection path implementation 完了 (= 2026-05-27 user 明示 GO 経路)

### γ-1: 修正・新規 file literal (= plan v3 Annex β-7 § 6 mandate point literal 反映)
...
### γ-2: 4 build matrix verify result (= γ-1 build verify gate ALL PASS)

| build | flag combination | sha256 (= verify-runtime-selection.sh empirical 結果) |
|---|---|---|
| **(B1) production baseline** | `PMDNEO_USE_PMDDOTNET=0` + `TEST_MODE_PMDDOTNET_SONG_SELECT=0` + no PMDDOTNET_MML | `b15883fe59804a201e13d0c05f083c1c3dd31fbfb1efd193b34d550d18f561e4` = ACTIVE_BASELINE byte-identical ✓ |
...
| **(B3) PMDDOTNET on + SONG_SELECT off** | `PMDNEO_USE_PMDDOTNET=1` + `TEST_MODE_PMDDOTNET_SONG_SELECT=0` + PMDDOTNET_MML=preflight-staggered.mml | `99e5644d9b47027d0f0a36e11c50797a9482c5b933a9d7f15d666df23f821ec2` = ADR-0073 ε state baseline (= K + L-Q only override carry) |
| **(B4) PMDDOTNET on + SONG_SELECT on** | `PMDNEO_USE_PMDDOTNET=1` + `TEST_MODE_PMDDOTNET_SONG_SELECT=1` + PMDNEO_SONG=2 + PMDDOTNET_MML=preflight-staggered.mml | `ff9a5013474f7b58685d5d57c484c956d4fc7010583a2ee3ef126ec67eb6ffbc` = song_table[40..59] song2_part_a〜z entries 挿入 active + main.c PMDNEO_SONG=2 経路 |

### γ-3: NH-1 design observation (= main agent γ impl 中に発見、 user judgment 軸該当)
...
#### user judgment 軸該当部 (= 自走しない mandate carry)
...
→ **main agent autonomous で進める範囲は γ build verify ALL PASS 確認 + 本 finding literal record まで**、 (a)/(b)/(c) いずれも user 明示 GO mandatory point。
...
### γ-5: Codex Rescue impl-review chain placeholder

sprint γ impl-review 投入予定 = main agent commit + Codex Rescue impl-review (= 5 必須軸 AXIS-RT-1〜5 carry + AXIS-FIX-1〜N new axes for impl verify + AXIS-NH-1 design observation evaluation)、 must-fix 0 + 全 PASS で approve、 round 1 approve target (= sprint γ impl review 経験則 20 分 threshold 内)。
