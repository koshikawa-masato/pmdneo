...
6. 実装可能な場合 `tools/fxp2wav-surge/` として PoC (= shell wrapper or Python wrapper + native binary、 license 整合確認 = Surge XT license は GPL-3.0、 PMDNEO GPL-3.0 と整合)
7. spike 結果を ADR-0033 §Annex A-8 (= 新規後埋め枠) に literal 反映

**制約 (= 23rd session ι' 末 user 明示、 spike track 1 規律):**
...
**branch 名候補 (= user 確認軸、 別 session 着手時に確定):**
- `spike-fxp2wav-surge` (= 推奨、 spike 性格明示)
- `wip-fxp2wav-surge` (= 既存 wip- prefix convention 整合、 `wip-pmddotnet-opnb-extension` と並列)
- `research-fxp2wav-surge` (= research 性格明示)

**spike 結果反映先 (= main branch への戻し方):**
- **spike 成立** (= CLI 動作確認 + PoC 動作) → ADR-0033 §Annex A-8 後埋め + `tools/fxp2wav-surge/` 本線統合 commit (= main branch へ merge / cherry-pick)
...
**spike track 1 と β-main BD chain の関係:**
- β-main = BD hand-on artifact (= `.fxp` + audio capture or GUI bounce 経由 .wav) 受領 → encode → verify → ι commit (= 並行配置、 既存 fixture 非破壊)
- spike track 1 = fxp2wav-surge CLI 構築 spike (= dev investigation)、 BD-main の `.wav` 生成経路を将来 audio capture から CLI render に置換可能化
- 両 track 独立進行、 spike 成立後の BD-main rerun (= 同 .fxp で再生成 WAV byte-identical) で reproducibility 統合検証
