...
### §決定 27: AI-assisted patch generation + factory CC0 template acquisition + `.fxp` bridge + rendered-audio self-analysis workflow = AI 設計補助 + AI/toolchain template acquisition + AI/toolchain bridge + AI engineering 検査 + 越川氏 aesthetic final acceptance 軸独立 (= source-of-truth = `patch-spec.yaml`、 factory CC0 template acquisition + XML element name allowlist patching = scope-in、 arbitrary `.fxp` binary full generation + full binary RE = scope-out、 23rd session λ 起票 / μ 初回適用 / ν 全面再定義 / ξ AI/toolchain bridge 軸統合 / π AI/toolchain template acquisition 軸統合 + 越川氏 hand-on engineering 完全ゼロ化)
...
- AI 役割 5 = **rendered audio self-analysis** (= ν 追加、 engineering / signal analysis 軸)
...
- **patch-spec value の Surge XT 内部 scale 不変換 passthrough** (= π7 越川氏 2026-05-17 audition で実証された engineering FAIL root cause、 human-readable values (= ms / Hz / dB / enum 等) を Surge XT 内部 scale (= log-time / log-freq / normalized / integer enum 等) に変換せず直接 XML element value 注入する pattern は **禁止**、 unit conversion layer 経由 必須、 例 `decay_ms 280` → `log2(0.280) = -1.84` 変換後 Surge XT `a_env1_decay` element に注入、 Kick 909ish.fxp reference (= `SURGE_XT_FXP_BRIDGE_INVESTIGATION.md § 15.4`) 観測値が conversion table の literal 根拠、 「human-readable values must be converted to Surge XT internal scale」 wording literal、 unit conversion layer 実装は π8 commit 以降の bridge layer 拡張で fill 予定)

§決定 1 (= 100% 著作物のみ採用) / §決定 14 (= sound-alike caution) / §決定 16 (= prototype 粒度 + wording 規律) / §決定 19 (= 段階 1 集中) / §決定 24 (= aesthetic) / §決定 25 (= fxp2wav-surge external producer) / §決定 26 (= `.fxp` authoring spec) と整合、 §決定 27 は **AI-assisted patch generation + factory CC0 template acquisition + `.fxp` bridge + rendered-audio self-analysis workflow 軸** として独立追加 + ν 全面再定義 + ξ AI/toolchain bridge 軸統合 + π factory CC0 template acquisition + XML element name allowlist patching 軸統合 (= λ 8 軸 → ν 10 軸 → ξ/π 11 軸、 AI 役割 1 軸 → ν 2 軸 → ξ 4 軸 → π 5 軸、 4 段 → 5 段 chain、 user acceptance upstream → downstream、 越川氏 hand-on engineering 軸完全ゼロ化 / aesthetic final gate のみに純化、 factory CC0 template + XML element name allowlist patching scope-in、 「越川氏 認知負荷を engineering correctness から完全解放」 軸の完全貫徹)、 既存 §決定 1-26 と直交、 driver / fixture / verify script / runtime semantics 軸 ADR-0026-0032 完全不変。
