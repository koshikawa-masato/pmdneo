...
**BD chain 完了判定 (= sub-sprint β BD 1 commit 完了条件):**
- BD `.fxp` patch literal 保存 (= canonical source、 越川氏 100% 著作物 = §決定 1 整合)
- BD wav render 完了 (= §決定 8 整合、 越川氏 audition gate ✓)
- BD adpcma encode 完了 (= sha256 manifest 記録、 既存 `2608_BD.adpcma` (= unknown 起源 temporary fixture) との sha256 differ literal 確認 = 越川氏自前 sample の独立性証明)
- **BD chain workflow regulation literal 化** (= patch naming / 配置 / wav render parameter / encode option / verify gate を ADR-0033 § sub-sprint β workflow regulation section に literal 反映 = 残 5 音横展開で踏襲する規約)
- commit + push 完了 (= scope = `.fxp` + `.wav` + `.adpcma` + ADR-0033 workflow regulation 追記)
