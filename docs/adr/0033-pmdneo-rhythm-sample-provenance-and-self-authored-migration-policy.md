...
- 状態: **Draft** (= 2026-05-15 20th session 中盤起票 + 2026-05-15 21st session 冒頭 sub-sprint α 5 軸壁打ち決定 + §決定 15-19 追加 + §Annex A-5 / A-7 後埋め枠の調査軸 literal 化 + migration roadmap 段階 1 sub-sprint α 内容詳細化 + 2026-05-16 22nd session 冒頭 sub-sprint α 5 軸壁打ち決定 (= forensic 実施順序 / scripts canonical pipeline 粒度 / Surge XT install 環境 / prototype render naming / synthetic drum identity aesthetic) + §決定 20-24 追加 + §Annex A-5 inventory-first / sha256 canonical / observed-facts-first wording 補強 + §Annex A-7 4 系統 scripts 化 (= forensic-drum-samples.sh + wav_to_adpcma.sh + build_drum_samples.sh + verify-drum-samples.sh) + Makefile target 4 件 補足 + ## 重要 wording 規律 22nd session 追加 wording 反映 (= synth kit first-class identity / OPNA-era digital drum identity / ADPCM-A friendly drum design / Homebrew cask canonical / filename = drum identity / directory = kit identity 等) + 2026-05-16 23rd session 段階 1 sub-sprint α 実作業着手 = `scripts/forensic-drum-samples.sh` 新規作成 (= §決定 21 4 系統 1 件目 read-only inventory) + §決定 20 8 段階 flow mechanical 実行 + §Annex A-5 後埋め (= 12 file inventory 表 sha256 / size / meta / magic + 4 分類 all unknown + 重要 finding 5 件 = 18500 Hz roundtrip wav / source wav 不在 / 6 adpcma 別 sha256 / driver embed symbol / drum 軸 vendor wav 不在 literal 反映) + 23rd session 第 2 commit = `scripts/wav_to_adpcma.sh` + `scripts/build_drum_samples.sh` + `scripts/verify-drum-samples.sh` 3 件 interface fixation stub 新規作成 (= §決定 21 4 系統 2-4 件目、 9 項目 header / usage / not-implemented exit 2 sentinel / source wav 完成前は middle 実装しない literal 規律) + §決定 21 scripts 構成 4 件 status marker 反映 + sub-sprint α 進捗 23rd session 反映 section 追加 + 23rd session 第 3 commit ζ = top-level `Makefile` 新規作成 (= drum-sources / drum-build / drum-verify / drum-clean 4 target 軽量 stub、 「Makefile = 作業者向け入口、 script 名の写像ではない」 軸転換 literal、 wav_to_adpcma.sh は Makefile 直下に露出させない = drum-build 内部 invoke 扱い、 drum-clean は 23rd session ζ 新規 = rebuild path 確立後に有効化) + §決定 21 Makefile target 命名 update (= 旧 4 件 forensic-drum-samples / wav-to-adpcma / build-drum-samples / verify-drum-samples → 新 4 件 drum-sources / drum-build / drum-verify / drum-clean) + reasoning 「Makefile = 作業者向け入口」 literal 追加 + migration roadmap sub-sprint α + §Annex A-5 reference 同 target 命名 update + 23rd session 第 4 commit ζ' = §Annex A-7 superctr/adpcm evidence collection literal 反映 (= encoder repo `e431c94` HEAD / Public Domain Unlicense / build 成功 Apple clang 17.0.0 arm64 / ADPCM-A `ae` command 存在 / `-a` anti-overflow / signed 16-bit PCM little-endian input / stdlib 依存 / 2025-12-15 ValleyBell yma-overflow-fix maintenance / 7 ADPCM variant 対応 8 finding + §決定 18 採用条件 4 件クリア = encoder 採用 finalize ✓、 Surge XT install version 列は η commit で後追い後埋め) + 23rd session 第 5 commit η = §Annex A-7 Surge XT install version 列 後埋め完了 (= surge-xt 1.3.4 / install 2026-05-16 13:01:59 JST / path /opt/homebrew/Caskroom/surge-xt/1.3.4 / 419.8 MB / Apple Silicon arm64 / official upstream + cask source URL literal + brew lookup quirk reference 追加 = `brew list --versions <name>` formula default の cask 不在 quirk + `--cask` flag 明示で取得可能 future contributor 向け notation)、 23rd session 5 段階構成 (δ/ε/ζ/ζ'/η) で sub-sprint α 完了 milestone 達成 → sub-sprint β = BD 1 音先行 provenance chain 縦通し着手可能 + 23rd session 第 6 commit θ = sub-sprint β scaffolding 全面置換 (= 旧「6 種一括 patch 設計」 → 新「BD 1 音先行 chain → 残 5 音横展開」、 depth-first scaling 流儀 ADR レベル固定、 reproducible workflow first / aesthetic refinement second 軸、 BD chain 6 step literal 化 = patch / .fxp / wav / audition / adpcma / verify + 残 5 音 commit 粒度 3 候補 + 各 drum starting hint 維持 doc only commit、 sub-sprint β BD hand-on 着手準備完了 milestone) + 23rd session 第 7 commit θ' = sub-sprint β BD chain 着手前確定 parameters 8 軸 literal 固定 (= .fxp path / WAV no-normalize parameter / trim 規約 leading 0 + trailing +50-100 ms / WAV path / direct invoke encode 経路 -a anti-overflow ON / ADPCM-A 並行配置 path / 既存 fixture 隔離維持 / WAV → raw PCM 経路、 重要 wording 規律 = no normalize + 並行配置 + ι commit scope canonical の 3 件 literal、 ι commit (= 越川氏 hand-on 完成時) 安全 boundary 確立 milestone) + 23rd session 第 8 commit ι' = §決定 25 追加 = Surge XT fxp2wav CLI 化を future から self-authored source-of-truth pipeline 必須調査として scope-in 昇格 (= spike track 1 並走、 別 branch / 別 commit 系列 / 別 session 着手、 7 step work + 3 制約 + branch 名候補 3 件 + 成立/不成立時の反映先 literal、 β-main BD chain は止めない並走規律、 23rd session ζ' §Annex A-7「公式 distribution CLI render 不在」 literal 観察を踏まえた軸転換、 spike 性格 = .fxp source-of-truth ⇄ WAV 再生成可能経路の必須化、 audio capture 経路は段階 3 完全ホワイト化 milestone に持ち込めない reproducibility 規律根拠) + 23rd session 第 9 commit ι'' = §決定 25 repo boundary 規律補強 = 「scope-in だが repo-in ではない」 中核 wording literal 追加 (= fxp2wav-surge = PMDNEO 外部 spike + required external producer 扱い、 superctr/adpcm と同 pattern、 PMDNEO source tree に Surge fork / JUCE / build artifacts を入れない 4 件目制約追加、 役割境界 literal = PMDNEO consumer / fxp2wav-surge producer、 branch 名候補 → external repo location 候補に置換 = github.com/koshikawa-masato/fxp2wav-surge 推奨、 spike 成立時の tools/ 取り込み禁止 literal、 PMDNEO license narrative 独立性確保 repo hygiene 規律) + 23rd session 第 10 commit κ = §決定 26 追加 = Surge XT `.fxp` authoring spec literal 固定 (= 7 軸 = patch naming / synthesis goal / note convention / output constraint / patch design constraint / metadata-provenance / acceptance gate + 非目標 4 件 = YM2608 BD 模倣 / 完成音追い込み / 6 音同時設計 / GUI 依存暗黙手順 + spec 適用順序 + spec 範囲外、 BD hand-on 着手前の `.fxp` source-of-truth 仕様規律確立、 patch naming = `2608_bd.fxp` / Surge XT display `PMDNEO 2608 BD Self v0`、 note = MIDI 36 / vel 127 / 800 ms、 output = mono / no normalize / -6〜-3 dBFS peak / clipping 禁止、 design = Surge XT 内蔵 only / external sample 禁止 / random+drift+unseeded modulation 禁止 or seed 固定 / long tail 回避 / 低域+attack 明確化、 provenance = version + author + date + render command + `SURGE_RNG_SEED` + sha256 `.fxp` / `.wav` / `.adpcma`、 acceptance gate 5 件 = fxp2wav-surge render 可 + bit-identical WAV + 44100 mono 16-bit + BD 聴感成立 + 既存 fixture 別物並行配置、 残 5 音横展開時の canonical template として BD spec 再利用可能化、 §決定 14 sound-alike caution / §決定 16 prototype 粒度 / §決定 24 aesthetic / §決定 25 fxp2wav-surge external producer 整合、 `.fxp` = 単なる hand-on 成果物 → 再現可能 source-of-truth へ昇格、 BD hand-on artifact 受領前の安全 boundary 確立 + 残 5 音 template 確立 milestone、 driver / fixture / verify script / runtime semantics 軸 ADR-0026-0032 完全不変) + 23rd session 第 11 commit λ = §決定 27 追加 = AI-assisted patch generation workflow literal 固定 (= 8 軸 = AI 役割境界 / source-of-truth 階層 / patch spec 形式 / patch spec 最低必須 12 項目 / provenance chain 4 段階 / acceptance workflow / 越川氏 100% 著作整合 narrative / 成果物 path canonical 5 件 + spec 適用順序 8 step + scope-out 7 件、 AI 設計補助 + 越川氏 acceptance 軸独立確立、 AI = candidate 提示 / 越川氏 = audition + edit + accept 権限保持 / 最終 asset = 越川氏 100% 著作物方針不変、 source-of-truth = `patch-spec.yaml` / Markdown table (= `.fxp` binary より上位の human-readable layer)、 `.fxp` binary 直生成 / reverse engineering 永久 scope-out、 provenance chain = `patch-spec.yaml` → `.fxp` → `.wav` → `.adpcma` 4 段階、 §決定 26 (6) の 3 段階 sha256 を 4 段階に格上げ、 §決定 1 100% 著作物 / §決定 14 sound-alike caution / §決定 16 prototype 粒度 / §決定 19 段階 1 集中 / §決定 24 aesthetic / §決定 25 fxp2wav-surge external producer / §決定 26 `.fxp` authoring spec 整合、 越川氏 100% 著作と AI 利用の境界を ADR レベル literal で確立 milestone、 RX-11 clone / mimic / ROM recreation 方向 AI candidate は越川氏 reject 必須 wording 連動、 新規 directory `docs/design/rhythm-patches/synth/` patch spec source-of-truth canonical path 確立、 driver / fixture / verify script / runtime semantics 軸 ADR-0026-0032 完全不変) + 23rd session 第 12 commit μ = §決定 27 (6) acceptance workflow step 1 初回適用 = `docs/design/rhythm-patches/synth/2608_bd.patch-spec.yaml` AI candidate 生成 (= 303 行 / 22 top-level keys、 path B AI-assisted workflow 初回実 file 化、 BD = 残 5 音 template 候補確立、 §決定 26 7 軸 + §決定 27 12 必須項目 + §決定 24 aesthetic 5 target 整合、 OSC1 Sine pitched body + OSC2 S&H Noise click + LP24 800Hz + Soft Clip +3dB + AHDSR D=280ms + pitch sweep 130Hz→65Hz / 50ms、 provenance chain step 1 literal 記録 + step 2-4 pending state、 越川氏 audition / edit / accept 待ち、 driver / fixture / verify script / runtime semantics 完全不変) + 23rd session 第 13 commit ν = §決定 27 全面再定義 = AI-assisted patch generation + **rendered-audio self-analysis workflow** へ拡張 (= λ 8 軸 → ν 10 軸、 acceptance workflow 5 step → 11 step、 4 段 → 5 段 provenance chain、 user acceptance **upstream → downstream 再配置**、 engineering pass / aesthetic accept **軸独立分離**、 AI 役割 1 軸 (= spec 設計補助) → 2 軸 (= spec 設計補助 + rendered audio self-analysis 10 項目) 拡張、 AI self-analysis 10 項目 = waveform sanity / peak / RMS / clipping / silence / attack / decay / transient strength / spectral balance / tail length、 ν 中核 wording = 「acceptance は downstream、 upstream ではない」 / 「machine-checkable quality gate + human aesthetic final gate 2 layer」、 成果物 5 → 6 件に拡張 = `analysis-report.yaml` 新規追加 (= `docs/design/rhythm-patches/synth/2608_<drum>.analysis-report.yaml`)、 scope-out 7 → 9 件に拡張 = AI による aesthetic judgement / 越川氏 acceptance signature 代理生成 永久 scope-out 2 件追加、 23rd session ν 末 user finding = AI 機械検査を user audition 前に済ませる方が user の aesthetic judgement 軸 clean / 認知負荷低減、 §決定 1 100% 著作物 / §決定 14 sound-alike caution / §決定 24 aesthetic / §決定 26 spec 整合維持、 23rd session μ で生成済 `2608_bd.patch-spec.yaml` は ν workflow stage 軸に対応する形で同 commit 内で更新 (= `acceptance` → `workflow_stage: candidate-pending-render-analysis` + `engineering_pass: pending` + `aesthetic_acceptance: pending` 3 field 軸分離、 provenance chain 4 段 → 5 段拡張 = step 4 AI self-analysis 新規挿入 + step_4 → step_5 adpcma renumber、 acceptance_criteria 2 階層化 = engineering_pass_ai_self_analysis_10_items + aesthetic_acceptance_user_final_gate)、 driver / fixture / verify script / runtime semantics 軸 ADR-0026-0032 完全不変) + 23rd session 第 14 commit ξ = §決定 27 ν step 2 役割再配置 + `.fxp` bridge 軸統合 (= ν の本質 (= acceptance downstream / 越川氏 認知負荷を engineering correctness から解放) を貫徹するため、 ν step 2 の「越川氏 hand-on」 を「AI/toolchain による patch-spec → `.fxp` template-based bridge」 へ役割再配置、 越川氏 hand-on は template `.fxp` 作成 1 度のみに限定 = 全 6 drum 種共通 base、 drum-specific patch は AI/toolchain bridge 生成、 越川氏 review 対象は rendered audio のみ = aesthetic final gate 純化、 ξ scope-in 5 件 = patch-spec.yaml → template `.fxp` parameter patching bridge + template `.fxp` provenance/hash 記録 + patching 対象 parameter allowlist + generated `.fxp` は candidate + `.fxp` → wav render → AI self-analysis → user final gate、 ξ scope-out 拡張 4 件 = arbitrary `.fxp` binary full generation + full `.fxp` binary format reverse engineering + `.fxp` checksum/chunk format 無制限解析 (= 永久 scope-out、 必要最小限のみ許容) + AI が user acceptance を代替すること (= 永久 scope-out、 user 明示 wording literal)、 ξ 中核 wording 4 件 = template-based `.fxp` candidate generation / parameter allowlist patching / bridge not full synthesizer patch compiler / engineering candidate before aesthetic judgement、 ν 中核 wording (= acceptance downstream / machine-checkable + aesthetic final gate 2 layer) 維持、 AI 役割 2 軸 → 4 軸 拡張 (= spec 設計補助 + `.fxp` bridge + render orchestration + self-analysis)、 §決定 27 (9) 成果物 path 6 件 → 9 件 拡張 (= `assets/drum_samples/synth/templates/2608_template.fxp` + `docs/design/rhythm-patches/synth/parameter-allowlist.yaml` + `docs/design/rhythm-patches/synth/template.fxp.provenance.yaml` 3 件 ξ 新規追加、 全 6 drum 種共通 1 件のみ)、 acceptance workflow 11 step structure 不変 (= step 2 + 3 wording のみ AI/toolchain 軸に置換)、 §決定 27 (11) ξ scope-in literal 5 件 subsection 新規追加、 §決定 27 (1) AI 役割境界 2 軸 → 4 軸 expand、 23rd session μ で生成済 `2608_bd.patch-spec.yaml` workflow comment block step 2/3 wording のみ AI/toolchain 軸に update (= patch parameter / structure 不変)、 driver / fixture / verify script / runtime semantics 軸 ADR-0026-0032 完全不変、 ξ scope = ADR-0033 doc only + 2608_bd.patch-spec.yaml workflow comment 軽微 update、 spike 投資調査 (= Surge XT `.fxp` format / patch-spec → fxp bridge feasibility / 最小 spike) は ο commit 以降に分離) + 23rd session 第 15 commit ο1 = ο1 spike実 = Surge XT `.fxp` bridge investigation + schema + read-only spike (= 4 件新規 = `docs/design/rhythm-patches/synth/SURGE_XT_FXP_BRIDGE_INVESTIGATION.md` + `template.fxp.provenance.yaml` + `parameter-allowlist.yaml` + `scripts/fxp_template_patch.py`、 計 +725 / -0 lines、 VST2 FXP container 28 byte header literal 確定 + FPCh body parse synthetic fixture で動作確認、 「bridge, not full synthesizer patch compiler」 軸 technical 成立、 feasibility 総合 HIGH、 ο2-10 chain 候補 literal、 driver / fixture / verify script / runtime semantics 完全不変) + 23rd session 第 16 commit π = §決定 27 ξ → π 軸転換 = AI/toolchain factory CC0 template acquisition + XML element name allowlist patching 軸統合 (= 越川氏 hand-on engineering 接点を **完全ゼロ化**、 ξ で残った「越川氏 template 作成 1 度」 も AI/toolchain 軸に置換、 ν 本質「越川氏 認知負荷を engineering correctness から解放」 完全貫徹、 5 件成果 = (a) `assets/drum_samples/synth/templates/2608_template.fxp` 実 acquisition = Surge XT factory `Templates/Init Sine.fxp` (= CC0 license) byte-identical 複製 31538 byte sha256 0366cc1057a4286e243ff44c128a4a27a17476a92c72fcdbd3c5d6d105082839、 (b) `template.fxp.provenance.yaml` 全 fields fill + license narrative + chunk 内部構造 literal、 (c) `parameter-allowlist.yaml` XML element name 軸対応 update + prohibited / allowed XML elements literal、 (d) `SURGE_XT_FXP_BRIDGE_INVESTIGATION.md` § 11 = π findings literal (= factory CC0 license + chunk 内部構造 (= sub3 magic + 28 byte binary header + XML body + 8 byte trailing binary) + XML element name patching approach)、 (e) ADR-0033 §決定 27 = π 軸統合 = AI 役割 4 → 5 軸 (= +template acquisition) + workflow step 2 内 template acquisition AI 化 + 成果物 path canonical π refinement + π scope-in 2 件追加 (= factory CC0 acquisition + XML element name patching)、 π 中核 wording 2 件追加 = 「factory CC0 template acquisition」 + 「XML element name allowlist patching」、 越川氏 work = rendered audio aesthetic audition / accept のみ (= step 10、 唯一の hand-on 接点)、 ν 中核 wording + ξ 中核 wording 全部維持、 π2 以降 chain = XML extract → element 一覧 dump → allowlist fill → patch subcommand 実装 → bridge invoke → render → self-analysis → 越川氏 audition、 driver / fixture / verify script / runtime semantics 完全不変) + 23rd session 第 17 commit π2 = ο1 spike 評価 + 実 evidence proof (= scripts/fxp_template_patch.py に extract-xml + survey-params + verify-repack 3 subcommand 追加 = 279 → 560 行、 byte-identical stdout/file 軸、 regex-based value modification + chunkByteSize recalc bit-exact 実証、 chunk 内部構造実測訂正 = trailing 8 → 0 byte、 「patch-spec → template .fxp XML patching bridge が実現可能」 evidence-level proof PASS) + 23rd session 第 18 commit π3 = parameter-allowlist.yaml comprehensive enumeration (= 14 → 56 element fill、 11 group 構成 = OSC1 12 / OSC2 6 / OSC3 3 / OSC mixer 8 / Filter1 6 / Filter2 3 / Filter routing 3 / Waveshaper 2 / AEG 5 / FEG 5 / Master + scene + noisecol 3、 全 56 件 template 実測値 + patch_spec_field mapping + notes 付与、 56/558 = 10.0% で patch-spec.yaml 主要 field 100% カバー、 残り 502 件 default change_protected、 status: partial-fill-core-elements → active 移行 = π4 patch subcommand 実装で invoke 可能 state、 driver / fixture / verify script / runtime semantics 完全不変) + 23rd session 第 19 commit π4 = `scripts/fxp_template_patch.py patch` subcommand 実装 + invariant verify pre-write gate + 5 test scenario PASS (= 8 段 process + 3 helper functions、 script 560 → 843 行、 patch-spec passthrough 数値型 only filter で XML 破壊 safety 確立、 invariant verify 4 軸 (= sub3 / trailing / allowlist 外 element / XML well-formed) pre-write 検出機能発揮、 mechanical proof PASS / semantic proof は π10 以降の別軸) + 23rd session 第 20 commit π5 = canonical BD candidate 生成 = AI/toolchain bridge 初回 actual invoke (= `assets/drum_samples/synth/patches/2608_bd.fxp` 31351 byte / sha256 15161403b6...8397a9c713、 patch-spec 13 numeric 値 passthrough + 6 string skip、 13 element 修正、 chunkByteSize 31478 → 31291 (= -187)、 invariant verify 4 軸 PASS、 越川氏 hand-on engineering ゼロ + AI/toolchain bridge workflow 初回成果物 PMDNEO repo 内配置 milestone、 mechanical proof PASS、 semantic correctness は π10 越川氏 aesthetic accept で判断) + 23rd session 第 21 commit π6 = §Annex A-8 fxp2wav-surge external producer 採用根拠 後埋め (= spike repo `koshikawa-masato/surge-spike` `c6b957e` commit 評価、 6 採用条件全 PASS + deterministic 性 `SURGE_RNG_SEED` env var 経由確立、 1 行 patch `SurgeStorage.h RNGGen` で seed=2608 で 2 回 render bit-identical sha1 `a29aa45c...` 確認、 CLI 仕様 `--patch / --out / --note 36 / --velocity 127 / --duration-ms 800` literal、 PMDNEO repo boundary 完全遵守 = §決定 25 ι'' literal 規律 untouched、 採用判定 = ✓ PMDNEO 側採用、 §決定 27 (6) workflow step 3 external producer 指名、 caveats 7 件 = deterministic patch / mono downmix / CLI 拡張 / build 軽量化 / error handling / PMDNEO `.fxp` 未検証 / skipPatchLoadDataPathSentinel、 採用後 sub-sprint 継続 5 項目 = setRandomSeed 公開 / upstream PR / PMDNEO 6 音 smoke test 拡張 / CLI flag 化 / headless error sink、 π7 actual integration test = PMDNEO `2608_bd.fxp` (= π5 生成済) + fxp2wav-surge で `.wav` render = caveat 6 解消の別 commit 予定、 PMDNEO 内 tools/ / vendor/ 取り込み禁止 literal 維持、 driver / fixture / verify script / runtime semantics 完全不変) + 23rd session 第 22 commit π7 = actual integration test 実施 + audition NG + Kick 909ish reference 比較 + passthrough 禁止 wording 追加 (= caveat 6 解消 ✓ = PMDNEO 2608_bd.fxp が fxp2wav-surge で load + render 成功、 sha1 f32e272c... / sha256 e46960ae... / 88244 byte / 65 Hz fundamental / -25 dBFS peak、 越川氏 2026-05-17 audition NG = 「ものすごい低音だけで BD ではない」、 root cause 確認 = patch-spec passthrough の semantic mismatch (= ms / Hz が Surge XT 内部 log-time / log-freq scale と不整合)、 Kick 909ish.fxp (= spike smoke test で正常 BD render) と 2608_bd.fxp を 56 element 並列 extract + 主要 differ 25 件 + Surge XT scale 解釈 candidates 6 件 mapping literal 化、 SURGE_XT_FXP_BRIDGE_INVESTIGATION.md § 15 = π7 integration test + audition NG + Kick 909ish reference + Surge XT scale 推定 + π8 chain candidate の 7 sub-section 新規追加、 ADR-0033 §決定 27 (1) AI 役割 3 = unit conversion layer 必須化 wording 追加、 scope-out 10 → 11 件拡張 = (11) patch-spec value 不変換 passthrough 禁止 = engineering FAIL root cause 明示、 中核 wording 追加 = 「human-readable values must be converted to Surge XT internal scale」、 越川氏 directive = A (Kick 909ish 比較) → D (unit conversion 実装) 順、 ν 規律違反反省点 = engineering FAIL を audition 前に detect すべきだった (= AI self-analysis 10 項目 = π8+ で実装)、 π8 unit conversion layer 実装 / π9 re-invoke + re-render / π10 audition の chain literal、 driver / fixture / verify script / runtime semantics 完全不変) + 23rd session 第 23 commit π8 = feature-guided synthesis investigation (= 越川氏 directive = Grok 提案踏襲、 target-feature matching optimization 軸統合の architectural pivot 候補調査、 `docs/design/rhythm-patches/synth/FEATURE_GUIDED_SYNTHESIS_INVESTIGATION.md` 新規作成 320 行 = 7 section comprehensive analysis (= 0 TL;DR / 1 現状認識 / 2 methodology / 3 領域別調査 = 3.1 feature extraction librosa / 3.2 parameter optimization CMA-ES Instrumental 2603.15905 等 / 3.3 wavetable route WaveEdit + wt-tool.py / 3.4 ADR-0033 影響分析 / 3.5 candidate A/B/C 比較 / 3.6 推奨 architecture / 4 §決定 27 update 提案 / 5 π9+ chain / 6 関連外部資料 / 7 scope-out)、 inversion stance = FFT 直接逆算非採用 / feature-guided search 採用、 重要 finding = wavetable route は OSC scale mismatch のみ解決で silver bullet ではない (= envelope/filter/drive unit conversion 依然必要)、 推奨 architecture = Option A++ = feature-guided parameter search (= librosa + scipy.optimize / CMA-ES + Kick 909ish baseline + 6-10 continuous parameter のみ search + enum routing 系 baseline 固定 + AI self-analysis 10 項目を loss vector として再利用)、 candidate B (wavetable) は二次候補で sample derivative narrative risk + WaveEdit GUI 経由 越川氏 hand-on 復活 risk 警戒、 「reference-inspired (= OK) vs derivative (= NG)」 境界 wording 提案 = feature 経由 / 数値関係 / 結果 wav / narrative / ADR 整合 5 軸 table 化、 §決定 27 / §決定 14 / §決定 1 全部整合維持、 越川氏 hand-on engineering 接点ゼロ規律維持、 ADR-0033 §決定 27 (1) AI 役割 5 拡張 wording + §決定 14 補強 + §決定 28 候補 = 越川氏 review 後 π9 commit で literal 反映予定、 driver / fixture / verify script / runtime semantics 完全不変、 本 commit = investigation document only、 librosa / scipy import + script 作成は π10 以降) + 23rd session 第 24 commit π9 = ADR-0033 §決定 27 + §決定 14 = π8 investigation 採用判断 literal 反映 (= 越川氏 directive 通り Option A++ 採用 / wavetable route 二次候補保留、 §決定 14 補強 = reference-inspired vs derivative 5 軸 table + 適用例 4 件 + 判断規律 literal 本体組込、 §決定 27 (1) AI 役割 5 拡張 = post-render check + search loss feedback 2-role hybrid 化、 §決定 27 (12) 新規 sub-section = target-feature matching optimization axis = target_wav reference + librosa feature extraction + Kick 909ish baseline + CMA-ES / scipy.optimize 6-10 continuous parameter search + AI self-analysis loss vector 再利用 + 越川氏 aesthetic gate 最終 literal、 wavetable route 位置付 literal = secondary option (= IFFT-based synthetic only)、 §決定 14 sound-alike caution 補強 統合 = target wav 5 軸 reference-inspired 必須 / sample byte 保存禁止 literal、 §決定 27 scope-out 11 → 13 件拡張 = (12) FFT direct inversion 永久禁止 + (13) wavetable from existing sample byte derivative 永久禁止 (= IFFT-based synthetic のみ scope-in)、 §決定 27 中核 wording 2 件追加 = feature-guided parameter search, not exact inversion + target wav as aesthetic reference, not source-of-derivative、 §決定 27 closing paragraph update = AI 役割 5 2-role hybrid 化 / 軸数 11 → 12 / scope-out 11 → 13、 driver / fixture / verify script / runtime semantics 完全不変、 本 commit = ADR doc only = librosa / scipy import + script 作成は π10 以降) + 23rd session 第 25 commit π10 = feature extraction spike + Kick 909ish baseline + 2608_bd distance metric proof (= scripts/feature_search.py 新規 340 行 = extract + compare 2 subcommand、 librosa 0.11.0 + scipy 1.17 + soundfile + numpy install + 動作確認、 Kick 909ish.wav baseline feature 確立 = peak -4.5 dBFS / attack 5.78 ms / decay 1e 1.07 ms / spectral centroid 159 Hz / low_band_ratio 80% (= BD characteristic profile)、 2608_bd.wav vs Kick 909ish 比較 = distance_score 39.83 (= 大差) + 主要 differ 5 feature = attack 5.78 vs 781 ms (= envelope broken) / trailing 766 vs 0 ms (= decay 効かず sustain 化) / peak -4.5 vs -25 dBFS (= 越川氏「低音量」 audition と完全一致) / spectral centroid 159 vs 262 Hz / low_band 80% vs 99% (= filter 効かず pure sine)、 aesthetic gate NG ↔ feature distance 方向一致 実証、 FEATURE_GUIDED_SYNTHESIS_INVESTIGATION.md § 6 追記 = 5 sub-section (= 6.1 spike script + 6.2 Kick baseline + 6.3 distance proof + 6.4 aesthetic ↔ feature 方向一致 + 6.5 π11+ chain)、 driver / fixture / verify script / runtime semantics 完全不変、 本 commit = 1 新規 script + 1 doc update + ADR frontmatter chain、 §決定 27 (12) target-feature matching optimization axis 実装着手の第 1 step PASS、 π11 distance metric tune / π12 optimize subcommand 実装 / π13 .fxp 再生成 / π14 self-analysis / π15 audition chain literal) + 23rd session 第 26 commit π11 = metric calibration + validate subcommand + analysis-report.yaml schema 固定 (= 越川氏 directive 通り search loop 前 metric calibration、 新規 file = docs/design/rhythm-patches/synth/feature-rules.yaml 240 行 = BD 8 thresholds + 15 distance_weights + 8 failure_categories + analysis_report_schema 固定、 新規 subcommand = scripts/feature_search.py validate <wav> --rules ... --drum-type BD --format yaml|json + helper _evaluate_criterion (= rule DSL safe parser) + _get_feature_value、 BD 初期 7 rule → π11 calibration 8 rule = (1) attack_ms < 20 critical (2) peak [-6,-3] dBFS warning (3) low_band [0.5, 0.95] critical (= upper bound 追加 = pure sine catch) (4) decay_1e_ms [0.5, 500] warning (= range 緩和 + severity 下げ = π11 calibration) (5) clipping == 0 critical (6) spectral_centroid < 500 Hz warning (7) transient > 2.0 warning (8) trailing_silence_ms > 300 critical = π11 新規 = 最強 discriminator for sustain 化、 failure_categories 8 件 + likely_root_cause + fix_axis 分類 literal = envelope_broken_attack / envelope_broken_decay / sustain_化 (= π11 新規) / volume_mismatch_peak / spectral_imbalance_low / spectral_imbalance_centroid / transient_weak / clipping、 動作確認 = Kick 909ish.wav 8/8 PASS = engineering_pass exit 0 / 2608_bd.wav 3/8 PASS + 5 fail categorize = engineering_fail exit 67 = 越川氏 audition 完全方向一致、 analysis_report_schema 固定 = ν step 4 deliverable と field 共有 = π14 で validate output redirect で literal 化可能、 FEATURE_GUIDED_SYNTHESIS_INVESTIGATION.md § 7 = π11 evidence 7 sub-section 追記 + § 7 scope-out 維持 → § 8 へ rename、 driver / fixture / verify script / runtime semantics 完全不変、 §決定 27 (5) AI self-analysis 10 項目 が validate subcommand で literal 動作確認、 §決定 27 (12) target-feature matching optimization axis = π12 search loop の前段 metric foundation 完成、 π12 = optimize subcommand 実装 + CMA-ES / scipy.optimize 経由 6-10 continuous parameter search + closed loop) + 23rd session 第 27 commit π12 = black-box optimization closed loop end-to-end + engineering_pass 達成 (= scipy.optimize.differential_evolution + bridge + render + score 連鎖、 feature-rules.yaml drum_rules.BD に search_space 9 continuous parameter literal 追加 = a_env1_attack/decay/release + a_env2_decay + a_filter1_cutoff/lowcut + a_level_noise + a_ws_drive + a_osc1_pitch、 scripts/feature_search.py optimize subcommand +400 行 = scipy + subprocess + trial history + 2 reports 生成、 fixed_baseline = 45 elements (= Kick 909ish.fxp 由来 enum/routing 系)、 1 回 end-to-end run = 128 trials in 12 sec = best score 0.911 (= π7 NG 39.83 から 43x 改善) = engineering_pass exit 0 + 1 warning (= volume_mismatch_peak、 critical 0 件)、 best params = a_env1_attack -7.44 (= 3 ms) / a_env1_decay -1.60 (= 330 ms) / a_filter1_cutoff -7.42 / a_ws_drive 7.03、 critical 改善 = attack 781→11 ms (= envelope 修復) + trailing 0→694 ms (= sustain 化 解消) + peak -25→-15.5 dBFS + low_band 99%→90%、 deliverables = optimization-report.yaml (= 55KB、 全 trial history + reproducibility command + top 5 + sha256 inputs) + analysis-report.yaml (= 3.4KB、 validate output) + best fxp (= /tmp ephemeral) + best wav (= external spike repo)、 π12 中核 wording 3 件 ADR §決定 27 (12) 追加 = LLM is not the selector / fixed seed + trial history / machine-checkable gate before audition、 driver / fixture / verify script / runtime semantics 完全不変、 越川氏 hand-on engineering ゼロ維持 = π13 越川氏 aesthetic audition only、 FEATURE_GUIDED_SYNTHESIS_INVESTIGATION.md § 8 = π12 evidence 8 sub-section + § 8 scope-out → § 9 rename) + 23rd session 第 28 commit π13 = π12 result aesthetic_rejected + metric_v1 retroactive reclassification + metric_v2 calibration plan (= 越川氏 audition 2026-05-17 literal = 「reference / target の BD と全然似ていない」、 metric_v1 score 改善 (= 39.83 → 0.911) と human audition 判断 が乖離、 重要 design lesson 3 件 ADR §決定 27 (12) 中核 wording 追加 = optimizer is also not the final selector / human aesthetic gate is authoritative / metric pass ≠ aesthetic pass + metric versioning + audition correlation check 必須、 wording 規律 = 「current metric decreased」 のみ / 「近づいた」 「reference に類似」 等 aesthetic 含意 wording 禁止、 metric_v1 limitation 分析 = static feature 中心で time-varying spectral envelope + MFCC / log-mel + onset detail + perceptual loudness 不在 = drum 音色 core を捉えきれず、 metric_v2 calibration 計画 = MFCC + log-mel spectrogram + onset envelope time-series + time-varying spectral flux + LUFS perceptual loudness + spectral contrast 追加 + DTW / per-frame Euclidean、 human audition correlation check methodology = N=5-8 candidate hand-craft + 越川氏 aesthetic score + Spearman correlation > 0.7 verify を optimize 実行前 hard gate、 π12 retroactive treatment = status aesthetic_rejected (= NOT engineering_pass) / metric_pass_v1 retroactive label / 2608_bd.optimized.wav rejected candidate / 既存 2608_bd.fxp π5 NG version 維持 (= overwrite せず metric_v2 calibration 完了後再判断)、 docs + reports update only (= scripts / driver / fixture / verify script 完全不変)、 2608_bd.optimization-report.yaml + analysis-report.yaml に aesthetic_audition + metric_calibration_status section 追記 + reclassification metric_pass_v1 literal、 feature-rules.yaml status: initial-calibration → aesthetic_uncorrelated_v1 retroactive、 FEATURE_GUIDED_SYNTHESIS_INVESTIGATION.md § 9 = π13 metric calibration failure analysis 8 sub-section 新規 + § 9 scope-out → § 10 rename、 π14 metric_v2 spike + π15 audition correlation check + π16 metric_v2 で optimize 再 run + π17 audition の chain literal) + 23rd session 第 29 commit π14 = metric_v2 feature extraction + feature-rules-v2.yaml + audition-check subcommand + N=8 candidate generation (= 越川氏 directive 通り、 5 deliverables = (1) scripts/feature_search.py extract_features に v2 feature 10 件追加 = MFCC mean/std (13 coeff list × 2) + log-mel mean/std (13 mel bands list × 2) + onset_strength mean/std/peak (3 scalar) + spectral_contrast_mean (7 bands list) + spectral_flux_std (scalar) + lufs_integrated (= pyloudnorm ITU-R BS.1770) (2) _compute_distance_score を vector feature 対応に拡張 = list of floats を L2 vector distance + target L2 norm normalize + 既存 scalar handling 共存 (3) feature-rules-v2.yaml 新規 = v1 8 thresholds 維持 + v2 distance_weights (= mfcc_mean 3.0 / log_mel_mean 2.5 / spectral_contrast 2.0 / lufs 1.5 等) + status: calibration-in-progress (4) audition-check subcommand 新規 = audition-input.yaml + --scores "5,1,4,3,..." + --threshold 0.7 で Spearman rank correlation 計算 + verdict (= PASS 負方向 r > 0.7 / FAIL otherwise) + audition-output.yaml 生成 + exit code 0 (= pass) / 68 (= correlation_fail) (5) N=8 candidate wav 生成 (= ~/Projects/surge-spike/test-assets/audition_candidates/) = candidate_01 kick_909ish target reference + 02 π5 passthrough NG + 03 typical BD hand-crafted + 04 snappy BD + 05 deep BD + 06 midrange BD + 07-08 random sample seed=2608/31337、 audition-input.yaml に literal params + sha + scoring template、 dependencies = pyloudnorm 0.1.1 install + librosa 0.11.0 (= π10 install 済) 既存、 dummy run 検証 = scores 5,1,4,3,3,4,2,2 で Spearman -0.788 |r| > 0.7 → PASS verdict (= 動作 OK、 但し越川氏 実 audition で異なる scores 想定)、 wording 規律遵守 = current metric correlation のみ literal / aesthetic 含意 wording 禁止、 driver / fixture / verify script / runtime semantics 完全不変、 π15 越川氏 audition + scoring → π15.5 correlation 判定 → π16 PASS なら optimize 再 run / FAIL なら metric_v3 calibration へ、 越川氏 hand-on engineering 接点ゼロ維持 = audition only) + 23rd session 第 30 commit π15 = audition directive 軸転換 = absolute 1-5 score + Spearman correlation → **pairwise preference learning** (= 越川氏 directive 2026-05-17 = π14 metric_v2 correlation plan は metric calibration であって preference learning ではない、 軸を「越川氏 が何を好むか」 の literal 学習に転換、 5 deliverables = (1) scripts/feature_search.py に preference-learn subcommand 新規 +204 行 = sklearn LogisticRegression (= pairwise feature difference を入力 / preference label を 0/1 で学習) + StandardScaler 標準化 + 20 scalar feature only (= vector feature 除外で robustness 確保) + reject expansion (= rejected ID は全 non-rejected より < 扱いで pair に展開) + per-candidate predicted preference score 出力、 (2) preference-input.yaml 新規 = 8 candidate 全 28 pair enumerate + preference: null (= 越川氏 が "A"/"B"/"tie" を埋める) + rejected section、 (3) preference-dataset.yaml = 学習用 pair literal (= 越川氏 fill 後 generate)、 (4) preference-model-report.yaml = train_accuracy + per-candidate predicted preference score + coefficient inspection、 (5) FEATURE_GUIDED_SYNTHESIS_INVESTIGATION.md § 11 新規 = π15 directive shift 8 sub-section = 11.1 越川氏 directive reframing + 11.2 deliverables 3 件 + 11.3 logistic pairwise ranking model rationale (= Bradley-Terry rejected / preference regression rejected / logistic 採用) + 11.4 越川氏 input flow + 11.5 dummy run results + 11.6 wording discipline 5 件 + 11.7 next_step chain + 11.8 π14 retroactive treatment as v1 + § 11 scope-out → § 12 rename、 ADR-0033 §決定 27 (12) 中核 wording 5 件追加 = (a) feature matching alone does not model human aesthetic preference (= π14 metric_v2 correlation plan was insufficient because it calibrates feature metric, not human preference literal 反映) + (b) metric correlation is not preference learning + (c) human preference is the primary objective + (d) feature distance is auxiliary + (e) optimizer is not final selector; preference model is also not final selector (= π13 「optimizer is also not the final selector」 wording 拡張 = LLM + optimizer + preference model 全部 supporting layer、 越川氏 final accept のみ authoritative)、 dummy run 検証 = 11 pair + 1 reject = 36 training sample 生成 + train_accuracy 1.0 (= 実 audition 後は overfitting risk 別軸)、 wording 規律 = 「current metric correlation = 0.XX」 / 「predicted preference score = 0.XX」 のみ literal / 「越川氏 が好む方向」 等 aesthetic 含意 wording 禁止、 model 軸 = pairwise comparison + reject label の minimum dataset (= 1-5 absolute score より cognitive load 低 + ranking 軸学習に直結)、 next_step chain literal = π15 越川氏 fill preference-input.yaml → π15.5 preference-learn invoke + model 学習 + per-candidate predicted score 出力 → π16 PASS なら optimize 再 run (= preference model + feature distance combined objective) / FAIL なら preference data 拡充 or reject expansion 戦略変更、 driver / fixture / verify script / runtime semantics 完全不変、 π14 audition-check subcommand retain (= metric_v2 calibration が必要になった場合の sub-tool として v1 位置付け、 v2 = preference learning が primary)、 越川氏 hand-on engineering 接点ゼロ維持 = pairwise audition + reject judgement only) + 23rd session 第 31 commit π15.A = pairwise audition helper script 追加 (= 越川氏 directive 「A/B/tie するための script + 結果保存」、 `scripts/audition_pairwise.py` 新規 約 240 行、 入出力 = `preference-input.yaml` に直接書き戻し + 起動時 `.bak` backup、 操作 = A=好み A / B=好み B / T=tie / R=replay / S=skip / Q=save & quit、 逐次保存 (= 各入力後 yaml rewrite) で中断対応 + 再起動で残り pair から再開可能、 28 pair の未入力分のみ列挙 + 既入力 pair は自動 skip、 afplay subprocess block 同期 + A → B 順 + replay 対応、 reject prompt 末尾 (= 全 candidate id 表示 + 既存 rejected marker 表示 + space 区切入力 + valid id filter)、 dry run 検証 = backup 作成 + Q 即終了 + yaml round-trip byte 同一 OK、 driver / fixture / verify script / runtime semantics 完全不変、 PyYAML round-trip で comment 消失するが audition 後の input.yaml は preference-learn subcommand 入力としてのみ使うため運用上問題なし、 `.bak` ファイルが backup、 next step = 越川氏 invoke `python3 scripts/audition_pairwise.py docs/design/rhythm-patches/synth/preference-input.yaml` で pair audition 着手可能) + 23rd session 第 32 commit π15.5 corrective = input interpretation failure 検出 + schema / script / ADR / memory 再発防止統合修正 (= 越川氏 audition 後 invoke した preference-learn 結果を確認したところ「越川氏 audition と完全一致 / candidate_01 best / ranking 0.9864 〜 0.0666」 と report したが、 越川氏指摘 = 8 candidate **全部 aesthetic_rejected**、 preference A/B/tie は relative ranking のみで absolute acceptance ではない、 私の report wording は false claim、 root cause = こちらの input schema 設計が不十分 = relative ranking と absolute acceptance を別軸として扱う field がなく、 rejected: [] を「reject 入力未実行」 ではなく「全 candidate accept」 と false 解釈、 reject prompt の wording 「BD として明らかに不可」 が absolute acceptance gate を捉え損ねた、 7 件修正 = (1) preference-model-report.yaml に validity_status section 上書き = status invalid_input_interpretation + retraction 4 件 + root cause + true state + literal rules + retained content note + next action、 削除せず failure analysis evidence として保持 (2) preference-input.yaml schema 修正 = three_axis_separation section 新規 (= axis 1 pairwise relative / axis 2 individual acceptance / axis 3 global reject) + acceptance_status section 新規 (= global_reject_all: true / accepted_candidates: [] / rejected_candidates: 全 8 件 / notes 「relative preference ≠ candidate acceptance」 literal)、 既存 preferences.pairs 28 件 A/B/tie + rejected: [] は relative ranking retention として保持、 audition_protocol step 3/4 wording 「relative judgment のみ、 採用可否ではない」 / 「acceptance_status を別軸で記録」 に改訂 (3) scripts/feature_search.py preference-learn に acceptance_status gate 追加 = global_reject_all: true 検出時 training skip + status invalid_no_accepted_candidate + literal rules 5 件 + next action 出力 + EXIT_INVALID_STATE (= 67) 新規 sentinel、 越川氏 真の audition input で動作確認 = exit 67 + invalid report 出力 + training 実行されず (4) scripts/audition_pairwise.py reject_prompt → acceptance_prompt rename + 2 段構成 = Step A 「全 candidate reject [Y/N/S]」 一括 + Step B (= N 時のみ) 「各 candidate [A/R/S] 個別判定」、 結果は acceptance_status section に save (= preferences.pairs と独立軸)、 docstring + argparse description + summary output を 3 軸独立 protocol wording に統一 + --skip-rejected → --skip-acceptance rename (5) ADR §決定 27 (12) 中核 wording 6 件追加 (= 1 件本 entry 開始時の重複防止メモ) = relative preference ≠ candidate acceptance / pairwise ranking is auxiliary to acceptance gate / all candidates can be globally rejected even if pairwise preferences exist / 3 軸独立 / preference model is not the accepted asset selector / 越川氏 aesthetic gate is authoritative の 6 件、 π13 「optimizer is not final selector」 を asset acceptance 軸まで拡張 (6) FEATURE_GUIDED_SYNTHESIS_INVESTIGATION.md § 11.9 新規 = π15.5 input interpretation failure literal documentation = false claims 列挙 / root cause / 修正後 3 軸 protocol、 § 11 scope-out → 既存 § 12 維持 (7) memory `feedback_relative_preference_vs_absolute_acceptance.md` 新規 + MEMORY.md index 1 行追加 = ranking と acceptance は別軸 / pairwise preference は relative judgment / absolute acceptance は別 schema field / global reject all は独立 flag の generalizable 規律、 重要な認識 = 「越川氏の入力が悪かった」 ではなく「こちらの入力解釈・schema 設計が不十分だった」、 越川氏 directive 「all candidates can be globally rejected even if pairwise preferences exist」 を ADR レベル literal で固定、 π15.5 = preference learning 成功 NOT、 input interpretation failure を検出し schema / script / ADR / memory に再発防止を入れる corrective commit として扱う、 next audition flow = 新規 candidate set 2 生成 + audition v2 protocol (= acceptance gate primary + pairwise relative auxiliary) で再着手、 driver / fixture / verify script / runtime semantics 軸 ADR-0026-0032 完全不変、 本 commit = corrective scope = ADR doc + investigation doc + 2 script + 2 yaml + memory + MEMORY.md の 7 件 only) + 23rd session 第 33 commit (= 113d5bc Codex 経由) = deterministic drum analysis profile artifacts 追加 = analyze-drum subcommand 新規 (= scripts/feature_search.py +662 行) + USAGE.md / PARAMETER_SENSITIVITY_AND_ANALYSIS_DESIGN.md / BD_ANALYSIS_AND_SENSITIVITY_TABLE_DESIGN.md 3 doc 新規 (= +1561 行 total)、 chain = WAV → common feature extraction (= waveform / level / timing / spectrum / pitchedness 5 category 共通 schema) → deterministic drum-kind classifier (= 6 kind BD/SD/HH/CYM/TOM/RIM + confidence + rule_version) → profile selection (= classifier auto or manual override) → profile-specific summary (= focus_features + parameter_axes_for_sensitivity + feature_snapshot) → artifact 3 件 (= analysis-scalar.yaml + analysis-timeseries.json + analysis-summary.yaml)、 classifier_scope = profile-selection-only-not-acceptance literal、 scope = metric_is_final_judge: false / optimizer_involved: false / human_audition_final_gate: true、 driver / fixture / verify script / runtime semantics 完全不変 + 23rd session 第 34 commit π15.6 = one-factor sensitivity sweep vertical slice 成立 = π15.5 corrective 軌道修正路線で「score 最大化ではなく 2608_*.wav の parameter sensitivity 対応表生成」 を入口 tool 化 (= 越川氏 directive 「optimizer / preference-learning 続行ではなく deterministic analysis + drum profile + sensitivity table chain」、 ADR §決定 27 (12) 軌道修正 wording に「sensitivity table is diagnostic only / one-factor sweep / not a selector」 系の規律維持)、 3 件修正 = (1) scripts/feature_search.py に sensitivity-sweep subcommand 新規 +約 450 行 = 1 parameter / 1 delta / 1 render / 1 analyze / 1 row literal protocol + helpers 6 件 (= _fxp_load_segments / _fxp_extract_parameter_value / _fxp_patch_single_parameter / _invoke_fxp2wav_surge / _analyze_drum_into / _scalar_feature_delta / _effect_summary_from_delta) + fxp_template_patch.py module import で sub3 / xml / trailing 構造保持 + invariant verify pre-write + SURGE_RNG_SEED=2608 env 経由 deterministic + sensitivity-table.yaml + .csv 2 format 出力 + scope literal 5 件 (= optimizer 不使用 / best candidate 選別なし / accept/reject 判断なし / effect_summary は機械 literal label / human audition final gate)、 vertical slice 動作確認 = baseline `assets/drum_samples/synth/patches/2608_bd.fxp` (= diagnostic-baseline / aesthetic-rejected label) + parameter a_osc1_pitch (= baseline value 0) + deltas -3,-1,0,1,3 semitone で 6 render (= baseline + 5 trial) + 6 analyze 実行 = delta=0 row の wav_sha256 が baseline render と完全一致 (= e46960ae...) で deterministic round-trip literal proof、 delta=-3 で attack -6.3 ms / sub band +0.159 / body_freq -0.14 Hz / effect_primary=shorter_attack、 delta=+3 で attack +3.9 ms / sub band -0.398 / body_freq +21.5 Hz (= 45 Hz → 67 Hz) / effect_primary=longer_attack の 5 row table 生成、 1 軸 isolation 不完全 (= a_osc1_pitch だけ動かしても band balance + body pitch + attack が同時に動く) を literal 観察 = interaction を捉えた diagnostic 性質を vertical slice で literal 確認、 (2) docs/design/rhythm-patches/synth/USAGE.md に "One-factor sensitivity sweep" section 追記 = trial protocol literal + command literal (= --deltas=... 形式必須の argparse leading-dash 罠 + --producer-cmd 絶対 path + --seed 2608 default SURGE_RNG_SEED env) + output structure (= baseline/ + delta_<value>/ + sensitivity-table.yaml + .csv) + vertical slice π15.6 evidence + constraints 6 件 + Claude Code handoff section、 (3) ADR-0033 frontmatter chain 113d5bc + π15.6 entry literal 追加 (= 33 commit + 34 commit 含む chain 化、 113d5bc Codex 経由 retroactive entry 同時記録)、 sweep output (= /private/tmp/pmdneo-sensitivity-osc1-pitch/ 配下の baseline/ + 5 delta dir + sensitivity-table.yaml + .csv) は repo に入れず ephemeral retain = tool の再現手順を repo に固定 + 生成物は必要になった段階で artifact chain として入れる安全 path、 USAGE.md 内 literal command で越川氏 hand-on で再現可能、 残り 6 軸 (= a_osc1_octave / a_env1_attack / a_env1_decay / a_env1_release / a_env2_decay / a_lowcut) sweep は別 commit / 別 session で横展開、 driver semantics 不変 / 既存 fixture 不変 / 既存 2608_BD.adpcma 非破壊 / optimizer / preference-learning 再開禁止 / selection / accept / reject 判定禁止、 sensitivity table = parameter → feature delta diagnostic 記録 only) + 23rd session 第 35 commit π15.7 = 6 軸 horizontal sweep finding + bug fix 統合 commit (= π15.6 vertical slice 完了扱いを受けて残り 6 軸 horizontal sweep diagnostic 実行 + 1 bug fix + finding 文書化、 commit value = 「生成物ではなく診断結果」 を repo に固定、 /private/tmp の generated artifact (= WAV / patched .fxp / analysis-* / sensitivity-table) は repo 未投入維持、 越川氏 directive 通り unit conversion 実装には進まない literal)、 3 件修正 = (1) scripts/feature_search.py `_fxp_patch_single_parameter` bug fix = outer byteSize 更新 logic を完全 skip 修正 = baseline .fxp の outer byteSize field が literal 0 (= Surge XT factory `Init Sine.fxp` 由来 spec 違反だが Surge XT 自体は値を読まないため動作上問題なし) であるため、 string 短縮 delta (= 例 a_lowcut "-72.00000000000000" → "-72.000000" の 8 byte 短縮) で `0 + (-8) = -8` を unsigned 4 byte に変換しようとして `OverflowError: can't convert negative int to unsigned` 発生、 修正 = outer byteSize update 行削除 + fxp_template_patch.py 流儀 (= outer byteSize は元値維持) と整合化、 vertical slice (= a_osc1_pitch) では string 拡張 only で発見できなかった bug を horizontal sweep の a_lowcut 軸で初発見 (= deltas 0,10,20,30,40 で全 trial が string 短縮 → 確実に表面化)、 (2) docs/design/rhythm-patches/synth/PARAMETER_SENSITIVITY_AND_ANALYSIS_DESIGN.md § 19 新規 6 軸 horizontal sweep finding section 追記 = 8 sub-section = 19.1 共通 deterministic 保証 (= 全 6 軸 delta=0 wav_sha256 = e46960ae... で完全一致 literal、 deterministic sweep chain 成立) / 19.2 軸別分類 表 (= silent axis / isolated axis / multi-axis lever / unexpected interaction 4 分類) / 19.3 silent axes (= a_env1_decay / a_env2_decay = 全 delta で全 feature delta 0 = π5 patch-spec passthrough mismatch literal evidence、 patch-spec 280 ms / 50 ms 値を Surge XT log-time field に注入で `2^280 sec` / `2^50 sec` 非物理値 + internal clamp で effect 不能) / 19.4 isolated axis (= a_env1_attack = attack_ms のみ動く clean single-axis lever、 但し sign 非線形 = -3 で -650 ms / -1 で -283 ms / +1 で -15 ms / +3 で -100 ms = baseline 0 が log-time 極大点 2^0 = 1 sec で両側 attack 短縮) / 19.5 multi-axis lever (= a_osc1_octave = attack + body_freq + band balance 同時、 -2 で attack -11.5 ms / body_freq -21.5 Hz / sub +0.186、 +1 で body_freq +64.6 Hz / sub→low 完全 band shift -0.814 / +0.814、 +2 で body_freq +193.8 Hz / low_mid dominant +0.898、 BD identity 大変動) / 19.6 unexpected interaction (= 19.6.1 a_lowcut → attack 副次 effect = lowcut -72 → -32 引き上げで attack 軸のみ非単調 -7.4 ms / -1.3 ms / -16.8 ms = filter envelope or waveshaper feedback 経路仮説、 19.6.2 a_env1_release sign asymmetric = -3 で tail +108.6 ms / +3 で attack +7.7 ms 微小干渉) / 19.7 重要 finding 6 件固定 = (a) delta=0 row 全 6 軸 baseline SHA 一致 (b) deterministic sweep chain 成立 (c) a_env1_attack isolated attack axis (d) a_osc1_octave multi-axis strong lever (e) a_env1_decay / a_env2_decay silent axis = π5 mismatch literal evidence (f) a_lowcut / a_env1_release unexpected interaction / 19.8 結論 = 現 baseline は sensitivity 測定で 6 軸中 2 軸 silent state、 unit-converted baseline 作成が次判断候補だが本 commit では unit conversion 実装には進まない literal、 越川氏 directive 維持 (= optimizer 再開禁止 / preference-learning 再開禁止 / unit conversion layer 実装は次 commit 判断軸で本 commit はここでは記録のみ / best 選別禁止 / accept/reject 判定禁止 / generated artifact repo 未投入)、 (3) ADR-0033 frontmatter line 3 chain π15.7 entry literal 追加、 重要 finding として固定 = 「π5 patch baseline の structural defect が sensitivity sweep で表面化」 + 「unit conversion layer が次判断候補だが本 commit では実装しない」 = 「今の baseline は sensitivity を測るには一部壊れている」 という事実を repo レベルで literal 化、 次の判断は unit-converted baseline を作るかどうか、 generated artifact (= /private/tmp/pmdneo-sensitivity-osc1-octave / -env1-attack / -env1-decay / -env1-release / -env2-decay / -lowcut の 6 dir 配下) は repo 未投入維持 = 「生成物ではなく診断結果」 が commit value、 driver / fixture / verify script / runtime semantics 軸 ADR-0026-0032 完全不変、 本 commit = bug fix + design doc 19 + ADR doc の 3 件 only) + 23rd session 第 36 commit π15.8 = unit-converted diagnostic baseline 1st round 実装 + 検証 (= partial success + failure evidence literal 固定、 「成功した baseline」 ではなく「診断可能性を広げた first diagnostic baseline」 として commit、 越川氏 directive literal 通り 1st round の partial success / failure evidence を literal 化、 2nd round は別 commit / 別判断)、 7 件修正 = (1) docs/design/rhythm-patches/synth/parameter-unit-conversion.yaml 新規 (= conversion formula data 独立 yaml、 4 envelope time = log2(ms/1000) hypothesis + 1 filter cutoff = 12*log2(Hz/440) hypothesis + 残り identity / verified、 全 hypothesis に status: hypothesis + verify_required: true literal + evidence_of_mismatch literal、 sensitivity sweep 逆方向 verify protocol literal、 越川氏 directive 通り conversion formula は data として独立 + script 側 value passthrough 責務分離) (2) docs/design/rhythm-patches/synth/2608_bd-diagnostic.patch-spec.yaml 新規 (= human_intent + converted_internal_value 両方記録 spec、 越川氏 directive 補正 literal、 acceptance.is_aesthetic_candidate: false + aesthetic_acceptance: rejected literal で aesthetic-rejected 明示、 5 parameter conversion 適用 = a_env1_attack 5ms → -7.643856 / a_env1_decay 280ms → -1.836501 / a_env1_release 50ms → -4.321928 / a_env2_decay 50ms → -4.321928 / a_filter1_cutoff 800Hz → 10.349958、 残り identity literal) (3) assets/drum_samples/synth/patches/2608_bd-diagnostic.fxp 新規 generated artifact (= 31352 byte / sha256 c132faee4b7e74a2f6af9f6c522956a489cf62b52280bc3d7c8ae5d78607d256、 既存 2608_bd.fxp = template 不変 retain + chained _fxp_patch_single_parameter で 5 element 注入、 invariant verify pre-write 各 step 経由、 label literal 「unit-converted diagnostic baseline / aesthetic-rejected」) (4) scripts/feature_search.py に make-diagnostic-baseline subcommand 新規 +約 130 行 = patch-spec.acceptance.aesthetic_acceptance literal 「rejected」 でなければ EXIT_INVALID_STATE refuse + chained _fxp_patch_single_parameter で injection_targets 列を逐次 .fxp 注入 + injection_log YAML 報告 + scope literal 5 件 (= aesthetic-rejected / optimizer 不使用 / preference-learning 不使用 / best 選別なし / accept-reject 判定なし)、 越川氏 directive aesthetic-rejected label 明示で aesthetic candidate と誤読防止を script level で gate (5) docs/design/rhythm-patches/synth/USAGE.md に Unit-converted diagnostic baseline section 追記 = command literal + inputs 3 件 + output 2 件 + verify by sensitivity sweep + 1st round result (= sha256 + active 4/6 silent 2/6) + constraints 5 件 (6) docs/design/rhythm-patches/synth/PARAMETER_SENSITIVITY_AND_ANALYSIS_DESIGN.md § 20 新規 9 sub-section = 20.1 artifact identity (= generated .fxp sha256 c132faee... + delta=0 render sha256 9f7f7e23... literal) / 20.2 1st round conversion 5 軸 table / 20.3 6 軸 sweep gate 結果 (= gate 1a env1_decay 解消 ✓ / gate 1b env2_decay 解消 ✗ / gate 2 self-consistent ✓ / gate 3 render 全成功 ✓ / gate 4 optimizer/accept なし ✓) / 20.4 軸別 active/silent 判定 = π15.7 vs π15.8 比較 table / 20.5 重要 finding 6 件固定 = (a) unit conversion 有効性実証 = env1_decay 解消 (b) env2_decay silent 持続 (c) env1_release 想定外 silent 化 (d) active 4/6 silent 2/6 (e) conversion table v0.1.0 hypothesis = 1 round 不完全 (f) 2nd round は別 commit/別判断 / 20.6 失敗 conversion 2 件 literal = Failure A env2_decay 完全 silent 持続 + Failure B env1_release 想定外 silent 化 + 各 仮説候補 3 件 / 20.7 conversion table iterative refine 規律 = 1st/2nd/3rd round protocol literal / 20.8 scope 制約遵守 8 件 / 20.9 commit value 越川氏 wording literal 「成功した baseline ではなく診断可能性を広げた first diagnostic baseline」 (7) ADR-0033 frontmatter line 3 chain π15.8 entry literal 追加、 generated diagnostic .fxp は repo 投入 (= 越川氏 directive literal、 越川氏 hand-on 再現用 + structural baseline 整備) + sweep output (= /private/tmp/pmdneo-diag-*) は repo 未投入維持、 越川氏 hand-on directive 必須 finding 8 件 literal 記録 = generated .fxp sha256 c132faee4b7e74a2f6af9f6c522956a489cf62b52280bc3d7c8ae5d78607d256 / delta=0 render sha256 9f7f7e23c9181effb11d8aa248d73b3d059c93b5a519f5071b113a871a71fa7c / env1_decay 解消 / env2_decay silent / env1_release 想定外 silent / active 4/6 / silent 2/6 / conversion table hypothesis v0.1.0 = 1 round 不完全 / 2nd round 別 commit、 scope 制約 7 件遵守 = optimizer 再開なし / preference-learning 再開なし / accept/reject なし / best 選別なし / 既存 2608_bd.fxp 不変 / 既存 2608_bd.patch-spec.yaml 不変 / driver semantics 不変、 driver / fixture / verify script / runtime semantics 軸 ADR-0026-0032 完全不変、 本 commit = repo 投入 3 件新規 (= conversion yaml + diagnostic patch-spec yaml + diagnostic .fxp) + script 修正 1 件 (= subcommand 追加) + 既存 doc 修正 3 件 (= USAGE + PARAMETER_SENSITIVITY § 20 + ADR) の 7 件 only) + 23rd session 第 37 commit π15.9 = unit-converted diagnostic baseline v0.2.0 = structural dependency expansion (= π15.8 1st round failure 2 件の root cause 分析 + 解消、 6/6 軸 active 化達成、 conversion formula 不変 + structural dependency setup を schema 上で分離、 v0.1.0 は完全 retain で並走、 越川氏 directive literal 通り envmod / sustain range expansion sweep は π15.10 候補 scope-out)、 6 件修正 = (1) docs/design/rhythm-patches/synth/parameter-unit-conversion.yaml v0.2.0 update (= v0.1.0 conversion 軸 retain + version_history v0.1.0 retain + 新規 `structural_dependencies` section 追加 = enables_a_env1_release: a_env1_sustain 0.5 + enables_a_env2_decay 3 件複合 = filter1_type 1 + filter1_envmod 0.5 + env2_sustain 0.0、 各 entry に evidence (= scratch sweep path) + status: hypothesis-verified-by-scratch-sweep + pi15_8_silent_baseline_state literal + effect_magnitude_note literal、 重要規律 = conversion vs structural dependency schema 上で分離、 conversion formula 修正ではなく setup parameter 追加) (2) docs/design/rhythm-patches/synth/2608_bd-diagnostic-v2.patch-spec.yaml 新規 (= v0.2.0 spec、 既存 v0.1.0 不変 retain、 conversion 5 軸 retain + structural_dependency_setup 4 軸新規、 各 parameter に parameter_kind: conversion or structural_dependency_setup 明示、 acceptance.aesthetic_acceptance: rejected literal、 label literal "unit-converted diagnostic baseline v2 / aesthetic-rejected / for sensitivity measurement only / structural dependency expansion"、 injection_targets 9 件) (3) assets/drum_samples/synth/patches/2608_bd-diagnostic-v2.fxp 新規 generated artifact (= sha256 c03d32284d5d9108da905bcce6674b09a5912845cb5b46f0262ab2c069013517 / 31361 byte / 9 element injection、 既存 v0.1.0 2608_bd-diagnostic.fxp は完全 retain 上書きなし) (4) docs/design/rhythm-patches/synth/PARAMETER_SENSITIVITY_AND_ANALYSIS_DESIGN.md § 21 新規 8 sub-section = 21.1 v2 artifact identity (= sha256 literal) + 21.2 root cause 分析 (= Failure A 3 件複合 dependency literal + Failure B sustain=0 dependency literal、 各 verify = scratch sweep path literal) + 21.3 schema 分離 (= conversions vs structural_dependencies + parameter_kind 明示) + 21.4 6 軸 sweep gate 結果 (= 5 gate 全 PASS) + 21.5 軸別 active/silent 判定 (= π15.7/π15.8/π15.9 比較 table + active count 4→4→6 / silent 2→2→0) + 21.6 重要 finding 6 件 + 21.7 scope-out 9 件 (= envmod range / sustain range expansion sweep は π15.10 候補) + 21.8 commit value 越川氏 wording literal「failure 原因を structural dependency として反映した v2 diagnostic baseline」 (5) docs/design/rhythm-patches/synth/USAGE.md に "v0.2.0 baseline = structural dependency expansion (= π15.9)" subsection 追記 = root cause 2 件 wording literal + setup 4 件 literal + invoke command literal + v2 sha256 + result literal (= 6/6 active / 0/6 silent / v0.1.0 retain) (6) ADR-0033 frontmatter line 3 chain π15.9 entry literal 追加、 必ず記録する値 literal 8 件 = v2 .fxp sha256 c03d32284d5d9108da905bcce6674b09a5912845cb5b46f0262ab2c069013517 / delta=0 baseline render sha256 28442a6ed106fa2cfcbe2b5b8eb008244faeea092c0f55d80823f7de17858114 / active count 6/6 active 0/6 silent / silent 解消 = a_env1_release + a_env2_decay の 2 軸 / conversion formula 不変維持 / structural dependency setup 4 件追加 / aesthetic-rejected literal / 越川氏 audition 対象外、 重要 finding = effect 軸が baseline 構造で移動する (= a_env1_decay の effect が v1 tail 軸 → v2 attack 軸へ移行、 silent 化ではなく effect representation の literal 変化、 sustain=0.5 で sustain phase 形成され decay phase 短縮の副次効果)、 scope 制約 7 件遵守 = optimizer 再開なし / preference-learning 再開なし / accept-reject 判定なし / best 選別なし / 既存 v0.1.0 上書きなし (= 並走 retain) / aesthetic candidate 作成なし / envmod-sustain range expansion sweep なし、 generated v2 .fxp は repo 投入 (= 越川氏 directive literal で active diagnostic baseline retention) + sweep output (= /private/tmp/pmdneo-v2-* / pmdneo-scratch-*) は repo 未投入維持、 driver / fixture / verify script / runtime semantics 軸 ADR-0026-0032 完全不変、 本 commit = 新規 2 件 (= v2 patch-spec yaml + v2 .fxp) + 修正 4 件 (= conversion yaml v0.2.0 update + PARAMETER_SENSITIVITY § 21 + USAGE v2 subsection + ADR π15.9 entry) の 6 件 only) + 23rd session 第 38 commit π15.10 = chain 切り替え BD authoring 方針 doc 化 (= 越川氏 directive literal、 sensitivity 解析の深掘り (= π15.7/π15.8/π15.9 chain) を切り上げ、 v2 diagnostic baseline + sensitivity findings を起点に BD authoring 軸に戻す literal 規律、 本 commit は doc 化のみ = 実装 / aesthetic candidate 作成は含まない)、 2 件修正 = (1) docs/design/rhythm-patches/synth/BD_AUTHORING_PLAN.md 新規 = 8 section literal record = 0 本 doc の位置付け + 役割分担 + scope-out 8 件 / 1 authoring baseline = v2 diagnostic baseline (= sha256 c03d3228... + 6/6 active + sensitivity findings 直接適用可能 + baseline 不可触原則) / 2 target feature snapshot = authoring anchor (= 2608_BD-roundtrip feature_snapshot literal 引用 = sub 0.800 / body_freq 45.166 Hz / attack 9.351 ms / decay 1.838 ms / tail 0 / noisiness 0.000435、 target 利用方針 = §決定 14 reference-inspired (OK) / derivative (NG) 境界遵守、 sample byte 直接利用禁止、 feature diff は完成 gate ではない literal) / 3 workflow protocol = 1 parameter change → render → analyze-drum → feature diff → decision (= trial 最小単位 literal、 multi-parameter trial 禁止、 trial output retention table = repo 投入 ✗ / `/private/tmp` ephemeral / decision log は越川氏判断軸 scope-out、 deterministic 保証 = SURGE_RNG_SEED=2608 env + delta=0 self-consistency) / 4 phase 区分 = phase 1 target vs v2 baseline literal diff (= read-only、 v2 render + analyze-drum + diff 計算、 output ephemeral) + phase 2 targeted parameter trials (= 1 trial 1 ループ 8 step literal、 iteration 終了条件 3 件、 越川氏判断軸でない literal record gate、 scope-out aesthetic candidate label 化 + 越川氏 audition + best 選別 + multi-parameter trial + optimizer / preference-learning) + phase 3 aesthetic candidate 作成 (= 本方針 scope-out、 越川氏 別 directive / 別 commit 起点) / 5 authoring 優先軸 = target literal gap table (= body_freq 45 Hz / sub band 0.800 / attack 9 ms / decay 2 ms / tail 0 / noisiness 0、 各 feature の主な lever sensitivity findings 由来 = a_osc1_octave / a_osc1_pitch / a_env1_attack / a_env1_decay / a_env1_release / a_level_noise、 authoring 指針 only 完成 gate ではない) / 6 完成 gate / acceptance gate の literal 規律 = feature diff = advisory / 越川氏 audition = authoritative、 両者別軸混同しない / wording 規律 「target に近づいた」 ✗ / 「current feature diff = X」 ○ / 「ranking best」 ✗ / 「relative feature distance smaller」 ○、 metric-pass-is-not-aesthetic-pass / preference-learning-beats-metric-correlation 規律維持 / 7 関連 ADR / doc literal 引用 (= §決定 14 + §決定 25 + §決定 27 (12) + PARAMETER_SENSITIVITY § 15-21 + USAGE) / 8 本方針の commit value 越川氏 wording literal 「sensitivity 解析の深掘りを切り上げ v2 diagnostic baseline + sensitivity findings を起点に BD authoring 軸に戻す literal 規律」、 次の作業者が「解析を深掘りし続ける」 のではなく「authoring 方針に戻る」 ことを明確化、 phase 1/2/3 への着手は越川氏 別 directive 待ち / (2) ADR-0033 frontmatter line 3 chain π15.10 entry literal 追加、 scope 制約 8 件遵守 = 実装しない / aesthetic candidate 作らない / optimizer 再開しない / preference-learning 再開しない / accept-reject 判定しない / 既存 diagnostic v0.1.0 / v0.2.0 不変 / 既存 2608_bd.fxp / patch-spec 不変 / driver semantics 不変、 repo 投入 = doc 1 件新規 + ADR 1 件修正 only、 repo 投入なし = new fxp / generated wav / sensitivity table output / analysis artifacts、 driver / fixture / verify script / runtime semantics 軸 ADR-0026-0032 完全不変、 本 commit = 新規 1 件 (= BD_AUTHORING_PLAN.md) + 修正 1 件 (= ADR π15.10 entry) の 2 件 only) + 23rd session 第 39 commit π15.11 = transient lever 不在 + v3 baseline 設計判断 design note (= Phase 2 + Phase 2.5 isolated trial finding 整理、 transient_strength gap (-1.4) を埋める clean lever が v2 baseline に存在しない literal evidence 固定、 v3 で waveshaper / noise routing / OSC1 param を active 化することは unit conversion / structural dependency 延長ではなく synth architecture 選択に踏み込むため、 実装前に設計 note で「どこから先が音作りか」 の境界を literal 固定、 本 commit は doc 化のみ = 実装 / v3 .fxp 生成 / sweep 実行 / aesthetic candidate 作成は scope-out)、 2 件修正 = (1) docs/design/rhythm-patches/synth/BD_TRANSIENT_LEVER_DESIGN.md 新規 7 section literal record = 0 本 doc の位置付け (= 起点 Phase 2.5 認知 + 設計判断必要理由 = synth architecture 選択軸 + scope literal 7 件) / 1 Phase 2.5 summary (= A 系 transient lever 候補 3 軸 = a_ws_drive silent + a_level_noise silent + a_filter1_resonance partial + 1 軸のみ band trade-off、 B 系 band 補正 lever 候補 3 軸 = a_osc1_pitch weak microscale + a_filter1_cutoff microscale silent + a_lowcut not usable for band、 baseline SHA self-consistency OK 28442a6e..., 主要 finding 3 件 = transient gap clean lever 不在 / band 補正 lever 不在 or microscale / a_filter1_resonance +1 で band 軸破壊的 trade-off) / 2 root cause 分類 (= literal 3 category 分離 = unit conversion 問題ではないもの (a_filter1_resonance は formula 問題ではなく physical 性質) + structural routing 問題 (a_ws_drive / a_level_noise は routing 設定 missing、 v3 で解消可能) + synth architecture 選択問題 (waveshaper 有効化 = harmonic distortion 設計、 noise 有効化 = noise 加算設計、 OSC1 param = source 形状設計、 ここから先は diagnostic ではなく authoring decision choice)、 3 カテゴリ関係 figure literal = unit conversion v0.1.0 解消済 + structural routing v0.2.0 4 件解消済 + waveshaper/noise routing 未解消 / synth architecture 問題は音作り境界) / 3 v3 baseline 選択肢 4 件 = A waveshaper routing active 化 + B noise routing active 化 + C OSC1 internal parameters sweep + D v3 を作らず現 lever map で authoring candidate 方針へ進む / 4 各選択肢 risk = 共通 risk (= diagnostic baseline が sound design baseline に寄りすぎる) + option A risk (= waveshaper = harmonic distortion 加える設計判断、 BD identity 変動 + type 選択肢で探索空間増大) + option B risk literal (= target noisiness_ratio 0.000435 ほぼ完全 pure tone、 noise routing 有効化は target から離れる、 越川氏 directive literal「noise 増やす方向は危険」) + option C risk (= 7 軸増加で探索空間急増 + a_osc1_type 依存で param 意味変動) + option D risk (= transient gap を diagnostic 軸で埋められないこと literal 受容、 authoring 段階で「lever 不在」 顕在化可能性) + 同時複合 risk (= option A+B 同時導入で distortion+noise 同時、 個別 effect 判別不能、 越川氏 directive 同時導入避ける) / 5 推奨 literal 規律 = まだ aesthetic candidate 作らない + v3 作る場合の分離規律 (= 1 commit 1 structural axis 追加、 同時導入しない、 v3a/v3b/v3c 候補で個別 sensitivity sweep verify) + 越川氏 directive wording 整合 (= option B は最後の選択肢 / option A or C から優先) + option D 意義 (= 「target feature 完全 close する diagnostic baseline」 が存在しない可能性受容、 authoring 段階で越川氏 audition gate を §決定 27 (12) literal で貫徹) + 推奨 = option A waveshaper only or option D 現 lever で進む のいずれか、 option B (noise) は最後、 option C (OSC1 internal) は探索空間大、 最終判断は越川氏 directive 待ち / 6 関連 ADR / doc literal 引用 = §決定 14 / §決定 27 (12) ν 規律 / PARAMETER_SENSITIVITY § 19/20/21 / parameter-unit-conversion.yaml v0.2.0 / 2608_bd-diagnostic-v2.patch-spec.yaml / BD_AUTHORING_PLAN.md / 7 commit value 越川氏 wording literal「v3 baseline で何を active 化するかは unit conversion / structural dependency 延長ではなく synth architecture 選択になる、 ここで「どこから先が音作りか」 の線引きを設計 note で literal 固定してから進める」、 次に v3 baseline 作るとき or authoring candidate に進むときに設計判断を確認する materials として repo 固定、 本 commit は design note と ADR entry のみ、 実装は越川氏 別 directive 起点 / (2) ADR-0033 frontmatter line 3 chain π15.11 entry literal 追加、 scope 制約 8 件遵守 = 実装しない / v3 .fxp 作らない / sweep しない / aesthetic candidate 作らない / commit は design note + ADR entry のみ / optimizer / preference-learning なし / accept / reject 判定なし / 既存 v2 baseline 不変、 repo 投入 = doc 1 件新規 + ADR 1 件修正 only、 driver / fixture / verify script / runtime semantics 軸 ADR-0026-0032 完全不変、 本 commit = 新規 1 件 (= BD_TRANSIENT_LEVER_DESIGN.md) + 修正 1 件 (= ADR π15.11 entry) の 2 件 only) + 23rd session 第 40 commit π15.12 = Option D 採用 + minimal BD authoring candidate plan doc 化 (= π15.11 4 択 (A waveshaper / B noise / C OSC1 internal / D v3 不作成) の Option D 採用、 v3 diagnostic baseline 作らず v2 baseline + confirmed levers で authoring plan 3 件 literal 固定、 越川氏 directive literal「v3 で探索を広げるより少数 candidate plan に絞るほうが authoring 目的に戻れる」、 本 commit は plan doc 化のみ = 実装 / .fxp 生成 / WAV render / sweep / aesthetic candidate 作成は scope-out)、 2 件修正 = (1) docs/design/rhythm-patches/synth/BD_MINIMAL_AUTHORING_CANDIDATE_PLAN.md 新規 8 section literal record = 0 本 doc の位置付け (= 起点 π15.11 Option D 採用 + 越川氏 directive 理由 4 件 = v3 synth architecture 選択回避 / target noisiness ほぼ 0 / band balance 既に近い / transient lever 追いすぎは解析寄りすぎ + scope literal 7 件 + 前提 confirmed lever table + 認識 target との literal feature gap table) / 1 Plan A = minimal single-axis a_env1_release +3 only (= 構成 1 parameter、 expected feature direction = tail 168.6 → 0 literal match + 他 gap retain、 trade-off = 副次微小 pitch_conf +0.086、 not aesthetic candidate 理由 = target literal gap 複数残存 + 越川氏 audition gate なし + acceptance.aesthetic_acceptance: rejected のまま、 verification command literal 4 step、 artifact repo 投入判断 pending) / 2 Plan B = body pitch も合わせる a_env1_release +3 + a_osc1_octave -1 (= 構成 2 parameter、 expected feature direction 単純 sum 推定 = tail ~0 + body 43.1 + band 大幅悪化 + attack 大幅延長、 trade-off = band gap +0.013 → +0.200 / attack +1.2 → +10.2 ms、 not aesthetic 理由 = multi-axis trade-off + cumulative interaction 未検証 + 越川氏 audition gate なし + π15.10 §3.2 multi-parameter trial 禁止軸の design 段階扱い、 verification command literal、 repo 投入 pending) / 3 Plan C = band 補正仮説付き a_env1_release +3 + a_osc1_octave -1 + a_osc1_pitch +1 (= 構成 3 parameter、 expected feature direction 単純 sum 仮説 = a_osc1_pitch +1 で band shift ~55% offset + body_freq 43.1 → ~45.6 Hz target match 強化、 trade-off = band 部分補正完全ではない + cumulative interaction 未検証、 not aesthetic 理由 = 仮説段階 + interaction 未検証 + 越川氏 audition gate なし、 verification command literal 3 step chain、 repo 投入 pending) / 4 3 plan 比較 table (= literal feature 推定 + target match per axis + band integrity per plan + overall risk = A low / B mid / C mid) / 5 plan 選定の判断軸 = 越川氏 directive 形成支援 5 問 + 私の推奨 (= Plan A verify execute 推奨、 B/C は A 結果見てから判断、 artifact 投入は越川氏個別 directive 維持) / 6 scope-out 9 件 literal / 7 関連 ADR / doc literal 引用 / 8 commit value 越川氏 wording literal「v3 architecture 選択回避し v2 baseline + confirmed levers で BD authoring candidate plan 3 件 literal 固定、 音を作る直前の計画として探索を広げず少数 candidate に絞ることで authoring 目的に戻る」 / (2) ADR-0033 frontmatter line 3 chain π15.12 entry literal 追加、 scope 制約 9 件遵守 = 実装しない / .fxp 生成しない / WAV render しない / 追加 sweep しない / aesthetic candidate 作らない / optimizer / preference-learning なし / accept / reject 判定なし / human audition まだ行わない / artifact repo 投入は越川氏別 directive 起点、 repo 投入 = doc 1 件新規 + ADR 1 件修正 only、 driver / fixture / verify script / runtime semantics 軸 ADR-0026-0032 完全不変、 本 commit = 新規 1 件 (= BD_MINIMAL_AUTHORING_CANDIDATE_PLAN.md) + 修正 1 件 (= ADR π15.12 entry) の 2 件 only) + 23rd session 第 41 commit π15.13 = Plan A/B/C scratch verify finding doc 化 + Plan A leading 認定 (= π15.12 で literal 固定した 3 plan を v2 baseline から scratch verify、 越川氏 directive literal 「Plan A を leading として記録、 Plan B/C は trade-off 大で promote しない、 Plan C band compensation 仮説 failed として記録、 cumulative interaction 非線形性 + band saturation を literal evidence 残す」、 既存 BD_MINIMAL_AUTHORING_CANDIDATE_PLAN.md に § 9 追記、 wording 規律 = "leading verification plan" のみ literal / "best" / "accepted" 禁止、 本 commit は doc 化のみ = generated artifact / aesthetic candidate 昇格 scope-out)、 2 件修正 = (1) docs/design/rhythm-patches/synth/BD_MINIMAL_AUTHORING_CANDIDATE_PLAN.md に § 9 追記 7 sub-section literal = 9.1 verify identity (= wav sha256 + classify、 Plan A は Phase 2 trial 1 と完全一致 deterministic ✓) / 9.2 feature literal comparison table (= target vs v2 base vs Plan A/B/C、 tail 全 plan 0.0 / body_freq Plan B/C 43.1 で target match / band sub Plan B/C 0.9995 で大乖離) / 9.3 per-plan summary (= Plan A clean isolated literal + Plan B band broken literal + Plan C band compensation failed literal、 Plan C は Plan B と band 完全同一 / 単独 effect cumulative で消失 / micro 差分 attack-0.93 ms など部分残存) / 9.4 important finding 4 件 = (1) cumulative parameter interaction is nonlinear (2) single-axis sensitivity cannot be linearly summed (3) octave -1 causes band saturation (4) pitch +1 cannot recover low band after saturation、 期待値 vs 実際値 literal 比較で線形累積仮定の破綻 evidence + saturation 軸 identify が future cumulative authoring で必要 / 9.5 Plan ranking table (= Plan A leading verification plan / Plan B not promoted band broken / Plan C not promoted compensation hypothesis failed、 重要 wording 規律 literal = "leading verification plan" ○ / "best plan" ✗ / "accepted plan" ✗、 越川氏 audition gate / aesthetic acceptance gate 通過前 wording 禁止) / 9.6 next decision (= Plan A 昇格は越川氏別 directive 起点、 no artifact committed / human audition final gate / 昇格時の選択肢 = new patch-spec.yaml + acceptance.aesthetic_acceptance: pending) / 9.7 scope-out 7 件 literal (= aesthetic candidate 昇格しない / generated artifact repo 投入なし / accept-reject 判定なし / best 選別なし / optimizer-preference learning なし / 別 plan combination 探索なし / human audition まだ行わない) / (2) ADR-0033 frontmatter line 3 chain π15.13 entry literal 追加、 scope 制約 8 件遵守 = doc only / no generated fxp / no rendered wav / no analysis artifact / no commit of /private/tmp output / no aesthetic candidate / no accept-reject / no optimizer-preference learning、 repo 投入 = doc 1 件追記 + ADR 1 件修正 only、 driver / fixture / verify script / runtime semantics 軸 ADR-0026-0032 完全不変、 本 commit = 修正 2 件 (= BD_MINIMAL_AUTHORING_CANDIDATE_PLAN.md § 9 追記 + ADR π15.13 entry) only) + 23rd session 第 42 commit π15.14 = Plan A scratch audition failure literal record (= doc-only commit、 Plan A 4 artifact 生成後越川氏 audition で「音として認識できない」 と reject、 root cause = v2 baseline / Plan A peak_dbfs -25 dBFS で target -3 dBFS から 22 dB 不足、 4 artifact は repo 投入せず /private/tmp/pmdneo-plan-a-rejected/ に退避 = ephemeral retain、 越川氏 directive literal「normalize に逃げない / .fxp 側の gain / mixer 構造として解く / Plan A is not promoted / pre-audition engineering gate is not aesthetic acceptance」、 本 commit は doc-only = generated artifact / sweep / aesthetic candidate / ADPCM-A encode 全て scope-out)、 3 件修正 = (1) docs/design/rhythm-patches/synth/BD_MINIMAL_AUTHORING_CANDIDATE_PLAN.md § 10 追記 6 sub-section literal record = 10.1 audition finding (= Plan A scratch artifact generated but not committed + 越川氏 audition「音として認識できない」 + root cause level too low + target peak -3 dBFS / v2 base + Plan A peak -25 dBFS / gap 22 dB + tail feature 良好だが audible-level gate failed + feature similarity ≠ audition-ready level + diagnostic baseline did not guarantee audible amplitude) / 10.2 level metrics literal table (= peak / rms / clipping、 v2 baseline 既に -25 dBFS、 Plan A 継承) / 10.3 rejected artifact retention table (= 4 file の /private/tmp/pmdneo-plan-a-rejected/ 退避 path literal) / 10.4 important wording 5 件 literal (= Plan A is not promoted / scratch audition failed due to inaudible level / next task is audible-level structural dependency investigation / pre-audition engineering gate not aesthetic acceptance / normalize による解決はしない = .fxp 側 gain / mixer 構造で解く) / 10.5 root cause classification table (= 新カテゴリ「audible-level structural problem」 を unit conversion / structural routing / synth architecture と並ぶ第 3 カテゴリとして literal 追加、 sensitivity 軸 ≠ audible level 軸 / 6/6 active 化が audition 可能性を意味しない literal proof) / 10.6 next step (= audible-level structural dependency 調査、 a_volume / a_level_o1 / mixer / scene gain 候補、 isolated gain sweep on v2 baseline、 target peak -6 〜 -3 dBFS、 no normalize、 clipping == 0 維持、 /private/tmp only、 本 commit scope-out) / (2) docs/design/rhythm-patches/synth/BD_AUTHORING_PLAN.md に § 6.4 audible-level pre-audition gate 追記 3 sub-section = 6.4.1 audible-level gate と越川氏 audition gate の別軸 literal (= engineering pre-condition vs aesthetic acceptance、 audible-level 通過しないと audition gate 到達不可) + 6.4.2 normalize による解決 禁止 literal (= render 後処理ではなく .fxp 側 gain/mixer 構造として解く、 candidate parameter a_volume / a_level_o1 / mixer / scene gain、 isolated gain sweep + literal effect 観察、 v2.1 audible-level extension 候補 or 別 baseline chain) + 6.4.3 phase 3 entry 条件 update = 6 gate 化 (= 1 feature gap advisory + 2 sensitivity findings 蓄積 + 3 authoring candidate plan literal + 4 ★ audible-level engineering gate clear ★ 新規 + 5 scratch candidate generation + 6 越川氏 audition gate)、 gate 4 clear なしで gate 5/6 進むと audition 機械的 reject = π15.14 literal experienced / (3) ADR-0033 frontmatter line 3 chain π15.14 entry literal 追加、 scope 制約 7 件遵守 = doc-only / generated artifact なし / commit /private/tmp なし / aesthetic candidate なし / ADPCM-A encode なし / optimizer-preference learning なし / accept-reject 判定なし、 driver / fixture / verify script / runtime semantics 軸 ADR-0026-0032 完全不変、 本 commit = 修正 3 件 (= BD_MINIMAL_AUTHORING_CANDIDATE_PLAN.md § 10 追記 + BD_AUTHORING_PLAN.md § 6.4 追記 + ADR π15.14 entry) only)、 段階 1-3 完了後に Accepted 移行予定、 注: step 18 = ADR-0032 simultaneous trigger semantics proof と並走、 step 番号は段階 1 着手時に再採番予定、 ADR-0033 自体は policy fixation で step 軸とは独立)
- 起票日: 2026-05-15
...
**23rd session π9 補強 = 「reference-inspired vs derivative」 境界 5 軸 table** (= π7/π8 investigation 由来、 future AI-assisted workflow + feature-guided synthesis 等の judgement basis 化):

| 軸 | reference-inspired (= OK、 §決定 14 sound-alike caution 範囲内) | derivative (= NG、 §決定 14 / §決定 1 / §決定 2 違反) |
|----|----------------------------------------------------------------|------------------------------------------------------|
| feature 経由 | spectral characteristic を guide として使う (= 情報読取 only) | wav の sample byte をそのまま再生 (= byte copy) |
| 数値関係 | distance score < threshold で convergence (= 「近接」 で確定) | byte-level identical (= 「同一」 で確定) |
| 結果 wav | new synthetic = 越川氏 設計 spec + AI 探索 + 越川氏 accept | 元 wav の degraded copy (= compression / decimation 経由でも derivative) |
| narrative | 「こういう感じの音を目指す」 (= aesthetic reference) | 「同じ音を再現する」 (= exact reproduction、 §決定 24 禁止 wording 該当) |
| ADR-0033 整合 | §決定 14 + §決定 1 + §決定 2 + §決定 24 全部整合 | §決定 14 + §決定 1 + §決定 2 違反、 §決定 24 「exact reproduction」 該当 |

**判断規律**: 5 軸 **全部 reference-inspired** なら OK (= ADR-0033 範囲内)、 **1 軸でも derivative** なら NG (= 設計見直し必要)。 feature-guided synthesis (= AI/toolchain で target wav の feature を読取り → parameter search) は 5 軸全部 reference-inspired 側、 sample-based wavetable creation (= 既存録音 byte を直接 wavetable 化) は 「結果 wav」 「narrative」 軸で derivative 側に寄る = 採用時 要設計検討。

**適用例**:
- Surge XT 同梱 Kick 909ish.fxp を **feature reference** として 越川氏 BD design に近づける parameter search = ✓ reference-inspired (= §決定 27 π8 採用 architecture)
- 70s/80s drum 録音 を STFT → wavetable 化 + そのまま Surge XT に load = △ derivative risk (= sample byte 再利用、 narrative 「同じ音」 軸該当)
- IFFT で 設計 harmonic content から synthetic wavetable 構築 = ✓ reference-inspired (= 数学合成のみ、 既存録音不使用)
- Yamaha mask ROM dump を feature 抽出 + parameter search = ✗ derivative + §決定 2 違反 (= 永久 scope-out 維持)
...
### §決定 27: AI-assisted patch generation + factory CC0 template acquisition + `.fxp` bridge + rendered-audio self-analysis + target-feature matching optimization workflow = AI 設計補助 + AI/toolchain template acquisition + AI/toolchain bridge + AI engineering 検査 + feature-guided parameter search + 越川氏 aesthetic final acceptance 軸独立 (= source-of-truth = `patch-spec.yaml`、 factory CC0 template acquisition + XML element name allowlist patching + feature-guided parameter search via librosa + CMA-ES/scipy.optimize = scope-in、 arbitrary `.fxp` binary full generation + full binary RE + FFT direct inversion + wavetable from existing sample byte derivative = scope-out、 23rd session λ 起票 / μ 初回適用 / ν 全面再定義 / ξ AI/toolchain bridge 軸統合 / π AI/toolchain template acquisition 軸統合 + 越川氏 hand-on engineering 完全ゼロ化 / π9 target-feature matching optimization 軸統合 + 「feature-guided, not exact inversion」 wording 確立)

§決定 1 (= 100% 著作物のみ採用) / §決定 14 (= sound-alike caution) / §決定 16 (= prototype 粒度 + wording 規律) / §決定 19 (= 段階 1 集中) / §決定 24 (= aesthetic) / §決定 25 (= fxp2wav-surge external producer) / §決定 26 (= `.fxp` authoring spec) で patch source-of-truth の中身規約まで確定、 23rd session λ で AI-assisted patch generation workflow を起票、 μ で BD candidate を 1 件生成、 ν で全面再定義 (= user acceptance downstream 配置 + AI self-analysis 軸追加)、 ξ で AI/toolchain bridge 軸統合 + 越川氏 template 1 度作成のみに限定。 その結果 23rd session π で **「ξ で残った『越川氏 template 作成 1 度』 も engineering correctness 業務、 ν 本質を 完全 貫徹するなら AI/toolchain 軸に置くべき」 finding** 確認 = template は sound design asset ではなく **neutral parameter carrier**、 Surge XT factory `Templates/Init Sine.fxp` は CC0 license で取得可能 = AI/toolchain acquisition で代替可能。 23rd session π で **AI/toolchain による factory CC0 template acquisition + XML element name allowlist patching を scope-in 化** + **越川氏 hand-on engineering 接点を完全ゼロ化** で再配置:
...
- **ν 中間 (= 23rd session ν):** AI candidate spec → 越川氏 hand-on → wav render → AI self-analysis → ... → 越川氏 final accept (= **user acceptance を downstream に再配置**、 但し step 2 で越川氏 hand-on 残存)
- **ξ 中間 (= 23rd session ξ):** AI candidate spec → **AI/toolchain template-based bridge** → render → AI self-analysis → ... → 越川氏 final accept (= 越川氏 engineering 接点 = template 作成 1 度のみに縮小)
- **新 (= 23rd session π 採用):** AI candidate spec → **AI/toolchain factory CC0 template acquisition + XML element name allowlist patching** → render → AI self-analysis → ... → 越川氏 final accept (= **越川氏 hand-on engineering 接点を完全ゼロ化**、 越川氏 role = rendered audio aesthetic audition / accept のみ)
...
**11 軸 literal 拘束 (= λ 8 軸 → ν 10 軸 → ξ 11 軸 → π 11 軸 + AI 役割 5 軸):**

(1) **AI 役割境界 = 5 軸に拡張 (= 設計補助 + template acquisition + bridge + render + 検査):**
- AI 役割 1 = patch spec 設計補助 (= λ 起票分、 維持)
- AI 役割 2 = **factory CC0 template acquisition** (= π 追加、 Surge XT factory `Templates/Init Sine.fxp` 等 CC0 patch を byte-identical 複製 + provenance literal 記録、 「越川氏 hand-on engineering 完全ゼロ化」 軸)
- AI 役割 3 = **patch-spec → `.fxp` allowlist patching bridge** (= ξ 起票 / π refine、 XML element name allowlist patching、 toolchain implementation、 「bridge, not full synthesizer patch compiler」、 **π7 で unit conversion layer 必須化 = patch-spec の human-readable values (= ms / Hz / dB / enum 等) を Surge XT 内部 scale (= log-time / log-freq / normalized / integer 等) に変換してから XML element value に注入する規律確立、 passthrough 注入は engineering FAIL の root cause で禁止**)
- AI 役割 4 = **wav render orchestration** (= ξ 追加、 fxp2wav-surge external producer invoke、 toolchain integration、 1-command 化 goal)
- AI 役割 5 = **rendered audio self-analysis** (= ν 追加、 engineering / signal analysis 軸、 **π9 拡張 = feature-guided parameter search の optimization loss としても再利用、 AI 役割 5 が post-render check + search loss feedback の 2-role hybrid 化**、 詳細 = (12) target-feature matching optimization axis literal 参照)
- AI は **arbitrary `.fxp` binary full generation を行わない** (= ξ 制約 維持、 factory acquisition + XML element name patching のみ scope-in、 「bridge, not full synthesizer patch compiler」)
- AI は **full `.fxp` binary format reverse engineering を行わない** (= ξ 制約 維持、 必要最小限のみ許容、 chunk container 軸 + XML body 軸の 2 層で解析十分、 「checksum / chunk format の無制限解析」 は scope-out)
...
(6) **acceptance workflow (= ν 全面再定義 11 step + ξ step 2/3 役割再配置 + π step 2 内 template acquisition 完全 AI 化):**
1. AI が drum role + design intent (= 越川氏指定) を受け取り、 `patch-spec.yaml` candidate 生成
2. **AI/toolchain による (a) factory CC0 template acquisition (= 1 度のみ実施、 全 6 drum 共有) + (b) patch-spec → `.fxp` XML element name allowlist patching** (= 「engineering candidate before aesthetic judgement」、 §決定 26 spec 整合 verify、 §決定 27 (9) π artifact `template.fxp` / `parameter-allowlist.yaml` / `template.fxp.provenance.yaml` 経由)
...
10. **越川氏 audition / edit / accept (= 最終 gate、 aesthetic / musical judgement、 rendered audio のみが review 対象、 唯一の hand-on 接点)**
11. accept 後 = `aesthetic_acceptance: accepted` + signature + ι commit (= 並行配置、 既存 fixture 非破壊)

注意: π で step 2 内の template acquisition も AI/toolchain 軸に置換、 越川氏 engineering 接点 = **完全ゼロ化**。 全 6 drum 種共通 template (= `2608_template.fxp`) は AI/toolchain が Surge XT factory `Templates/Init Sine.fxp` (= CC0 license) を byte-identical 複製で acquisition + `template.fxp.provenance.yaml` literal 記録、 越川氏 hand-on **不要**。 drum-specific patch は AI/toolchain XML element name allowlist patching で生成、 越川氏 review 対象は **rendered audio のみ** = aesthetic judgement 純化、 「越川氏 認知負荷を engineering correctness から完全解放」 軸の完全貫徹。
...
(9) **成果物 path canonical (= 6 → 9 件に拡張、 ξ bridge artifacts 3 件 + π で template ownership 変更):**
- `docs/design/rhythm-patches/synth/2608_bd.patch-spec.yaml` (= λ μ 起票分、 維持)
- `assets/drum_samples/synth/templates/2608_template.fxp` (= **ξ 起票 / π acquisition**、 全 6 drum 種共通 neutral carrier、 Surge XT factory `Templates/Init Sine.fxp` (= CC0 license) の byte-identical 複製、 AI/toolchain 取得 = **越川氏 hand-on 不要**)
- `docs/design/rhythm-patches/synth/parameter-allowlist.yaml` (= **ξ 新規 / π refine**、 template `.fxp` 内 XML element name allowlist (= byte offset ではなく XML element tag 軸)、 AI bridge が touch 可能な parameter element を literal 制限、 「parameter allowlist patching」 軸)
- `docs/design/rhythm-patches/synth/template.fxp.provenance.yaml` (= **ξ 新規 / π acquisition fill**、 template 由来 + CC0 license narrative + sha256 + Surge XT 1.3.4 install + chunk 内部構造 (= sub3 magic + 28 byte binary header + XML body + 8 byte trailing) literal、 reproducibility 確保)
- `assets/drum_samples/synth/patches/2608_bd.fxp` (= 維持、 ξ/π で AI/toolchain bridge 出力に役割変更、 candidate 扱い)
...
(11) **ξ/π scope-in literal = AI/toolchain bridge + template acquisition 軸 (= ξ 起票 / π 拡張):**
- `patch-spec.yaml` → template `.fxp` allowlist patching bridge (= §決定 27 (1) AI 役割 3 軸、 toolchain implementation、 「bridge, not full synthesizer patch compiler」)
- template `.fxp` の provenance / hash / license 記録 (= §決定 27 (9) artifact `template.fxp.provenance.yaml` 経由、 sha256 + Surge XT version + acquisition method + CC0 narrative、 reproducibility + license trail 確保)
- patching 対象 parameter の allowlist (= §決定 27 (9) artifact `parameter-allowlist.yaml` 経由、 「parameter allowlist patching」 軸の literal 制限、 π で XML element name 軸対応 = byte offset ではなく XML element tag 経由 patching)
- generated `.fxp` は candidate (= §決定 27 (1) AI 役割 3 軸出力、 越川氏 aesthetic final gate 経由で asset 化、 「engineering candidate before aesthetic judgement」)
- `.fxp` → wav render → AI self-analysis → user final gate (= §決定 27 (6) workflow step 2-10 整合、 全段 candidate 扱い、 越川氏 final gate でのみ asset 化)
- **AI/toolchain factory CC0 template acquisition (= π 新規)**: Surge XT factory `Templates/Init Sine.fxp` 等 CC0 license patch を byte-identical 複製で取得 + provenance literal 記録 (= 越川氏 hand-on engineering ゼロ化、 「越川氏 認知負荷を engineering correctness から完全解放」 軸貫徹)
- **XML element name allowlist patching (= π 新規 refine)**: template `.fxp` 内部 chunk = sub3 magic + binary header + XML body + trailing binary の構造を確認後、 XML body 内 parameter element の value attribute 修正経由で patching、 byte offset 不要 (= safer + cleaner、 「bridge, not full synthesizer patch compiler」 軸維持)

(12) **target-feature matching optimization axis (= π8/π9 新規追加、 architectural pivot 採用):**

π7 越川氏 audition で **patch-spec passthrough の semantic mismatch** が aesthetic gate で露呈 = engineering FAIL を audition 前に detect すべきだった ν 規律違反反省点を踏まえ、 π8 investigation (= `docs/design/rhythm-patches/synth/FEATURE_GUIDED_SYNTHESIS_INVESTIGATION.md`) で **feature-guided parameter search** 軸を §決定 27 に統合採用 (= Option A++):

- **target_wav reference** (= aesthetic reference として使用、 例 Surge XT factory `Kick 909ish.wav` / public domain 録音 / 越川氏 自作 reference) → librosa 経由 **feature extraction** (= spectral centroid / flux / RMS / attack ms / decay curve / pitch tracking 等)
- **reference patch baseline** (= 例 Kick 909ish.fxp 56 element 値、 `SURGE_XT_FXP_BRIDGE_INVESTIGATION.md § 15.4` 実測 reference) を **continuous parameter search の starting point** として使用
- **continuous parameter search loop** = CMA-ES / scipy.optimize で 6-10 continuous parameter のみ search (= `a_env1_attack/decay/release` / `a_env2_decay` / `a_filter1_cutoff` / `a_lowcut` / `a_level_noise` / `a_ws_drive` / `a_osc1_pitch/octave` 等)、 **enum / routing 系は baseline 固定** (= `a_osc1_type` / `a_filter1_type` / `a_fb_config` / `a_ws_type` / `a_mute_noise` 等は search 対象外、 Kick 909ish 値で固定)
- 各 iteration = (a) propose params → (b) bridge invoke (= §決定 27 (1) AI 役割 3 軸) → (c) fxp2wav-surge render (= §決定 25 external producer) → (d) **AI self-analysis 10 項目 = optimization loss vector として再利用** (= AI 役割 5 拡張) → (e) optimizer update
- max iteration ≈ 100、 各 iter ≈ 1-3 sec、 全体 ≈ 5-10 分想定
- converged params → final `.fxp` candidate → 越川氏 aesthetic audition (= §決定 27 (6) step 10、 final gate)

**target-feature matching optimization の中核 wording (= π9 追加 + π12 拡張):**
- 「**feature-guided parameter search, not exact inversion**」 (= 非一意性で exact inversion 不可能、 search で convergence)
- 「**target wav as aesthetic reference, not source-of-derivative**」 (= §決定 14 reference-inspired vs derivative 5 軸 table 整合、 feature 経由情報読取 only)
- 「**engineering FAIL は audition 前段 self-analysis で detect**」 (= ν 規律 + AI 役割 5 拡張 literal 強化、 π7 NG 反省点反映)
- 「**human-readable patch-spec → unit conversion → Surge XT internal scale → bridge → render → feature distance → search update**」 の closed loop literal
- 「**LLM is not the selector; optimizer is the selector**」 (= π12 追加、 LLM 役割 = search space 設計 / weight 設計 / rule 設計 / failure 説明 / report、 Optimizer 役割 = candidate 生成 / score 最小化 / seed 固定 / trial history / best 選定、 LLM の「良さそう」 判断は patch 採用に使わない、 deterministic な数値 + threshold gate で literal 選定)
- 「**fixed seed + trial history + reproducibility**」 (= π12 追加、 全 trial history を optimization-report.yaml literal 保存、 reproducibility_command literal + sha256 input 記録で再現可能)
- 「**machine-checkable engineering gate before human audition**」 (= π12 追加、 hard gate = distance_score <= threshold + validate 8 rules critical PASS、 engineering FAIL の candidate は越川氏 audition に回さない literal 規律、 ν step 7-10 chain 整合)
- 「**optimizer is also not the final selector**」 (= π13 追加、 LLM だけでなく optimizer も final 選定権限なし、 metric pass 結果は越川氏 aesthetic gate 通過を保証しない、 π12 で 128 trials score 39.83 → 0.911 + validate engineering_pass 達成も越川氏 audition で「reference / target の BD と全然似ていない」 = aesthetic_rejected の literal evidence、 ν 「acceptance is downstream」 + ξ 「越川氏 hand-on engineering ゼロ」 + π 「越川氏 aesthetic final gate」 chain 強化)
- 「**human aesthetic gate is authoritative**」 (= π13 追加、 metric pass / score / validate result は necessary but not sufficient、 越川氏 judgement が ground truth、 「metric pass ≠ aesthetic pass」 を ADR レベル literal 固定)
- 「**metric pass ≠ aesthetic pass / metric versioning + audition correlation check 必須**」 (= π13 追加、 metric は v1/v2/v3 で iterate、 各 version で N candidate human audition + Spearman correlation > 0.7 verify を optimize 実行前 hard gate、 correlation 低い metric で optimize 実行 禁止、 「current metric decreased」 wording only - 「近づいた」「reference に類似」 等 aesthetic 含意 wording 禁止)
- 「**feature matching alone does not model human aesthetic preference**」 (= π15 追加、 π14 audition correlation check (= 1-5 score + Spearman) は metric calibration のみ = 越川氏 aesthetic preference の **学習** ではない、 π15 で preference learning へ軸転換 = pairwise comparison + reject label + logistic pairwise ranking)
- 「**metric correlation is not preference learning**」 (= π15 追加、 metric が target feature に correlate しても aesthetic preference は不明、 越川氏 何を好むかは別軸の preference model で学習)
- 「**human preference is the primary objective**」 (= π15 追加、 optimize objective = α × feature_distance + β × (1 - predicted_preference) で preference model output が primary)
- 「**feature distance is auxiliary**」 (= π15 追加、 feature distance は preference model に対する補助軸、 reference に近い ≠ 越川氏 が好む)
- 「**optimizer is not final selector; preference model is also not final selector / 越川氏 aesthetic gate remains authoritative**」 (= π15 追加、 π13 wording 拡張、 LLM + optimizer + preference model 全部 supporting layer、 越川氏 final accept のみが authoritative)
- 「**relative preference (= A is better than B) ≠ candidate acceptance (= A is acceptable)**」 (= π15.5 追加、 23rd session input interpretation failure を契機に固定、 pairwise preference label A/B/tie は relative ranking のみであり absolute acceptance gate ではない、 越川氏 が 28 pair 全部 fill しても 8 candidate 全部 reject 可能、 wording 混同は false claim を生む)
- 「**pairwise ranking is auxiliary to acceptance gate**」 (= π15.5 追加、 preference model output は relative distance のみ、 「越川氏 が好む順序」 wording は invalid、 ranking は acceptance を意味しない補助軸)
- 「**all candidates can be globally rejected even if pairwise preferences exist**」 (= π15.5 追加、 schema は global_reject_all flag を独立軸として持つ、 「pairwise を埋めた = どれか accept」 は false 推論)
- 「**3 軸独立 = pairwise relative ranking / individual absolute acceptance / global reject all**」 (= π15.5 追加、 preferences.pairs (= axis 1) と acceptance_status (= axis 2 + 3) は別軸 schema、 audition_pairwise.py + preference-learn が同 schema を gate する)
- 「**preference model is not the accepted asset selector / 越川氏 aesthetic gate is authoritative**」 (= π15.5 追加、 ν 規律「acceptance is downstream」 + π13「optimizer is not final selector」 を asset acceptance 軸まで拡張、 preference model output から asset accept を決定してはならない literal)

**wavetable route の位置付け (= π9 確定 = future / secondary option):**
- π8 investigation で wavetable route は **secondary option** として保留決定:
  - **silver bullet ではない** (= OSC scale mismatch のみ解決、 envelope / filter / drive の unit conversion 依然必要)
  - **sample byte derivative narrative risk** (= 既存録音から直接 wavetable 化は §決定 14 reference-inspired vs derivative 5 軸 table で「結果 wav」 「narrative」 軸 derivative 寄り)
  - **WaveEdit GUI 経由 = 越川氏 hand-on 復活 risk** (= π 軸転換 narrative と緊張)
  - **engineering complexity 高** (= wt-tool.py / WaveEdit / wavetable validation 統合)
- 採用条件 = Option A++ で aesthetic accept 届かない場合に再検討、 採用時は **IFFT-based synthetic** (= 設計 harmonic で構築、 既存録音不使用) 軸限定
- §決定 27 (12) では **primary path = parameter search、 secondary = wavetable (IFFT-based のみ)** で literal 確定、 wavetable from existing sample byte は scope-out (= 下記 scope-out (13) 連動)

**§決定 14 sound-alike caution 補強 統合**:
- (12) feature-guided search で使用する target wav は **5 軸 table 全部 reference-inspired** であること必須 (= §決定 14 補強 table 参照)
- AI bridge は target wav の **sample byte を再生用に保存しない / wavetable 化しない / final asset に含めない** (= reference-inspired のみ、 derivative 化を防ぐ literal 規律)
...
**scope-out (= §決定 27 で confirm しない、 future 軸 13 件、 ξ refinement + π9 追加):**
...
- **patch-spec value の Surge XT 内部 scale 不変換 passthrough** (= π7 越川氏 2026-05-17 audition で実証された engineering FAIL root cause、 human-readable values (= ms / Hz / dB / enum 等) を Surge XT 内部 scale (= log-time / log-freq / normalized / integer enum 等) に変換せず直接 XML element value 注入する pattern は **禁止**、 unit conversion layer 経由 必須、 例 `decay_ms 280` → `log2(0.280) = -1.84` 変換後 Surge XT `a_env1_decay` element に注入、 Kick 909ish.fxp reference (= `SURGE_XT_FXP_BRIDGE_INVESTIGATION.md § 15.4`) 観測値が conversion table の literal 根拠、 「human-readable values must be converted to Surge XT internal scale」 wording literal、 unit conversion layer 実装は π10 以降の bridge layer 拡張で fill 予定)
- **FFT / STFT 等による Surge XT parameter 直接逆算 (= exact inversion) は禁止** (= π8/π9 新規 scope-out、 inverse synthesis は非一意 = 同 rendered audio を産む parameter set 無数 = underdetermined、 加えて aesthetic は数値的 encode 不可で exact inversion 軸採用は §決定 14 sound-alike caution / §決定 24 「exact reproduction」 禁止 wording に直接抵触、 「acceptance は downstream」 規律も bypass する risk、 採用 path = (12) target-feature matching optimization = **feature-guided parameter search** (= 非一意性受容 + convergence で確定 + 越川氏 aesthetic gate 必須)、 「feature-guided parameter search, not exact inversion」 中核 wording literal)
- **wavetable from existing sample byte derivative (= 既存録音 byte を直接 wavetable 化して Surge XT に load)** (= π8/π9 新規 scope-out、 §決定 14 reference-inspired vs derivative 5 軸 table の 「結果 wav」 「narrative」 軸 derivative 寄り、 §決定 1 100% 著作物 narrative + §決定 2 ROM dump 排除 / §決定 3 出所不明 排除 / §決定 4 既存 sample pack 排除 全部抵触可能性、 採用可能 wavetable route = **IFFT-based synthetic** (= 設計 harmonic content から数学合成、 既存録音不使用) のみ secondary option として future 検討可、 wavetable from existing sample byte 経路は 永久 scope-out 維持、 「IFFT-based synthetic wavetable only, no sample byte derivative」 中核 wording literal)

§決定 1 (= 100% 著作物のみ採用) / §決定 14 (= sound-alike caution、 π9 補強で reference-inspired vs derivative 5 軸 table 統合) / §決定 16 (= prototype 粒度 + wording 規律) / §決定 19 (= 段階 1 集中) / §決定 24 (= aesthetic) / §決定 25 (= fxp2wav-surge external producer) / §決定 26 (= `.fxp` authoring spec) と整合、 §決定 27 は **AI-assisted patch generation + factory CC0 template acquisition + `.fxp` bridge + rendered-audio self-analysis + target-feature matching optimization workflow 軸** として独立追加 + ν 全面再定義 + ξ AI/toolchain bridge 軸統合 + π factory CC0 template acquisition + XML element name allowlist patching 軸統合 + π9 target-feature matching optimization 軸統合 (= λ 8 軸 → ν 10 軸 → ξ/π 11 軸 → π9 12 軸、 AI 役割 1 軸 → ν 2 軸 → ξ 4 軸 → π 5 軸 (= role 5 が π9 で post-render + search loss feedback 2-role hybrid 化)、 4 段 → 5 段 chain、 user acceptance upstream → downstream、 越川氏 hand-on engineering 軸完全ゼロ化 / aesthetic final gate のみに純化、 factory CC0 template + XML element name allowlist patching scope-in、 「越川氏 認知負荷を engineering correctness から完全解放」 軸の完全貫徹、 「feature-guided parameter search, not exact inversion」 / 「target wav as aesthetic reference, not source-of-derivative」 中核 wording literal 追加、 scope-out 11 → 13 件 = FFT direct inversion + wavetable from existing sample byte derivative 永久 scope-out 追加、 wavetable route = future / secondary option (= IFFT-based synthetic only) literal 確定)、 既存 §決定 1-26 と直交、 driver / fixture / verify script / runtime semantics 軸 ADR-0026-0032 完全不変。
...
### A-8: fxp2wav-surge external producer 採用根拠 (= 23rd session π6 後埋め完了)

§決定 25 (= 23rd session ι'/ι'' literal) + §決定 27 (1) AI 役割 4 軸 = wav render orchestration + §決定 27 (6) acceptance workflow step 3 で確定した「fxp2wav-surge = required external producer」 を 23rd session π6 で literal 化。 §Annex A-7 superctr/adpcm reference pattern 踏襲 = repo URL + commit sha + 採用条件 PASS table + license + build 手順 + caveats。

**spike repo metadata (= 23rd session 並行 external work):**

| 項目 | 値 |
|------|------|
| spike repo URL | https://github.com/koshikawa-masato/surge-spike |
| spike commit sha | `c6b957ec44d3db0d5c4e85fe3f40872893caa31d` |
| spike commit date | 2026-05-16T09:13:33Z |
| spike commit author | koshikawa-masato |
| upstream Surge | https://github.com/surge-synthesizer/surge (shallow clone, default branch as of spike date) |
| build env | macOS Apple Silicon arm64 / Apple clang 17.0.0 / CMake 4.0.1 |
| license | GPL-3.0 (= Surge XT inherited、 PMDNEO GPL-3.0 narrative 整合) |
| PMDNEO repo boundary | repo untouched (= §決定 25 ι'' literal 規律遵守、 source tree / symlink / 相対参照すべて zero) |
| spike scope | PoC 「成立判定 spike」 (= 完成品実装ではない、 最小経路で `.fxp → wav` 成立可否判定) |

**6 採用条件 PASS table (= spike CLAUDE.md literal 採用条件 vs spike SPIKE_REPORT.md 実測結果):**

| # | 採用条件 | 結果 | 根拠 |
|---|----------|------|------|
| 1 | source build (macOS Apple Silicon arm64) | ✅ | `cmake -Bbuild`: 32.6s configure / `cmake --build` 1 発成功 |
| 2 | `.fxp` load 可能 | ✅ | `SurgeSynthesizer::loadPatchByPath()` 経由で `Kick 909ish.fxp` / `TestInitSaw.fxp` を load 成功 |
| 3 | note-on / note-off 発火 | ✅ | `playNote(0, 36, 127, 0)` → render 内に明確な attack transient (samples: 0→3→15→34→48→185→505→976→1540) |
| 4 | duration 指定可能 | ✅ | `--duration-ms 800 --tail-ms 200` → 44100 samples = 1.0s exact |
| 5 | 44100 Hz mono 16-bit WAV 出力 | ✅ | `file`: `RIFF (little-endian) data, WAVE audio, Microsoft PCM, 16 bit, mono 44100 Hz` |
| 6 | 手順を残せる | ✅ | spike repo `SPIKE_REPORT.md` (= 12.4 KB) |

**deterministic 性 = 1 行 patch + `SURGE_RNG_SEED` env var (= 重要 milestone):**

upstream Surge XT の `RNGGen` は wall-clock seed なので run 間 bit-identical にならない (= 24% sample differ 観測)。 spike では `src/common/SurgeStorage.h` の `RNGGen` ctor を 1 ヶ所改修して `SURGE_RNG_SEED` env var 経由 deterministic seed 注入を有効化:

```cpp
// SurgeStorage.h line ~2010
struct RNGGen {
    static std::minstd_rand::result_type initialSeed() {
        if (const char *s = std::getenv("SURGE_RNG_SEED")) {
            try { return static_cast<std::minstd_rand::result_type>(std::stoull(s)); }
            catch (...) {}
        }
...
- ✅ `SURGE_RNG_SEED=2608` で 2 回 render → sha1 完全一致 (`a29aa45c7128735eff9fe885617beb8fda07ea43` ×2)
- ✅ `SURGE_RNG_SEED=9999` に変えると sha1 不一致 (= seed が確実に効いている)
- ⚠ env var 無しは非 deterministic (= 10,492 / 44,100 samples differ、 peak diff 8%)

upstream PR 化する場合は env var ではなく `SurgeStorage::setRandomSeed(uint32_t)` 公開 method + CLI `--seed` flag が望ましい (= spike 段階は env var で十分、 採用後 sub-sprint で昇格)。

**CLI 仕様 (= spike 段階 固定 default、 採用後 sub-sprint で flag 化):**
...
fxp2wav-surge \
  ...
  --velocity 127 \
  --duration-ms 800
# sample-rate=44100 / channels=mono / bit-depth=16 fixed default
# SURGE_RNG_SEED=<int> env var で deterministic seed
```

**新規追加 source 構成 (= spike 内、 PMDNEO 側 import なし):**

```
surge/src/fxp2wav-surge/
├── CMakeLists.txt    # 14 行: add_executable + link surge::surge-common + juce::juce_audio_basics
└── main.cpp          # 約 210 行: argparse + render loop + 自前 RIFF/WAVE writer
```

`surge/src/CMakeLists.txt` に `add_subdirectory(fxp2wav-surge)` を 1 行追加。 RNGGen 1 行 patch は optional but recommended。

**注意点 (= spike caveats 7 件、 採用後 sub-sprint で扱う):**

1. **deterministic 性は env var patch 必須** (= upstream 未統合、 build 再現性 / regression test には mandatory、 音だけ出れば optional)
2. **stereo→mono downmix = 単純 `(L+R)/2`** (= BD のような低域中心 source は問題なし、 stereo pan / chorus 強い patch では位相干渉で peak 落ちる可能性、 採用後 `--mix-mode {sum,left,rms}` 検討)
3. **CLI 拡張** (= `--sample-rate` / `--bit-depth` / `--channels` は spike 段階 fixed default のまま、 採用後 sub-sprint で flag 化)
4. **build configuration 軽量化余地** (= 現状 VST3 / AU / CLAP / standalone まで configure する、 `-DSURGE_BUILD_FX=FALSE -DSURGE_BUILD_XT=FALSE` で軽量化検討余地)
5. **error handling 強化要** (= Surge 側 `reportError` は GUI 想定で `std::cerr` 不出力、 採用後 headless error sink inject 強化要、 現状 `loadPatchByPath` の bool 戻り値のみ check)
6. **PMDNEO 2608_bd.fxp での load 未検証** (= spike では Surge factory `Kick 909ish.fxp` + `TestInitSaw.fxp` で smoke test、 PMDNEO 由来 `.fxp` (= π5 で生成済 `assets/drum_samples/synth/patches/2608_bd.fxp`) 検証は π7 actual integration test で扱う)
7. **`SurgeStorage::skipPatchLoadDataPathSentinel`** (= patch DB scan を skip しているので「他 patch dependency 含む patch (例: wavetable cross-ref)」 で load 失敗可能性、 self-contained `.fxp` は OK、 確認要)

**spike 観測値 (= 採用判断 evidence):**

| 項目 | 値 |
|------|------|
| binary size | 7.27 MB (`build/src/fxp2wav-surge/fxp2wav-surge`) |
| build wall time | configure 32.6s + build 約 5-10 分 (= surge-common + JUCE 含む) |
| `Kick 909ish.fxp` rendered WAV | 88,244 byte = 44 RIFF header + 44,100 samples × 2 byte |
| peak amplitude | 20,409 / 32,767 (= ~62% FS) |
| attack transient (first 10 samples) | 0, 3, 15, 34, 36, 48, 185, 505, 976, 1540 |
| nonzero samples | 11,239 / 44,100 (= 25%、 残り 75% decay tail で 0) |
| same-seed sha1 (seed=2608, 2 runs) | `a29aa45c7128735eff9fe885617beb8fda07ea43` ×2 (= bit-identical) |
| diff-seed sha1 (seed=9999) | 不一致 (= seed 適用確認) |
| no-seed 2 runs | 不一致 (= 10,492 / 44,100 samples differ, max abs diff 2,600, RMS 300) |

**採用結果 + 根拠の literal 反映状況:**

- **23rd session π6 = fxp2wav-surge spike evidence collection 完了 ✓** (= spike repo `c6b957e` commit + 6 採用条件 PASS + deterministic 性確立 + 観測値 literal 反映済)
- **採用判定 = ✓ PMDNEO 側採用** (= §決定 25 commitment 通り、 §決定 27 (6) workflow step 3 の external producer として fxp2wav-surge 指名)
- **PMDNEO repo boundary 完全遵守** (= spike 期間中 PMDNEO source tree / symlink / 相対参照 zero、 §決定 25 ι'' literal 規律遵守確認)
- **採用後 sub-sprint 継続項目 (= spike SPIKE_REPORT.md 末尾 5 項目):**
  1. `SurgeStorage::setRandomSeed(uint32_t)` 公開 method 化 + `--seed` flag (= env var → CLI 経由昇格)
  2. upstream PR 化検討 (= env var ベース merge され難い、 setter method 経由が望ましい)
  3. PMDNEO 正規 6 音 (= 2608_bd / SD / CYM / HH / TOM / RIM) での smoke test 拡張
  4. CLI flag 化 (= `--sample-rate` / `--channels` / `--bit-depth`)
  5. headless error sink 注入 (= Surge `reportError` capture)
- **π7 actual integration test = PMDNEO 由来 `2608_bd.fxp` (= π5 生成済) + fxp2wav-surge で `.wav` render 動作確認** (= caveat 6 解消、 別 commit で実施予定)
- **PMDNEO 内 `tools/` / `vendor/` への取り込み禁止** (= §決定 25 ι'' literal 規律維持、 spike 採用後も external require reference のみ)
