...
- 状態: **Draft** (= 2026-05-15 20th session 中盤起票、 段階 1-3 完了後に Accepted 移行予定、 注: step 18 = ADR-0032 simultaneous trigger semantics proof と並走、 step 番号は段階 1 着手時に再採番予定、 ADR-0033 自体は policy fixation で step 軸とは独立)
- 起票日: 2026-05-15
...
#### sub-sprint α: ADR-0033 起票 + current temporary fixture 起源調査 + Surge XT 環境設置

ADR-0033 Draft 起票 (= 本 commit、 §決定 14 件 + scope + roadmap + §Annex 枠 + 完了判定 10 項目)。

current temporary fixture 機械的起源調査:
- `samples.inc` 内 rhythm sample addr literal を確認、 build pipeline で生成される binary の元 source file を特定
- `vendor/PMDDotNET/` / `vendor/pmd48s/` / `vendor/ngdevkit-examples/` 配下に rhythm wav / ADPCM binary が存在するか機械的 grep
- 存在する場合は sha256 比較 (= snesmusic.org/hoot/drum_samples.zip 等の community 配布物との一致確認、 `fmopn_2608rom.h` との一致確認)
- 起源結果を §Annex A-5 に literal 記録 (= 起源 = Yamaha 由来 / 越川氏自前 / 出所不明 / license clean 第三者著作物 の 4 分類で確定)

越川氏 macOS に Surge XT install (= 無料、 GPL-3.0):
- 公式 https://surge-synthesizer.github.io/ から download
...
reference 機材設置 (= 越川氏個人 license、 PMDNEO 同梱しない):
- AREX 2011 (= free VSTi、 Windows のみ、 越川氏判断で skip 可)
- SampleScience RX-11 HD (= 無料 / 個人 reference、 越川氏判断軸)

完了判定:
- ADR-0033 file 新規作成 + 越川氏 review 反映済
- §Annex A-5 起源調査結果 literal 記録済
- Surge XT install 確認、 BD prototype 1 件 hand-on
...
ADR-0033 起票時点で PMDNEO `samples.inc` に embed されている既存 rhythm sample の起源:

**(= 段階 1 sub-sprint α で機械的調査後に literal 記載)**

調査軸:
1. `samples.inc` 内 rhythm sample 開始 addr + binary 内容を確認
2. `vendor/PMDDotNET/` / `vendor/pmd48s/` / `vendor/ngdevkit-examples/` 配下に rhythm wav / binary 存在確認
3. 存在する file の sha256 を以下と比較:
   - `snesmusic.org/hoot/drum_samples.zip` 内 `2608_*.wav` (= community 配布物検出)
   - BambooTracker `chip/mame/fmopn_2608rom.h` 内 `YM2608_ADPCM_ROM[0x2000]` (= Yamaha mask ROM dump 検出)
   - PMDDotNET / PMDWin 同梱 RSS wav (= community 慣行検出)
   - ngdevkit-examples `06-sound-adpcma/assets/*.wav` (= license clean 第三者著作物検出)
4. git history で sample 関連 commit を辿り、 commit author / commit message から起源特定
...
調査結果は段階 1 sub-sprint α 完了時に本 §Annex A-5 を update commit で literal 反映。
...
`scripts/wav2adpcma.py` (= 仮称) で使用する 4-bit ADPCM-A encoder の採用判断:

**(= 段階 1 sub-sprint γ で encoder 採用判断後に literal 記載)**

候補 encoder:
1. `github.com/superctr/adpcm` (= sound chip ADPCM codec library、 YM2610 ADPCM-A 対応、 既存 library)
2. `github.com/freem/adpcma` (= Neo-Geo ADPCM-A sample encoder、 ただし作者推奨は superctr/adpcm)
3. 自前実装 (= 不採用、 CLAUDE.md §スコープ外 規律 + 3 行重複より早すぎる抽象化を避ける)

採用判断軸:
- license 整合 (= GPL-3.0 / MIT / Apache 等の PMDNEO 整合 license)
- 動作確認 (= 任意 wav から samples.inc 形式出力が成立)
- maintenance 状況 (= active commit 状況)
- PMDNEO build pipeline 統合容易性
- ADPCM-A 精度 (= 4-bit Yamaha 系 ADPCM の bit-accuracy)

採用結果 + 根拠は段階 1 sub-sprint γ 完了時に本 §Annex A-7 を update commit で literal 反映。
...
使用禁止 wording (= §Annex A-6 wording 規律):
