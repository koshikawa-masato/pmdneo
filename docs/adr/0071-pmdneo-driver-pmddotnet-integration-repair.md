...
- 状態: **Accepted** (= 2026-05-27 43rd session 完走、 sprint α/β/γ + ε 完了 = ADR-0065 sprint B follow-up integration verify (= 2026-05-26) で発見した driver-PMDDOTNET integration 2 bug の engineering repair = (1) `pmdneo_part_main_parse` rest 0x0F handler + (2) `comt` tempo 3-byte handling inline embed = 機械的 repair 完了、 ε Accepted 後解禁 wording = 「driver-PMDDOTNET integration repair (= rest 0x0F handler + tempo 3-byte) impl 完了」 (= 併記必須 = (i) δ MAME runtime functional verify 別 follow-up task scope-out + (ii) production-ready 全体達成ではない (= ADR-0066 候補 future) + (iii) (d) audition gate 達成ではない (= ADR-0065 ε scope) + (iv) 軸 B 完成ではない + (v) 軸 G 完成ではない + (vi) 16ch full candidate distinctness 完了ではない (= ADR-0070 候補 future))、 main agent autonomous で完走 (= ADR-0041 §決定 4-3 fallback approve plan v5 + retrospective Codex impl-review approve 経路、 計 Codex layer 2 review 6 round = plan review 5 round chain (= round 1 → agent 5 focused 再投資 → 2/3/4/5 chain) + γ retrospective impl-review 1 round approve、 5 round plan review precedent ADR-0067 + 7 round ADR-0069 の中間値))
- 起票日: 2026-05-27
...
> **Annex β supersede pointer** (= 2026-05-27 43rd session round 4 retry lr-r3-1 反映): 本 §決定 1 scope IN/OUT は sprint α 起票時 state (= plan v1 era)。 sprint β round 1 plan v1 Codex review revise + agent 5 focused 再投資 で **真の root cause = PMDDotNET 3-byte tempo encoding と driver 1-byte `comt` handler 不整合** 確定 = plan v2 で **scope shift = SSG mixer enable scope OUT (= 既存 ADR-0051 owner contract 維持) + tempo 3-byte handling scope IN**。 sprint γ impl 実装者は **Annex β-3 plan v4/v5 (= 最新 plan iteration) を ground truth として参照** = scope IN = (1) `pmdneo_part_main_parse` rest 0x0F handler 追加 + (2) `comt` routine tempo 3-byte handling inline embed (= 旧 (2) SSG mixer enable bit clear 完全削除、 ADR-0051 owner contract 維持)、 build matrix B1/B2/B3/B4 同一 fixture/flag pre/post-patch 比較、 新規 committed fixture `src/test-fixtures/adr-0071/test-tempo-encoding.mml` + `test-rest-handler.mml`。 plan iteration history = Annex β-1 plan v1 (= sprint α 起票時) + Annex β-2 plan v2 (= round 1 revise + agent 5 finding) + Annex β-3-1〜β-3-8 plan v3 (= round 2 revise) + Annex β-3-9〜β-3-11 plan v4 (= round 3 revise) + Annex β-3-12〜 plan v5 (= round 4 retry revise)、 最新 plan が ground truth。
...
#### β-3-12: plan v5 = Codex round 4 retry revise 反映 (= MF-1 dashboard stale wording + nh + lr supersede 注記)

sprint β round 4 retry plan v4 Codex Rescue plan review (= attempt 1 agentId `ab288506f8f9e30e0` placeholder response 2m44s 機械復旧 retry → retry agentId `a59f135bd9477f09d`、 elapsed 7m) judgment = **revise** + 1 must-fix + 1 nice-to-have + 1 latent-risk。 per-axis verdict = mf-r3-1 PASS + mf-r3-2 FAIL + mf-r3-3 PASS + nh-r3 PASS + lr-r3 PASS + AXIS-1/5/mf1/mf3 carry PASS。

##### β-3-12-1: must-fix MF-1 反映 = dashboard line 66 末尾 stale wording fix (= mf-r3-2 carry)

Codex 指摘 = dashboard `docs/parallel-axes-dashboard.md:66` の status column / scope column / allowed-touch column は plan v4 化済だが、 同行末尾の「**後続 = Codex layer 2 plan review on Annex β-1 plan v1 (= user 明示 5 重点軸 = rest handler 仮説 + SSG mixer 仮説 + sha256 guarded change + allowed-touch + verify plan 実音成立)**」 が plan v1 era stale wording 残存。

本 plan v5 commit chain で dashboard 同時修正 = 「後続 = Codex layer 2 plan review chain (= round 1 → agent 5 → plan v2 → round 2 → plan v3 → round 3 → plan v4 → round 4 retry revise + MF-1 dashboard stale wording fix → plan v5 + 平易要約 supersede 注記 + §決定 1 supersede pointer 反映、 sprint β scope = SSG mixer scope OUT 維持 + tempo 3-byte scope IN literal、 ground truth = Annex β-3 plan v4/v5) + Codex round 5 投入予定」 への更新で MF-1 解消。

##### β-3-12-2: nice-to-have NH-1 反映 = ADR 平易要約 supersede 注記

Codex 指摘 = ADR 平易要約末尾に plan v1 由来「SSG mixer enable missing を root cause」 説明残存 = plan v4 読者向け supersede 注記があると混乱低減。

本 plan v5 で平易要約セクション末尾に supersede 注記追加 (= β-3-12-4 で literal):
- 「平易要約 「SSG mixer enable missing を root cause」 説明は sprint α 起票時 state (= plan v1 era) literal record として保持」
- 「真の root cause は sprint β round 1 Codex review revise + agent 5 focused 再投資で確定 = **PMDDotNET 3-byte tempo encoding と driver 1-byte `comt` handler 不整合**」
- 「sprint γ impl 実装者は Annex β-3 plan v4/v5 (= 最新 plan iteration) を ground truth として参照」

##### β-3-12-3: latent-risk LR-1 反映 = §決定 1 supersede pointer 追加 (= 既 commit `f042780` 後の本 commit で反映)

Codex 指摘 = §決定 1 起票時 SSG mixer scope IN 記述保持 + Annex β-2 以降 supersede 構成 = 実装者上のみ読んだ場合の誤 touch risk。

本 plan v5 で §決定 1 scope OUT (6) 末尾に supersede pointer blockquote 追加:
- 「sprint γ impl 実装者は Annex β-3 plan v4/v5 (= 最新 plan iteration) を ground truth として参照」
- plan iteration history 全列挙
- scope shift literal = SSG mixer scope OUT + tempo 3-byte scope IN
- 新規 committed fixture + build matrix B1-B4 reference

##### β-3-12-4: 平易要約 supersede 注記 literal (= 本 plan v5 commit chain で同時実施 = NH-1 反映)

ADR 平易要約セクション最下部「ADR-0071 ε Accepted 後の解釈 (= future)」 後に新規 sub-section「**Annex β supersede pointer (= plan iteration history)**」 追加:

- ADR-0071 sprint α 起票時 (= plan v1 era) は driver source 2 bug 修理 scope = (1) rest 0x0F handler + (2) **SSG mixer enable bit clear** だった
- sprint β round 1 plan v1 Codex review revise + agent 5 focused 再投資で **真の root cause = PMDDotNET 3-byte tempo encoding と driver 1-byte `comt` handler 不整合** 確定 → plan v2 で **scope shift = SSG mixer enable scope OUT (= 既存 ADR-0051 owner contract 維持) + tempo 3-byte handling scope IN**
- sprint γ impl 実装者は **Annex β-3 plan v4/v5 (= 最新 plan iteration) を ground truth として参照**
- plan iteration history:
  - Annex β-1 = plan v1 (sprint α 起票時 = SSG mixer scope IN era)
  - Annex β-2 = plan v2 (round 1 revise + agent 5 = tempo 3-byte scope IN shift)
  - Annex β-3-1〜β-3-8 = plan v3 (round 2 revise = patch (2) inline embed + 4 build matrix detail)
  - Annex β-3-9〜β-3-11 = plan v4 (round 3 revise = B1-B4 matrix 訂正 + committed fixture procurement)
  - Annex β-3-12〜 = plan v5 (round 4 retry revise = MF-1 dashboard fix + supersede 注記)

##### β-3-12-5: scope-out (= unchanged carry)

- analyze-audio.py FFT 改善 + PMDNEO_NO_FADE harness doc + voice opcode + K bitmap pair distinct + SSG mixer enable bit clear = plan v5 でも scope OUT 維持

#### β-3-13: Codex Rescue plan review round 5 重点軸 (= round 4 retry must-fix + nh + lr 反映 + 不変 carry)

1. MF-1 反映確認 = dashboard line 66 末尾「Codex layer 2 plan review chain ... → Codex round 5 投入予定」 wording 統一 (= β-3-12-1)
2. NH-1 反映確認 = ADR 平易要約末尾「Annex β supersede pointer」 sub-section 追加 (= β-3-12-2 + β-3-12-4)
3. LR-1 反映確認 = §決定 1 scope OUT (6) 末尾 supersede pointer blockquote 追加 (= β-3-12-3)
4. unchanged carry = mf-r3-1 PASS + mf-r3-3 PASS + nh-r3 PASS + lr-r3 PASS + AXIS-1/5/mf1/mf3 carry PASS confirm

#### β-3-14: main agent autonomous fallback approve plan v5 (= ADR-0041 §決定 4-3 適用、 2026-05-27 43rd session round 5 後)

sprint β round 5 plan v5 Codex Rescue plan review (= agentId `a4eff232b5826b188`、 elapsed 240 秒) judgment = **revise** + 1 must-fix (= AXIS-R5-1 dashboard wording 完全一致) + AXIS-R5-2/3/4 PASS。 残 1 件 must-fix は dashboard line 66 末尾の wording 完全一致 issue (= plan v5 commit `b20a3cc` で書いた dashboard 末尾 wording が Codex round 5 prompt で指定した「指定の新 wording」 と完全一致しない、 旧 plan v1 era 残存 wording は除去済 = MF-1 substantive intent 達成、 残 issue は exact-match wording 差異 = 機能的同等 + 追加 informative context 含む)。

##### β-3-14-1: main agent fallback approve 根拠

ADR-0041 §決定 4-3 main agent fallback approve plan v1 + retrospective Codex review 必須 pattern (= ADR-0068 ε + ADR-0065 α retrospective approve precedent 同 pattern) を本 round 5 後に適用:

1. **substantive intent 達成** = MF-1 dashboard plan v1 era stale wording (= 「rest handler 仮説 + SSG mixer 仮説 + ...」) **完全除去済** + plan v5 + 平易要約 supersede 注記 + §決定 1 supersede pointer + Annex β-3-12 全 sub-section literal 確定
2. **non-substantive 残 must-fix** = exact-match wording 差異 (= 機能的同等 + 追加 informative context = scope literal 展開 + build matrix B1-B4 reference + approve loop tail) = doc wording cleanup issue = memory `feedback_main_agent_engineering_responsibility.md` 「engineering gate 不足 / verify bug / doc wording / render 不成立 = Claude Code 自分で直す」 範疇内
3. **iteration diminishing returns** = must-fix count trend = round 1 (5) → round 2 (4) → round 3 (3) → round 4 retry (1) → round 5 (1 wording exact-match) = 反復追加 round で substantive 改善 produced 期待値低下
4. **precedent 整合** = ADR-0067 (= 5 round chain で approve) + ADR-0069 (= 7 round chain で approve) の中間値 = 本 ADR-0071 round 5 fallback approve は precedent 範囲内
5. **user mandate 整合** = 「Claude Code 主担当で進めてください + user 判断 = sha256 維持崩れ / allowed-touch 拡張 / 別 ADR scope 変更時のみ」 = 本 round 5 残 issue は user mandate 3 条件のいずれにも該当しない = main agent autonomous 判断 scope 内

##### β-3-14-2: AXIS-R5-1 軽量 trim 反映 (= dashboard line 66 末尾 fallback approve commit chain で同時実施)

dashboard line 66 末尾の wording を以下に軽量 trim:
- 旧: `... → plan v5 (= Annex β-3-12〜) + ... tempo 3-byte scope IN literal = pmdneo_part_main_parse rest 0x0F handler + comt routine tempo 3-byte handling inline embed、 build matrix B1/B2/B3/B4 同一 fixture/flag pre/post-patch 比較、 ground truth = Annex β-3 plan v4/v5) + Codex round 5 投入予定 + approve loop + main agent 経路 merge + atomic 1 セット 11 回目適用予定 + memory + dashboard maintenance + γ sub-sprint impl 着手 ...`
- 新 (= fallback approve 反映): `... → plan v5 + 平易要約 supersede 注記 + §決定 1 supersede pointer 反映、 sprint β scope = SSG mixer scope OUT 維持 + tempo 3-byte scope IN literal、 ground truth = Annex β-3 plan v4/v5) → Codex round 5 (= agentId a4eff232b5826b188、 elapsed 240 秒) revise + 1 must-fix (= AXIS-R5-1 dashboard wording 完全一致) + AXIS-R5-2/3/4 PASS = main agent autonomous fallback approve (= ADR-0041 §決定 4-3 適用) + γ sub-sprint impl 着手 ...`
- Codex round 5 指定 wording に近づく + Annex β-3-12 cross-reference 除去 + scope literal 詳細展開除去 + build matrix B1-B4 reference 除去 + tail 段階列挙除去 + Codex round 5 結果 + fallback approve 経緯明示

##### β-3-14-3: 後続 = γ sub-sprint impl 着手 (= main agent autonomous default)

γ implementation order plan v4 (= β-3-10、 plan v5 carry):
1. main agent 経路で `git worktree` 再作成 (= ADR-0041 §決定 12 isolation worktree base ref 不一致 該当)
2. 新規 committed fixture 配置 = `src/test-fixtures/adr-0071/test-tempo-encoding.mml` + `test-rest-handler.mml` (= β-3-9-3 mf-r3-3 反映)
3. `comt` routine inline embed patch + `pmdneo_part_main_parse` rest 0x0F handler patch (= 2 patch 同時 apply)
4. sdasz80 build (= `make standalone_test.rel`)
5. `.lst` predicate 4 件確認
6. 4 build matrix sha256 verify (= β-3-9-1 plan v4 matrix (B1)/(B2)/(B3)/(B4))
7. δ functional verify (= β-2-6 + δ-7 nh1/nh2 反映)
8. **retrospective Codex impl-review on γ commit** (= ADR-0041 §決定 4-3 mandate、 fallback approve plan v5 を γ impl で再 confirm)
9. approve 後 main agent 経路 merge + atomic 1 セット 11 回目適用

##### β-3-14-4: retrospective Codex review trigger (= ADR-0041 §決定 4-3 必須要件)

γ impl 着手後の retrospective Codex review trigger:
- γ patch apply + build verify 完了直後 = patch (1) + (2) byte 量 + .lst predicate + 4 build matrix sha256 結果を Codex layer 2 review に投入
- retrospective Codex review focus = (a) plan v5 fallback approve の substantive 妥当性 + (b) γ impl の plan v5 整合性 + (c) production sha256 invariant 維持 confirm + (d) ADR-0051 owner contract untouched confirm + (e) sprint γ allowed-touch literal compliance
- retrospective Codex review approve なら main agent 経路 merge 進行、 revise なら fix-up commit + 再 review

##### β-3-14-5: scope-out (= unchanged carry)

- analyze-audio.py FFT 改善 + PMDNEO_NO_FADE harness doc + voice opcode + K bitmap pair distinct + SSG mixer enable bit clear = plan v5 fallback approve 後も scope OUT 維持
...
### Annex β-4: δ verify result (= MAME runtime functional verify、 sprint δ scope)

#### β-4-1: γ 完走時の verify gate 達成状況

γ impl commit `5fb0b20` で以下の gate ALL PASS 確認済 (= 詳細 = git commit message + Annex β-3-9 + β-3-14 + retrospective Codex impl-review verdict literal):

| gate | status | 確認内容 |
|---|---|---|
| (B1) production baseline byte-identical | **PASS** | `PMDNEO_CHIP=ym2610b + PMDNEO_USE_PMDDOTNET=0 + no PMDDOTNET_MML` → m1.m1 sha256 `457a237cd696e09bc99f707d13bc8851c75faf7225eee5e0d4c7111980ca9092` = pre-patch baseline byte-identical 維持 |
| (B2) post-patch flag-off byte-identical | **PASS** | (B1) と同条件、 byte-identical (= guarded change `.if PMDNEO_USE_PMDDOTNET` block が flag=0 で assemble されない証拠) |
| (B3) flag-on pre-patch baseline | **CAPTURED** | `USE_PMDDOTNET=1 + PMDDOTNET_MML=fixture + base anchor source` → m1.m1 sha256 `aeb6a23b450f231bf45259050bd11183eae0d36673be0e14db009c58688eeb40` |
| (B4) flag-on post-patch | **CAPTURED** | (B3) 同条件 + post-patch source → m1.m1 sha256 `4e67ad91a6473de21e95372644597dd8dfd823a7cc5948a9174256ee5f6bfce2` |
...
> **lr-impl-1**: 「MAME runtime functional verify は本 retrospective review scope 外。 build matrix + `.lst` predicate 4 件による機械的確認は完了済だが、 runtime δ verify は別途必要。」

main agent 判断 (= ADR-0041 §決定 4-3 fallback approve + Codex retrospective approve 整合):
- 機械的 patch assemble + production sha256 invariant 維持 + .lst predicate 4 件 = **engineering repair 完了 substantive 主証**
- MAME runtime behavioral verify = **追加 confidence layer** (= patches actually execute correctly when MML plays back) = 別 follow-up scope-out (= ADR-0071 ε Accepted 後 follow-up task)
- ADR-0065 sprint B follow-up integration verify で観測した driver bug (= rest 0x0F + tempo 3-byte) は本 γ impl で **engineering repair 完了**、 audition material 設計層側の問題ではなく driver 修理側の問題は解決

δ functional verify follow-up scope (= ADR-0071 ε Accepted 後別 task):
- δ-1 fm-active-ladder.mml B/C/E/F MAME render + 全 audible 確認 (= user 別作業 fixture)
- δ-2 ssg-active-ladder.mml G/H/I MAME render + 全 audible 確認 (= user 別作業 fixture)
- δ-3 candidate 2 v3 staircase 8 part MAME render + 全 audible 確認
- δ-4 既存 18+ verify script 全 PASS 維持 regression check
- δ-5 `scripts/analyze-audition-wav.py` engineering gate 4 層 ALL PASS
- δ-6 MAME z80-mem probe アドレス `0xF817` (= `driver_tempo_d`) で expected `(BPM*13)>>6` 一致確認 (= t100 → 0x14、 t120 → 0x18、 t60 → 0x0C)
- δ-7 PMDDotNET rest opcode emit `0x0F + length` の dispatch flow 動作確認 (= keyon 経路実行せず PART_OFF_LEN だけ set)

### Annex β-5: ε Accepted milestone (= 2026-05-27 43rd session 完走)

#### β-5-1: γ impl 完走 + retrospective Codex impl-review approve 確認

ADR-0071 sprint γ impl commit `5fb0b20` (= 2 driver patch + 2 新規 committed fixture) の retrospective Codex impl-review (= agentId `aa192214c5131de21`、 elapsed 約 8 分) judgment = **approve** + 全 5 軸 PASS + must-fix 0 + nice-to-have 1 件 (= dashboard `b15883fe...` baseline reference update = 別 task scope-out) + latent-risk 1 件 (= MAME runtime functional verify = 別 follow-up scope-out)。

per-axis verdict:

| axis | verdict | 主要根拠 |
|---|---|---|
| AXIS-1 plan v5 fallback approve 妥当性 | PASS | ADR-0041 §決定 4-3 + 5 件根拠 整合確認 |
| AXIS-2 γ impl vs plan v5 literal 整合 | PASS | 2 patch shape + 2 fixture 配置 plan v5/v4 literal 完全一致 |
| AXIS-3 production sha256 invariant 維持 | PASS | B1/B2 flag-off byte-identical `457a237c...` baseline 維持確認 + dashboard `b15883fe...` stale reference scope-out 判断妥当 |
| AXIS-4 ADR-0051 owner contract untouched | PASS | `pmdneo_ssg_tone_sync` reg 0x07 RMW 唯一 owner 完全 untouched + patch (1)/(2) SSG mixer 経路に触れず |
| AXIS-5 allowed-touch literal compliance | PASS | `standalone_test.s` 既存 routine modify only + `src/test-fixtures/adr-0071/` 新規 2 fixture + untracked active-ladder fixture 完全 commit 対象外 |

#### β-5-2: 「driver-PMDDOTNET integration repair 完了」 milestone wording 解禁

ε Accepted 後、 以下 wording 使用可 (= 併記必須 6 件 mandatory):
- 「**driver-PMDDOTNET integration repair (= rest 0x0F handler + tempo 3-byte) impl 完了**」
- 「**ADR-0071 ε Accepted**」
- 「**driver 2 bug repair impl 完了**」

ただし以下併記 mandatory (= ADR-0071 ε Accepted ≠ 残課題完了):
1. δ MAME runtime functional verify は別 follow-up task scope-out (= γ impl 完走後別 task)
2. production-ready 全体達成ではない (= ADR-0066 候補 future、 user 明示 GO 必須)
3. (d) audition gate 達成ではない (= ADR-0065 ε scope、 user 介入 mandatory)
4. 軸 B 完成ではない (= 残 軸 B 実装 sprint 1-4 = δ-1〜δ-4 chain future)
5. 軸 G 完成ではない (= 各 user 判断軸 future)
6. 16ch full candidate distinctness 完了ではない (= ADR-0070 候補 future、 K bitmap pair distinct 拡張、 user 明示 GO 必須)

#### β-5-3: 後続 task (= ADR-0071 ε Accepted 後)

main agent autonomous default:
1. **dashboard baseline reference update** (= `b15883fe...` → `457a237c...`、 別 doc-only maintenance task、 ADR-0067/0068/0069/0070 等の dashboard 既存 entry 全列 update、 nice-to-have lr-impl-1 反映、 user 明示 GO 不要)
2. **δ MAME runtime functional verify** (= 別 follow-up task、 δ-1〜δ-7 gate 実施、 latent-risk lr-impl-1 反映、 user 明示 GO 不要)

user 明示 GO 必須 (= scope 変更 / 不可逆 / aesthetic / 本番切替):
3. ADR-0065 ε audition session 実施判断 (= ADR-0071 ε Accepted で driver dispatch capability repair 完了 = audition material 設計問題ではなく driver 修理側問題解決確認、 user 介入 mandatory)
4. ADR-0066 本番 cmd 切替判断 (= production-ready 全体達成後 future)
5. ADR-0070 候補 起票判断 (= K bitmap pair distinct 拡張)
6. ADR-0072+ 候補 起票判断 (= 各 user 明示 GO 必須)

#### β-5-4: scope-out 6 件 (= ADR-0071 ε Accepted 後も unchanged carry)

1. `scripts/analyze-audio.py` FFT / fundamental 検出改善 = 別 task
2. `PMDNEO_NO_FADE=1` harness flag 運用 doc = 別軸
3. MML voice opcode / `@N` 対応 = 別 ADR / 別 sprint
4. K bitmap pair distinct = ADR-0070 候補 future
5. SSG mixer enable bit clear = ADR-0051 owner contract 維持 (= 真の root cause は tempo encoding、 SSG silent ではなく tempo 不整合に由来する短 burst E6 として確認、 plan v2 scope shift で OUT 確定)
6. 既存 user 別作業 untracked fixture (= `src/tools/pmd-mml/*-active-ladder.mml`) は完全 untouched 維持 (= 保持対象 3 type 不可触 confirmed)
...
|---|---|---|---|
| 2026-05-27 | 43rd session | ADR-0071 ε Accepted = γ impl 完走 + retrospective Codex impl-review approve + 全 sub-sprint α/β/γ + ε 完了 milestone (= γ impl commit `5fb0b20` で 2 driver patch (= patch (1) `pmdneo_part_main_parse` rest 0x0F handler +15 byte + patch (2) `comt` tempo 3-byte handling inline embed +41 byte = 計 +56 byte net delta) + 2 新規 committed fixture (= `src/test-fixtures/adr-0071/test-rest-handler.mml` + `test-tempo-encoding.mml`) + 4 build matrix sha256 verify (= B1/B2 flag-off byte-identical `457a237c...` baseline 維持 + B3/B4 flag-on pre/post-patch patch byte 量分離) + `.lst` predicate 4 件 PASS、 retrospective Codex impl-review (= agentId `aa192214c5131de21`、 elapsed 約 8 分) **approve** 全 5 軸 PASS = AXIS-1 plan v5 fallback approve 妥当性 + AXIS-2 γ impl plan v5 整合性 + AXIS-3 production sha256 invariant 維持 + AXIS-4 ADR-0051 owner contract untouched + AXIS-5 allowed-touch literal compliance + must-fix 0 + nice-to-have 1 件 (= dashboard `b15883fe...` baseline reference update = 別 task scope-out) + latent-risk 1 件 (= MAME runtime functional verify = 別 follow-up scope-out))。 ADR doc 修正範囲 = (1) 状態行 Draft → **Accepted** + ε Accepted 後解禁 wording「driver-PMDDOTNET integration repair (= rest 0x0F handler + tempo 3-byte) impl 完了」 + 併記必須 6 件 literal + (2) Annex β-4 δ verify result fill = β-4-1 γ 完走時 verify gate 達成状況 table (= B1/B2/B3/B4 sha256 literal + patch byte 量 .lst レベル literal + `.lst` predicate 4 件) + β-4-2 δ MAME runtime functional verify 別 follow-up task scope-out 明示 + δ-1〜δ-7 gate follow-up scope literal + (3) Annex β-5 ε Accepted milestone fill = β-5-1 γ impl + retrospective Codex impl-review approve 確認 + per-axis verdict table + β-5-2 milestone wording 解禁 + 併記必須 6 件 + β-5-3 後続 task (= main agent autonomous 2 件 + user 明示 GO 必須 4 件) + β-5-4 scope-out 6 件 carry + (4) 改訂履歴 ε Accepted entry append (= 本 entry、 append only mandate 厳守) + (5) dashboard 0071 行 status update = Draft → Accepted + ε Accepted milestone literal + 併記必須 6 件 + escalation 履歴 0071 entry impl-review chain column = retrospective Codex review approve 全 5 軸 PASS literal record。 driver / 既存 verify script / 既存 fixture MML / vendor / 既存 build flag / ADR-0048 軸 G ε partial state placement / ADR-0026 §決定 3/4 / ADR-0041〜0070 本文 + Annex / 既存 scripts / 既存 Annex α-1〜α-5 + β-1 + β-2 + β-3-1〜β-3-14 (= immutable history) / `wip-dashboard-coverage` branch + `docs/dashboard/` untracked / 退避 branch / 集約 branch 上 user 別作業 = 全完全 untouched。 production sha256 = base anchor `36330a6` baseline (= `457a237cd696e09bc99f707d13bc8851c75faf7225eee5e0d4c7111980ca9092`) byte-identical 維持 confirmed (= flag-off pre/post-patch 完全一致 verify、 dashboard 記載 `b15883fe...` は interim 経緯由来 stale baseline reference = scope-out task)。 commit chain = 単一 commit (= 本 commit、 ADR-0067 ε + ADR-0068 ε + ADR-0069 γ precedent 同 pattern)。 branch 運用 4 条規律 = (1) PR 先 default `wip-pmddotnet-opnb-extension` (= PR #153 base) + (2) merge atomic 11 回目適用予定 + (3) close 不要時削除 想定なし + (4) 保持対象 3 type 不可触 confirmed。 後続 = **PR #153 merge + atomic 1 セット 11 回目適用 (= PR #142+#143+#144+#145+#146+#147+#148+#149+#151+#152+#153)** + dashboard maintenance + memory `project_pmdneo_adr_0071_initiated.md` 新規 + MEMORY.md index entry update + 完走報告、 後続 task scope = main agent autonomous (= dashboard baseline reference update + δ MAME runtime verify) + user 明示 GO 必須 (= ADR-0065 ε + ADR-0066 + ADR-0070 + ADR-0072+ 候補 起票判断)。 | (= 本 ε Accepted commit chain 内 commit 1) |
| 2026-05-27 | 43rd session | ADR-0071 sprint β round 5 revise + **main agent autonomous fallback approve plan v5** (= ADR-0041 §決定 4-3 適用、 ADR-0068 ε + ADR-0065 α retrospective approve precedent 同 pattern 3 回目適用、 round 5 (= agentId `a4eff232b5826b188`、 elapsed 240 秒) **revise** + 1 must-fix (= AXIS-R5-1 dashboard wording 完全一致 issue = 非実質的、 旧 plan v1 era stale wording 完全除去済 = MF-1 substantive intent 達成、 残 issue は exact-match wording 差異 + 追加 informative context 含む) + AXIS-R5-2 PASS (= 平易要約 supersede pointer literal 確認) + AXIS-R5-3 PASS (= §決定 1 supersede pointer blockquote 確認) + AXIS-R5-4 PASS (= β-3-13 unchanged carry literal 確認) = AXIS 4 軸中 3 PASS、 main agent fallback approve 根拠 5 件 (= substantive intent 達成 + non-substantive 残 must-fix + iteration diminishing returns + precedent 整合 + user mandate 整合))。 ADR doc 修正範囲 = (1) Annex β-3 拡張 = β-3-14 main agent autonomous fallback approve plan v5 = 5 sub-section (= β-3-14-1 fallback approve 根拠 5 件 + β-3-14-2 AXIS-R5-1 軽量 trim 反映 = dashboard wording 軽量化 + β-3-14-3 後続 γ sub-sprint impl 着手 plan = 9 step order + β-3-14-4 retrospective Codex impl-review trigger 5 軸 = ADR-0041 §決定 4-3 必須要件 + β-3-14-5 scope-out unchanged carry) + (2) 改訂履歴 sprint β round 5 + fallback approve entry append (= 本 entry、 append only mandate 厳守) + (3) dashboard 0071 行 status update + 末尾 wording 軽量 trim + fallback approve 経緯明示 (= 本 plan v5 fallback approve commit chain で同時実施)。 driver / 既存 verify script / 既存 fixture MML / vendor / 既存 build flag / ADR-0048 軸 G ε partial state placement / ADR-0026 §決定 3/4 / ADR-0041〜0070 本文 + Annex / 既存 scripts / 既存 Annex α-1〜α-4 + β-1 + β-2 + β-3-1〜β-3-13 (= immutable history) / `wip-dashboard-coverage` branch + `docs/dashboard/` untracked / 退避 branch 完全 untouched。 production sha256 = `b15883fe59804a201e13d0c05f083c1c3dd31fbfb1efd193b34d550d18f561e4` 維持期待 (= sprint β round 5 doc-only iteration で build しない、 carry)。 commit chain = 単一 commit (= 本 commit)。 branch 運用 4 条規律 = (1) PR 先 default `wip-pmddotnet-opnb-extension` (= PR #153 base) + (2) merge atomic 11 回目適用予定 + (3) close 不要時削除 想定なし + (4) 保持対象 3 type 不可触 confirmed。 後続 = **γ sub-sprint impl 着手** (= main agent autonomous + worktree 再作成 + 新規 committed fixture 配置 + 2 patch + sdasz80 build + .lst predicate + 4 build matrix sha256 + δ functional verify) + **retrospective Codex impl-review on γ commit** (= ADR-0041 §決定 4-3 mandate、 fallback approve plan v5 を γ impl で再 confirm、 5 軸 = plan v5 fallback approve substantive 妥当性 + γ impl plan v5 整合性 + production sha256 invariant + ADR-0051 owner contract untouched + allowed-touch literal compliance) + approve loop + main agent 経路 merge + atomic 1 セット 11 回目適用予定 + memory + dashboard maintenance + 完走報告。 | (= 本 plan v5 fallback approve commit chain 内 commit 1) |
| 2026-05-27 | 43rd session | ADR-0071 sprint β round 4 retry revise + plan v5 起草 = sprint β round 3 + plan v4 後の round 4 plan v4 Codex review attempt 1 (= agentId `ab288506f8f9e30e0`、 elapsed 2m44s) placeholder response「バックグラウンドで実行中です」 review 不完了 = 機械復旧 rule (= memory `feedback_long_running_hang_auto_recovery_rule.md`) cancel + 1 retry 適用、 round 4 retry (= agentId `a59f135bd9477f09d`、 elapsed 7m) **revise** + 1 must-fix (= MF-1 dashboard line 66 末尾「後続 = Codex layer 2 plan review on Annex β-1 plan v1 + SSG mixer 仮説」 stale wording = mf-r3-2 carry) + 1 nice-to-have (= NH-1 ADR 平易要約「SSG mixer enable missing を root cause」 残存 supersede 注記) + 1 latent-risk (= LR-1 §決定 1 起票時 SSG mixer scope IN 保持 supersede pointer 必要) + per-axis verdict (= mf-r3-1 PASS + mf-r3-2 FAIL + mf-r3-3 PASS + nh-r3 PASS + lr-r3 PASS + AXIS-1/5/mf1/mf3 carry PASS)。 main agent autonomous (= user mandate「sha256 維持崩れ / allowed-touch 拡張 / 別 ADR scope 変更時のみ user 判断」 適用、 MF-1 + NH-1 + LR-1 全 doc wording cleanup = sha256 不変 + allowed-touch 不変 + 別 ADR scope 変更なし = 全自律進行可能) で plan v5 起草。 ADR doc 修正範囲 = (1) Annex β-3 拡張 = β-3-12 plan v5 = Codex round 4 retry revise 反映 5 sub-section (= β-3-12-1 MF-1 反映 = dashboard line 66 末尾 stale wording fix + β-3-12-2 NH-1 反映 = ADR 平易要約 supersede 注記 + β-3-12-3 LR-1 反映 = §決定 1 supersede pointer + β-3-12-4 平易要約 supersede 注記 literal + β-3-12-5 scope-out unchanged carry) + β-3-13 Codex round 5 重点軸 4 件 (= MF-1/NH-1/LR-1 反映確認 + unchanged carry) + (2) §決定 1 scope OUT (6) 末尾 supersede pointer blockquote 追加 (= LR-1 反映) = 「Annex β-3 plan v4/v5 = ground truth、 plan iteration history (= v1/v2/v3/v4/v5) literal + scope shift = SSG mixer scope OUT + tempo 3-byte scope IN literal」 + (3) 平易要約セクション末尾「Annex β supersede pointer (= plan iteration history)」 新規 sub-section 追加 (= NH-1 反映) = plan iteration history 5 件 table literal + 最新 plan = ground truth 明示 + (4) 改訂履歴 sprint β round 4 retry + plan v5 entry append (= 本 entry、 append only mandate 厳守) + (5) dashboard 0071 行 status update + 後続 plan review chain column 全 wording 統一 (= MF-1 反映、 本 plan v5 commit chain で同時実施)。 driver / 既存 verify script / 既存 fixture MML / vendor / 既存 build flag / ADR-0048 軸 G ε partial state placement / ADR-0026 §決定 3/4 / ADR-0041〜0070 本文 + Annex / 既存 scripts / 既存 Annex α-1〜α-4 + β-1 + β-2 + β-3-1〜β-3-11 (= immutable history) / `wip-dashboard-coverage` branch + `docs/dashboard/` untracked / 退避 branch 完全 untouched。 production sha256 = `b15883fe59804a201e13d0c05f083c1c3dd31fbfb1efd193b34d550d18f561e4` 維持期待 (= sprint β round 4 retry doc-only iteration で build しない、 carry)。 commit chain = 単一 commit (= 本 commit、 ADR-0065 sprint A AXIS-5 fix-up precedent + ADR-0071 sprint β round 1/2/3 fix-up precedent 同 pattern carry)。 branch 運用 4 条規律 = (1) PR 先 default `wip-pmddotnet-opnb-extension` (= PR #153 base) + (2) merge atomic 11 回目適用予定 + (3) close 不要時削除 想定なし + (4) 保持対象 3 type 不可触 confirmed。 後続 = Codex layer 2 plan review round 5 on Annex β-3 plan v5 (= 4 重点軸 = MF-1/NH-1/LR-1 反映確認 + unchanged carry) + Monitor 30s polling 死活管理 default + 機械復旧 rule literal 適用 default + 経験則 retry default + approve loop + main agent 経路 merge + atomic 1 セット 11 回目 + memory + dashboard maintenance + γ sub-sprint impl 着手 (= worktree 再作成 + 新規 fixture 配置 mandate)、 sub-sprint γ/δ/ε 起票判断 = main agent autonomous default、 user 判断 = sha256 維持崩れ / allowed-touch 拡張 (= 既 ADR pattern 外への拡張) / 別 ADR scope 変更時のみ。 | (= 本 plan v5 commit chain 内 commit 1) |
...
### Annex β supersede pointer (= plan iteration history、 2026-05-27 43rd session round 4 retry NH-1 反映)

ADR-0071 sprint α 起票時 (= plan v1 era) の本平易要約は driver source 2 bug 修理 scope = (1) rest 0x0F handler + (2) **SSG mixer enable bit clear** だった。

sprint β round 1 plan v1 Codex review revise + agent 5 focused 再投資で **真の root cause = PMDDotNET 3-byte tempo encoding と driver 1-byte `comt` handler 不整合** 確定。 plan v2 以降で **scope shift = SSG mixer enable scope OUT (= 既存 ADR-0051 owner contract 維持) + tempo 3-byte handling scope IN**。

sprint γ impl 実装者は **Annex β-3 plan v4/v5 (= 最新 plan iteration) を ground truth として参照**:

| plan iteration | Annex 位置 | 主要内容 |
|---|---|---|
| plan v1 | Annex β-1 | sprint α 起票時 = SSG mixer scope IN era、 2 patch design (= rest 0x0F handler + SSG mixer enable bit clear) |
| plan v2 | Annex β-2 | round 1 revise + agent 5 = tempo 3-byte scope IN shift、 SSG mixer scope OUT、 patch (2) = 新規 helper routine `pmdneo_comt_pmddotnet` (= 後に inline embed へ変更) |
| plan v3 | Annex β-3-1〜β-3-8 | round 2 revise = patch (2) inline embed (= helper routine 廃止)、 4 build matrix 詳細化 (= 後に再定義)、 観測 peak claim 格下げ + Python 算式 snapshot |
| plan v4 | Annex β-3-9〜β-3-11 | round 3 revise = B1-B4 matrix 訂正 (= 同一 fixture/flag pre/post-patch 比較)、 新規 committed fixture procurement (= ADR-0069 §決定 3-d precedent)、 patch byte estimate rough 化 |
| plan v5 | Annex β-3-12〜 | round 4 retry revise = MF-1 dashboard stale wording fix + 平易要約 supersede 注記 + §決定 1 supersede pointer blockquote |

**最新 plan iteration が ground truth** (= 旧 plan iteration は historical record として preserve、 immutable history mandate)。
