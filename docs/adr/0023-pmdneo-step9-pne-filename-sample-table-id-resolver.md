...
filename ↔ `sample_table_id` 対応表を **driver code 内 hand-written** で持つ。 vromtool.py / converter / build pipeline は一切触らない。 初期 entry は `PMDNEO01.PNE` / id 0x00 の 1 件のみ。
...
- 初期 entry: `PMDNEO01.PNE` / id 0x00 の 1 件のみ
...
#### 初期 directory 内容 (= step 9 出荷時)

```
entry 0: filename = "PMDNEO01.PNE\0\0\0\0" (= 16 byte fixed)
...
- **primary gate**: 0xFD32 観測
  - filename match 時 (= `PMDNEO01.PNE` fixture): `0xFD32 == 0x00`
  - filename mismatch 時 (= directory に存在しない filename fixture): `0xFD32 == 0xFF`
...
| γ | `.MN` load chain 末尾に `call pmdneo_resolve_sample_table_id` 追加 + memory inspection primary gate (= 0xFD32 = 0x00 確認) + mismatch fixture 検証 (= 0xFD32 = 0xFF 確認) | 0xFD32 match: `PMDNEO01.PNE` fixture で 0x00 / mismatch: directory に存在しない filename fixture で 0xFF |
| δ | verify infra 統合 (= `verify-step9-resolver.sh` 等) + step 5/6/7/8 既存 verify script 全件 PASS + MAME 試聴で audible regression なし + step 9 完了統合 handoff doc + ADR-0023 Accepted 移行 | 全 sub primary gate PASS + 既存 verify regression PASS + audible OK |
...
- driver code 内 hand-written directory 追加 (= entry 構造 + 初期 entry "PMDNEO01.PNE" / 0x00 + terminator entry) (= α)
...
6. **γ**: filename match fixture (= `PMDNEO01.PNE`) で `0xFD32 == 0x00` を memory inspection で確認
7. **γ**: filename mismatch fixture (= directory に存在しない filename を持つ `.MN`) で `0xFD32 == 0xFF` を確認
8. **δ**: verify infra (= `verify-step9-resolver.sh` 等) 整備 + commit + push
9. **δ**: step 5/6/7/8 既存 verify script 全件 PASS (= 既存 architecture regression なし)
10. **δ**: MAME 試聴で audible regression なし (= step 6-a silent-bcef fixture で確認)
11. **δ**: step 9 完了統合 handoff doc + ADR-0023 Accepted 移行 + commit + push
