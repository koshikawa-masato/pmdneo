...
| 05 SAMPLE2-baseline | (L only) | L=1 M=1 N=1 O=1 P=1 Q=1 (= L のみ MML 由来 + M-Q init keyon) | 同上 default 8/2 pattern |
...
- SAMPLE2-baseline (= FM A/B/C + SSG I + ADPCM-A L) → A-J 部分は default driven、 L only distinct
...
|---|---|---|---|
| α | (a) 実 MML 再生 統合 verify = **K+L-Q candidate distinctness proof + A-J default integration trace** (= plan v7 = 40th session driver ground truth based)。 candidate = `src/test-fixtures/step5/l-q-rhythm-song.mml` + `src/test-fixtures/step5/l-q-tutti.mml` + `src/test-fixtures/step11/l-q-rhythm-song-step5b.mml` (= K=3 L-Q distinct candidate)、 K distinctness は β scope future。 10 env × ymfm/z80-mem 2 trace 種 capture = 20 trace file + literal report | (a) | K+L-Q distinctness proof (= 各 candidate L-Q trace 個別記録) + A-J default integration trace record (= default 同一 8/2 pattern literal) + 16/16 ch carry actual literal record |
