...
##### existing resource activation + minimal MML 例外許可の原則 (= user 明示 40th session option 2、 plan v6-revised diff 2)

ADR-0068 原則 = existing resource activation (= 既存 candidate 統合 union)。 ただし α union coverage が 16 ch 不足判定の場合、 不足 ch 限定で minimal MML 新規追加を 例外許可 (= user 明示 40th session option 2)。 minimal MML 追加 scope:

- 不足 ch (= FM E/F + SSG G/H 等) の 発音確認に閉じる (= production-ready 音楽性 + audition scope-out)
- 追加 MML 数 = 不足 ch 数で minimal (= 例 = 4 ch 不足なら最大 4 file)
- minimal note fixture pattern (= ADR-0067 fixture 拡張完了 pattern 継承、 例 = `<part> @<voice> o4 l1 c` 単発 note)
- β scope で追加 (= α は existing resource activation のみ、 β で hybrid carry)
- ADR-0064 §決定 1(b)「全 16 ch trace-equivalence」 を v2/cmd 0x05 両経路 carry で完全達成

40th session task #50 ground truth ch coverage 表 (= existing resource union):

| chip 軸 | carry ch | 不足 ch (= β minimal MML 追加対象) |
|---|---|---|
| FM (6 ch = A-F) | A, B, C, D | E, F |
| SSG (3 ch = G-I) | I | G, H |
| ADPCM-B (1 ch = J) | J | (なし) |
| ADPCM-A (6 ch = L-Q) | L, M, N, O, P, Q | (なし) |
| 合計 | 12 / 16 | 4 ch |

K=3 candidate enumeration (= α-task 2 existing resource activation):
...
- α-task 1 = `src/test-fixtures/step5/l-q-rhythm-song.mml` = ADPCM-A L-Q

β minimal MML 候補 enumeration (= β PR3 で literal 確定):
- FM E ch minimal MML (= ym2610 + ym2610b 両発音)
- FM F ch minimal MML (= ym2610b only audible、 ADR-0006 §B chip target 制約)
- SSG G ch minimal MML
- SSG H ch minimal MML
...
|---|---|---|---|
| α | (a) 実 MML 再生 統合 verify = α-task 1 rhythm-only proof (= `src/test-fixtures/step5/l-q-rhythm-song.mml`) + α-task 2 **existing resource activation = 既存 candidate K=3 union による 12/16 ch carry confirm** (= `vendor/PMDDotNET/SAMPLE2-baseline.mml` + `vendor/PMDDotNET/test-aes-ad.mml` + `src/test-fixtures/step4/j-part-g.mml`、 不足 4 ch = FM E/F + SSG G/H は β で minimal MML 追加 carry) + 10 env × ymfm/z80-mem 2 trace 種 capture = 20 trace file + literal report | (a) | α-task 1 + α-task 2 両完了 + 12/16 ch carry confirm + 不足 4 ch literal 報告 + 残 4 ch β minimal MML carry plan literal |
| β | (b) 全 16 ch trace-equivalence 判定基準確定 + 比較実行 = 意図した v2 差分 / 意図しない差分 enumeration literal + α 取得 trace 20+ 件を input として comparison + **不足 4 ch (= FM E/F + SSG G/H) 用 minimal MML 新規追加** (= user 明示 例外許可、 hybrid 方針、 発音確認に閉じる) + **v2/cmd 0x05 両経路 trace-equivalence carry** (= 12 ch existing + 4 ch new minimal MML 統合) | (b) | 全 16 ch trace-equivalence 確認 (= 12 ch existing + 4 ch new minimal MML 統合) + 意図しない差分なし literal 確認 |
| γ | (c) 全 verify script 統合 ALL PASS = production binary 1 件に対して全 verify を 1 batch 実行 + 統合 report | (c) | 全 verify script ALL PASS literal 確認 + completion proof 統合 report (= 全 16 ch trace-equivalence 完了 confirmed) |
...
| PR2 | sub-sprint α = (a) 実 MML 再生 統合 verify = 新規 verify script + α-task 1 rhythm-only proof + α-task 2 existing resource activation K=3 union 12/16 ch carry confirm + 20 trace capture (= 10 env × 2 trace 種) + literal report + Annex α | 実装 + doc |
...
PR1 (= 本 PR) merge 後、 sub-sprint α 実装 PR2 を別 task chain で起票 (= ADR-0068 §決定 2 α scope literal = α-task 1 rhythm-only proof + α-task 2 existing resource activation K=3 union 12/16 ch carry confirm + 20 trace capture (= 10 env × 2 trace 種、 K=3 candidate + α-task 1) + literal report)。 α 完走後 β/γ/δ/ε 各 PR を continue。 ADR-0068 ε Accepted 後、 ADR-0065 候補 (= roadmap ⑥ audition) + ADR-0066 候補 (= roadmap ⑦ 本番 cmd 切替判断) 起票判断 (= 各 user 明示 GO 必須)。
