...
- 状態: **Accepted** (= 2026-05-21 39th session、 軸 B 実装 sprint 6 fade-out semantics の α/β/γ/δ/ε 全 sub-sprint 完走 (= PR #63/#64/#70/#71 MERGED) + user 判断 gate を経て Draft → Accepted 移行。 α = ground truth 調査 + 起票、 β = fade decay path 実装、 γ = fade finish path 実装、 δ = verify script 体系化 (= `verify-fadeout-semantics.sh` 16 gate ALL PASS)、 ε = completion + Accepted 判断。 ADR-0045 §J-4-6 literal 後続実装 ADR。 **軸 B 実装 sprint chain 6 sprint 中 2 本目** = sprint 5 mute (= ADR-0049 Accepted) 完了、 本 sprint 6 fade-out 完了。 **ADR-0050 Accepted = fade-out semantics 完了であり、 軸 B 全体完了ではない** = 実装 sprint 1-4 (= δ-1〜δ-4) 未着手、 「軸 B 完成」 表現不使用。 移行根拠 = Annex J)
- 起票日: 2026-05-20
...
| β (= fade decay path 実装 + driver-embedded fixture) | **完了** (= 39th session、 PR #64) | PR #64 | β driver 実装 Codex layer 2 review = 論点 1 (`pmdneo_psg_keyon` scope) / 論点 2 (pre-existing bug fix) 両 approve + 規律 D1-D5/D7-D10 全 PASS + escalate 2 件 (= pre-existing bug finding / byte-identical 用語) → user GO 取得 |
| γ (= fade finish path 実装) | **完了** (= 39th session、 PR #70) | PR #70 | γ kickoff plan Codex layer 2 review = round 1 revise must-fix 2 (= ADPCM-A bulk `0xBF` dump → `adpcma_keyoff` loop / SSG keyoff に tone disable 欠落) → round 2 approve + γ 実装 review approve (= 規律 check 全 PASS) |
| δ (= verify script 体系化 + 10 gate 完全化) | **完了** (= 39th session、 PR #71) | PR #71 | δ kickoff plan Codex layer 2 review = round 1 revise must-fix 6 (= §決定 7 10 gate 完全網羅 + test-tone-ladder.mml 採用) → round 2 approve → fixture 訂正 (= test-tone-ladder.mml staggered finding、 user 判断 gate option B) → round 3 approve + δ 実装 review (= 後続) |
| ε (= completion + Accepted 判断) | **完了** (= 39th session、 PR #72、 Draft → Accepted 移行) | PR #72 | ε 総点検 + Accepted 移行可否 Codex layer 2 review = round 1 revise must-fix 1 (= Annex G-2 破壊 register 一覧に γ routine `pmdneo_fade_finish_silence` 欠落) → G-2 表追記 → round 2 approve → user 判断 gate で Draft → Accepted 移行 |
...
## Annex G: β 実装 completion record (= fade decay path、 案 b)

### G-1: β deliverable

軸 B 実装 sprint 6 β = fade decay path 実装 (= 39th session、 PR #64)。 ADR-0050 §決定 3 案 (b) = volume hook factor 混入方式で楽曲全体 fade-out の段階減衰経路を実装した。

| 実装項目 | 内容 |
|---|---|
| `pmdneo_v2_fade_level` | 1 byte SRAM field @ 0xFD39 (= free region 先頭、 §決定 4)。 64 = 無減衰 / 0 = 完全減衰。 単一 fade 減衰 factor。 `nmi_cmd_5_init_mml_song` 冒頭で 64 初期化 |
| `pmdneo_fade_begin` | fade 開始共通 routine (= NMI cmd 6 + fade fixture 共通)。 level=64 / master=0x3F / counter=0 / state=1 |
| `pmdneo_v2_fade_tick` | IRQ tick 毎の fade decay 処理 (= 現 `irq_fade_*` を 0x0100 IRQ handler body から移設 + FM/SSG/ADPCM-B 拡張)。 IRQ handler body は `call pmdneo_v2_fade_tick` 1 行化 |
| `pmdneo_v2_fade_reapply` | fade step 毎に FM/SSG/ADPCM-B part 0-9 の volume hook を再適用 (= mute part skip + hook 未設定 skip) |
| `pmdneo_fade_scale` | `(raw * pmdneo_v2_fade_level) >> 6` 乗算 factor 混入 helper。 level=64 で exact passthrough |
| volume hook 4 箇所 | `fm_volume_hook` / `psg_volume_hook` / `adpcmb_volume_hook` / `pmdneo_psg_keyon` に `pmdneo_fade_scale` 混入 (= 案 b、 FM/SSG/ADPCM-B volume write path) |
| ADPCM-A master | reg `0x01` を `pmdneo_v2_fade_level` 派生値で直接 ramp (= 案 b、 volume hook 混入対象外) |
| `TEST_MODE_FADE_FIXTURE` | driver-embedded fixture toggle + `build.mk` / `build-poc.sh` wiring (= `PMDNEO_FADE_FIXTURE=1`) |

`pmdneo_psg_keyon` (= SSG note keyon 経路) も volume scale 対象とした。 SSG は keyon 毎に volume register (reg `0x08`+ch) を書き直すため、 scale しないと fade 中の SSG note が full volume に pop する (= ADR §決定 3 β境界「FM/SSG volume write path」 に含まれる、 Codex layer 2 論点 1 approve)。 FM keyon (`fm_keyon`) は TL を書かず、 ADPCM-B keyon (`adpcmb_keyon`) は reg `0x1B` を書かないため、 keyon 側 scale は SSG のみ必要。

### G-2: fade routine 破壊 register 一覧 (= §決定 5 mandate)

§決定 5 = fade routine の「破壊 register 一覧」 を ADR に literal 記録する。

| routine | 破壊 register | IX/IY |
|---|---|---|
| `nmi_cmd_6_fade_start` | AF (= `pmdneo_fade_begin` 経由) | IX/IY 不変 |
| `pmdneo_fade_begin` | AF | IX/IY 不変 |
| `pmdneo_v2_fade_tick` | AF/BC/DE/HL | IX/IY 不変 (= `pmdneo_v2_fade_reapply` 内部で IX push/pop) |
| `pmdneo_v2_fade_reapply` | AF/BC/DE/HL | IX = push/pop save、 IY 不変 |
| `pmdneo_fade_scale` | AF/BC/DE/HL | IX/IY 不変 |
| `pmdneo_fade_finish_silence` | AF/BC/DE/HL | IX/IY 不変 (= γ 新設 routine、 channel index 直接指定で part workarea / IX 不使用。 ε で §決定 5 破壊 register 一覧へ追記) |

`pmdneo_fade_finish_silence` は γ sub-sprint で新設された fade routine (= fade finish 時の全 chip keyoff、 Annex H-1)。 §決定 5 = fade routine 確定時に破壊 register 一覧へ追記する mandate に従い、 ε sub-sprint で本表へ 1 行追記し一覧を完成させた (= δ verify gate 11 の register preservation 検証と整合)。

IRQ handler body は `call pmdneo_v2_fade_tick` で fade 処理を呼ぶ。 IRQ handler body は AF/BC/DE/HL を push/pop するため AF/BC/DE/HL 破壊は許容範囲。 IX/IY は IRQ handler body が push しないが、 fade routine 群が IX/IY を不変に保つ (= `pmdneo_v2_fade_reapply` が IX を push/pop) ため verify gate 5 (= register preservation) を満たす。

### G-3: β finding = pre-existing `.org` overlap latent bug

β 実装中、 `.org 0x0066` NMI セクションが既に `.org 0x0100` `irq_handler_body` を越えて **silent overlap** していたことを発見した (= memory `feedback_org_section_overflow_silent_bug.md` と同 class)。 HEAD 時点で `nmi_cmd_6_fade_start` (= NMI command 6 = production fade trigger) の body 後半 (= 0x0100 以降) が `irq_handler_body` の機械語に上書き破損していた (= binary 243-m1.m1 の 0x0100-0x0101 が `irq_handler_body` の `di`/`push af` = `f3 f5`、 HEAD build でも再現確認)。 = Annex A が「実装済」 と記録した active 段階 fade は、 production trigger (NMI cmd 6) 経由では一度も動作していなかった。

**対処**: `nmi_cmd_6_fade_start` を 0x0066 セクションから 0x0610 セクション (= `.org` 制約なし) へ移設し overlap を解消 (= §決定 8 の fade routine 0x0610 配置指示と整合する fade trigger 修復)。 `nmi_dispatch` の `jp z, nmi_cmd_6_fade_start` は絶対 jp で配置非依存。 結果、 0x0066 セクションは 0x00FD で終了 (= binary 確認済、 overlap 解消)。 `pmdneo_v2_fade_level` の初期化も 0x0066 セクションではなく `nmi_cmd_5_init_mml_song` (= 0x0610 セクション) に配置し、 0x0066 セクションへの 1 byte 追加も避けた。

### G-4: β 検証結果

- production build PASS + `.lst` section check = fade routine 全件 0x0610 セクション (≥ 0x088D)、 0x0066 セクション 0x00FD 終了、 0x0100 セクション `nmi_cmd_unmask_part` 0x01A9 で 0x0200 未満
- fade fixture build (= `TEST_MODE_FADE_FIXTURE=1`) + MAME headless register trace: `pmdneo_v2_fade_level` (= 0xFD39) が 0x40→0x00 単調減少 66 writes、 ADPCM-A master reg `0x01` が 0x3F→0x00 単調減少 65 writes、 FM TL reg `0x42` が faded TL 0x79→0x7F 単調増加 (= 減衰) 64 reapply writes。 SSG reg `0x08-0x0A` + ADPCM-B reg `0x1B` も per-step write 発生 (= 値 0 は fixture song `test01/test02` が G/H/I V0 + J empty のため、 song 内容由来であり driver bug ではない)
- `verify-mute-semantics.sh` 7 gate ALL PASS (= baseline regression 9 script + mute fixture gate 1-5 + gate 7 `.org` overlap)
- production ROM binary 全体は base と byte-identical では**ない** (= fade 機能追加 = §決定 6 scope-in)。 保護しているのは非 fade path の register trace 等価 (= baseline 9 script PASS) + cmd 0x04 即時 silence byte-identical (= `IRQ.inc` 無変更、 gate 6)
- ADR-0050 verify gate 対応: gate 2 / 5 / 6 / 7 / 8 / 9 / 10 を β で確認。 gate 1 / 3 / 4 の formal verify script 化は δ sub-sprint (= `verify-fadeout-semantics.sh`)

## Annex H: γ 実装 completion record (= fade finish path)

### H-1: γ deliverable

軸 B 実装 sprint 6 γ = fade finish path 実装 (= 39th session、 PR #70)。 §決定 1 γ = fade 減衰完了 (= `pmdneo_v2_fade_level` 0 到達) 時に全 chip を safe silence へ落とす経路を実装した。

| 実装項目 | 内容 |
|---|---|
| `pmdneo_fade_finish_silence` | 新 routine (= 0x0610 セクション、 `pmdneo_fade_scale_zero` 直後)。 全 chip channel を keyoff: FM ch 0-5 → `fm_keyoff` / SSG ch 0-2 → `ssg_keyoff` + `pmdneo_ssg_tone_sync(A=0)` / ADPCM-A ch 0-5 → `adpcma_keyoff` / ADPCM-B → `adpcmb_keyoff`。 channel index 直接指定 = part workarea / IX 不使用 |
| `pmdneo_v2_fade_tick_finish` | `driver_fade_state`=0 後に旧 inline `reg 0x00=0xBF` bulk dump を削除し `call pmdneo_fade_finish_silence` へ置換 |

§決定 1 γ + ADR-0049 §決定 5 = 「4 chip keyoff routine 本体直接 call」。 β は ADPCM-A を `reg 0x00=0xBF` bulk dump で keyoff していた (= `adpcma_keyoff` 本体を call せず直接 register write)。 γ はこれを `adpcma_keyoff` ch 0-5 loop へ置換し、 FM/SSG/ADPCM-A/ADPCM-B 全 4 chip を keyoff routine 本体 call で統一した (= Codex layer 2 kickoff plan review round 1 must-fix 1)。

SSG keyoff は `ssg_keyoff` (= volume 0) のみでなく `pmdneo_ssg_tone_sync(A=0)` も呼び reg `0x07` tone bit を disable する (= ADR-0051 §決定 3/4 = SSG keyoff の symmetric tone disable 契約、 Codex layer 2 kickoff plan review round 1 must-fix 2)。 `pmdneo_ssg_tone_sync` は reg `0x07` shadow の唯一 RMW owner であり、 fade finish からの call は call site 追加であって owner の増加ではない (= ADR-0051 §決定 4 契約維持)。

### H-2: γ 検証結果

- production build PASS + `.lst` section check = `pmdneo_fade_finish_silence` 全体が 0x0610 セクション内、 0x0066 / 0x0100 / 0x0200 セクション不変 (= `nmi_cmd_unmask_part` 0x01A9 < `.org 0x0200`)、 overflow なし
- fade fixture build (= `TEST_MODE_FADE_FIXTURE=1`) + MAME headless register trace: fade finish burst を ymfm-trace で観測 = FM reg `0x28` ×6 (= 値 00/01/02/04/05/06 = `fm_keyoff_values[0..5]` exact) + SSG reg `0x08`/`0x09`/`0x0A`=00 ×3 (= keyoff volume 0) + reg `0x07` ×3 (= `pmdneo_ssg_tone_sync` tone disable) + ADPCM-A reg `0x00` ×6 (= 値 81/82/84/88/90/A0 = `0x80` | `adpcma_ch_bit_table` exact) + ADPCM-B reg `0x10` ×2 (= 01/00 = `adpcmb_keyoff`) = 計 20 write が routine 順通り。 fade finish は trace 全体で 1 回のみ発火 (= `driver_fade_state` 0xF819 が finish PC で 0 へ write、 ADPCM-A master reg `0x01` 0x00 到達直後)
- `verify-mute-semantics.sh` 7 gate ALL PASS (= ADR-0049 mute regression なし)
- `verify-ssg-tone-enable.sh` 12 gate ALL PASS (= ADR-0051 SSG tone-enable regression なし、 gate 11 = ADR-0050 fade routine 存在も PASS)
- Codex layer 2 review = γ kickoff plan round 1 revise (= must-fix 2 = ADPCM-A `adpcma_keyoff` 化 / SSG tone disable) → round 2 approve + γ 実装 review approve
- ADR-0050 verify gate 対応: gate 1 (= fade finish 全 chip keyoff) を γ で trace 確認。 formal verify script 化 (= `verify-fadeout-semantics.sh` 10 gate) は δ sub-sprint

## Annex I: δ 実装 completion record (= verify script 体系化)

### I-1: δ deliverable

軸 B 実装 sprint 6 δ = fade-out semantics の verify script 体系化 (= 39th session、 PR #71)。 §決定 1 δ = β/γ の手動確認を再現可能 gate に固定する。 driver 改修なし (= verify script + 検証用 fixture のみ)。

| deliverable | 内容 |
|---|---|
| `src/test-fixtures/axis-b/verify-fadeout-semantics.sh` | fade-out semantics verify script (= 16 gate)。 §決定 7 の 10 gate を完全網羅 + γ finish tone disable + ADR-0051 SSG tone-enable regression。 `verify-mute-semantics.sh` / `verify-ssg-tone-enable.sh` の構造踏襲 |
| `src/tools/pmd-mml/fadeout-verify.mml` | fade-out verify 専用 fixture (= FM B/C/E/F + SSG G/H/I V15 を t=0 から長 note で同時鳴動、 A/D silent、 ADPCM-A/B/rhythm なし、 明示音長のみ) |

### I-2: fade fixture MML = `fadeout-verify.mml` 新規作成 (= user 判断 gate)

δ verify gate の fade fixture は **新規 `fadeout-verify.mml`** を用いる。 既存 fixture では SSG volume の literal 減衰が観測できないため、 user 判断 gate (= option B) で fade-out verify 専用 fixture を新規作成した。

| 候補 | 不適合理由 |
|---|---|
| `test01/test02.mml` (= β fade fixture) | SSG G/H/I が V0 + ADPCM-B J empty (= Annex G-4 finding)、 SSG volume literal 減衰が観測不可 |
| `test-tone-ladder.mml` (= 聴覚診断用) | heavily staggered (= SSG G/H/I が 8-12 全音符のリーディングレスト後に開始)、 fade (= 約 2.7 秒で完走) の window 中に SSG が鳴らない |
| **`fadeout-verify.mml` (= 新規)** | FM B/C/E/F + SSG G/H/I を t=0 から長 note で同時鳴動。 fade window 中に FM/SSG が鳴り、 SSG volume の literal 減衰を観測可能 |

`test-tone-ladder.mml` は聴覚診断専用として保持し、 fade 検証には流用しない。

### I-3: 16 gate 構成 (= §決定 7 の 10 gate 完全網羅)

| gate | §決定 7 対応 | 検証 |
|---|---|---|
| 1 | gate 1 | fade 開始 = `driver_fade_state` 0→1 + master/counter/level init write |
| 2 | gate 2 | `pmdneo_v2_fade_level` 単調減少 0x40→0x00 |
| 3 | gate 2 | ADPCM-A master reg `0x01` 単調減少 0x3F→0x00 |
| 4 | gate 2 | FM TL reg `0x42` faded ramp 0x78→0x7F 単調非減少 |
| 5 | gate 2 | SSG volume reg `0x08-0x0A` literal 単調減少 0x0F→0x00 |
| 6 | gate 2 | ADPCM-B reg `0x1B` reapply 機構 gate (= fade step 毎 write、 literal decay は J empty で未確認) |
| 7 | gate 3 | dispatch fade 適用 = fade 中 SSG raw volume spike なし |
| 8 | gate 4 | fade finish burst が trace 全体で 1 回 |
| 9 | gate 4 | finish burst が FM/SSG/ADPCM-A/ADPCM-B 全 keyoff |
| 10 | (= γ 追加) | SSG finish tone disable = reg `0x07` progressive disable、 最終 0x3F |
| 11 | gate 5 | register preservation = fade routine push/pop balance |
| 12 | gate 6 | cmd 0x04 即時 silence = `IRQ.inc` byte-identical |
| 13 | gate 7+8 | ADR-0049 mute regression (= `verify-mute-semantics.sh` 7 gate、 baseline 9 script 内包) |
| 14 | (= 追加) | ADR-0051 SSG tone-enable regression (= `verify-ssg-tone-enable.sh` 12 gate) |
| 15 | gate 9 | `.org` overflow なし + fade fixture call が production build に生成なし |
| 16 | gate 10 | SRAM placement = `pmdneo_v2_fade_level` free region 内 |

ADPCM-B (= gate 6) は `fadeout-verify.mml` も J empty のため literal value decay は未確認。 reapply 経路が ADPCM-B part を loop する機構 gate に留める (= user 判断 gate)。 ADPCM-B literal 減衰の verify は ADPCM-B 内容を持つ fixture が必要で、 本 sprint scope 外。

### I-4: δ 検証結果

- `verify-fadeout-semantics.sh` 16 gate ALL PASS
- fade fixture trace 実測: `pmdneo_v2_fade_level` 0x40→0x00 (66 writes) / ADPCM-A master reg `0x01` 0x3F→0x00 (65) / FM TL reg `0x42` faded ramp 0x78→0x7F 単調非減少 (64) / SSG volume reg `0x08-0x0A` 0x0F→0x00 literal 単調減少 / fade finish burst 20 write 1 回発火
- regression: `verify-mute-semantics.sh` 7 gate + `verify-ssg-tone-enable.sh` 12 gate 全 PASS
- audition 役割を script 内に明記: (a) fade fixture (`PMDNEO_FADE_FIXTURE=1`) = δ verify trace、 (b) default production build = cmd 6 fade-out audition、 (c) `PMDNEO_NO_FADE=1` = tone-ladder audition
- ADR-0050 verify gate 対応: §決定 7 の 10 gate を `verify-fadeout-semantics.sh` で完全 script 化。 ε sub-sprint で全 sub α/β/γ/δ の completion 確認 + Draft → Accepted 移行判断

## Annex J: ε 実装 completion record (= completion + Draft → Accepted 移行)

### J-1: ε deliverable

軸 B 実装 sprint 6 ε = fade-out semantics の completion + Draft → Accepted 移行判断 (= 39th session、 PR #72)。 §決定 1 ε = 全 sub α/β/γ/δ verify gate PASS + 規律遵守確認 + Accepted 移行判断 (= user 判断 gate)。 doc-only (= driver / verify script 改修なし)。

### J-2: α/β/γ/δ 総点検結果

| 点検項目 | 結果 |
|---|---|
| α/β/γ/δ 成果物 | PR #63 (= α 起票) / #64 (= β fade decay path) / #70 (= γ fade finish path) / #71 (= δ verify script 体系化) 全 MERGED |
| `verify-fadeout-semantics.sh` | ε 時点 (= PR #71 merge 後の統合ブランチ) で再実行 = 16 gate ALL PASS |
| §決定 7 verify gate | 10 gate を `verify-fadeout-semantics.sh` で完全 script 化 (= gate 1-16 が §決定 7 gate 1-10 を完全網羅) |
| ADR-0049 mute regression | `verify-mute-semantics.sh` 7 gate + baseline 9 script 全 PASS |
| ADR-0051 SSG tone-enable regression | `verify-ssg-tone-enable.sh` 12 gate 全 PASS |
| cmd 0x04 即時 silence | `IRQ.inc` `snd_command_04_fade_out` が base branch と byte-identical (= gate 12、 §決定 8 遵守) |
| 規律遵守 | driver 改修は β/γ のみ (= `standalone_test.s`、 `IRQ.inc`/`WORKAREA.inc` 不変)、 δ/ε は doc + verify script。 軸 G ADR-0048 / 軸 C ADR-0043 / rhythm ADR-0026-0031 完全不可触 |

### J-3: ε 点検 finding = Annex G-2 破壊 register 一覧の完成

ε 総点検で、 Annex G-2「fade routine 破壊 register 一覧」 (= §決定 5 mandate) が β 時点で作成され 5 routine を記載するが、 γ で新設した routine `pmdneo_fade_finish_silence` の行を欠くことを発見した (= Codex layer 2 ε review でも同点を must-fix 指摘)。 §決定 5 mandate (= fade routine の破壊 register 一覧を ADR に literal 記録) 充足のため、 ε で `pmdneo_fade_finish_silence` (= 破壊 AF/BC/DE/HL、 IX/IY 不変、 δ verify gate 11 register preservation で検証済) の 1 行を G-2 表へ追記し一覧を完成させた (= 5 routine → 6 routine)。

### J-4: follow-up 整理 (= ADR-0050 Accepted 阻害なし)

| follow-up | Accepted 阻害なしの根拠 |
|---|---|
| FM attack click | ADR-0051 Annex C-5。 FM voice / dispatch 固有 finding で fade 経路と独立 |
| V0 SSG keyon literal trace gate | ADR-0051 Annex D-3。 SSG tone-enable verify follow-up で fade 経路と独立 |
| ADPCM-B literal value decay 未確認 | ADR-0050 Annex I-3。 `fadeout-verify.mml` が J empty のため gate 6 を機構 gate とした。 user 判断 gate で ADPCM-B literal 減衰は本 sprint scope 外と確定 (= §決定 6 scope 延長)、 reapply 経路の機構 gate で代替 |

3 件いずれも fade-out semantics の Accepted を阻害しない。 FM attack click / V0 SSG keyon literal trace gate は後続候補として保持する。

### J-5: Draft → Accepted 移行根拠 (= 2026-05-21 user 判断 gate)

ADR-0050 Draft → Accepted 移行は次を根拠に、 Codex layer 2 review approve + user 判断 gate を経て確定 (= §決定 11)。

1. α/β/γ/δ 全 sub-sprint 完走 = PR #63/#64/#70/#71 全 MERGED
2. `verify-fadeout-semantics.sh` 16 gate ALL PASS = §決定 7 の 10 gate を完全 script 化
3. fade decay path (= β) + fade finish path (= γ) + verify script 体系化 (= δ) が揃い、 楽曲全体 fade-out が FM 6ch + SSG 3ch + ADPCM-A + ADPCM-B の全 chip 段階減衰 → 完全 silence で全 chip keyoff として成立
4. regression = ADR-0049 mute 7 gate + ADR-0051 SSG tone-enable 12 gate 全 PASS
5. cmd 0x04 即時 silence byte-identical (= §決定 8)、 軸 G ADR-0048 / 軸 C ADR-0043 / rhythm 完全不可触の規律遵守
6. §決定 5 mandate (= 破壊 register 一覧) を ε で完成 (= J-3)
7. follow-up 3 件は Accepted 阻害要因ではない (= J-4)

**ADR-0050 Accepted = 軸 B 実装 sprint 6 fade-out semantics 完了。 軸 B 全体完了ではない** (= 軸 B 実装 sprint chain は実装 sprint 1-4 = δ-1〜δ-4 が未着手、 「軸 B 完成」 表現不使用)。
...
| 2026-05-20 | β 実装完了 (= 39th session、 PR #64) | fade decay path 実装 = `pmdneo_v2_fade_level` field + 4 fade core routine (= `pmdneo_fade_begin`/`pmdneo_v2_fade_tick`/`pmdneo_v2_fade_reapply`/`pmdneo_fade_scale`) + volume hook 4 箇所 fade scale 混入 + ADPCM-A master 直接 ramp + `TEST_MODE_FADE_FIXTURE`。 Annex G 追記 (= β completion record + fade routine 破壊 register 一覧 §決定 5 + β finding = `.org 0x0066` overlap pre-existing bug 修復 + 検証結果) + sub-sprint chain β 完了 reflect。 Codex layer 2 review 論点 1/2 両 approve + escalate 2 件 user GO。 軸 B 実装 sprint 6 = sprint chain 6 中 2 本目、 軸 B 全体未完了 |
| 2026-05-21 | γ 実装完了 (= 39th session、 PR #70) | fade finish path 実装 = `pmdneo_fade_finish_silence` routine 新設 (= FM/SSG/ADPCM-A/ADPCM-B 全 chip keyoff) + `pmdneo_v2_fade_tick_finish` を旧 ADPCM-A bulk `0xBF` dump から全 chip keyoff call へ置換。 SSG は ADR-0051 §決定 3/4 準拠で tone disable も実施、 ADPCM-A は ADR-0049 §決定 5 準拠で `adpcma_keyoff` 本体 call 化。 Annex H 追記 (= γ completion record + 検証結果) + sub-sprint chain γ 完了 reflect。 Codex layer 2 = kickoff plan round 1 revise must-fix 2 → round 2 approve + 実装 review approve。 fade fixture trace で finish burst 20 write routine 順通り + 1 回発火確認、 mute 7 gate + SSG tone-enable 12 gate 全 PASS。 軸 B 実装 sprint 6 = sprint chain 6 中 2 本目、 軸 B 全体未完了 |
| 2026-05-21 | δ 実装完了 (= 39th session、 PR #71) | verify script 体系化 = `verify-fadeout-semantics.sh` 新規 (= 16 gate、 §決定 7 の 10 gate 完全網羅 + γ finish tone disable + ADR-0051 SSG tone-enable regression) + fade-out verify 専用 fixture `fadeout-verify.mml` 新規 (= FM/SSG を t=0 から同時鳴動、 SSG volume literal 減衰を観測可能化)。 Annex I 追記 (= δ completion record + fixture 選択経緯 + 16 gate 構成 + 検証結果) + sub-sprint chain δ 完了 reflect。 Codex layer 2 = kickoff plan round 1 revise must-fix 6 → round 2 approve → fixture 訂正 (= test-tone-ladder.mml staggered finding、 user 判断 gate option B = `fadeout-verify.mml` 新規) → round 3 approve + δ 実装 review approve。 16 gate ALL PASS。 driver 改修なし (= verify script + fixture のみ)。 軸 B 実装 sprint 6 = sprint chain 6 中 2 本目、 軸 B 全体未完了 |
| 2026-05-21 | ε 完了 + Draft → Accepted 移行 (= 39th session、 PR #72) | completion + Accepted 判断 = α/β/γ/δ 総点検 + `verify-fadeout-semantics.sh` 16 gate ALL PASS (= ε 時点再実行) + ADR-0049 mute / ADR-0051 SSG tone-enable regression PASS + cmd 0x04 byte-identical 確認。 ε 点検 finding = Annex G-2 破壊 register 一覧の γ routine `pmdneo_fade_finish_silence` 欠落 → 1 行追記で完成 (= §決定 5 mandate 充足)。 Annex J 追記 (= ε completion record + 総点検結果 + finding + follow-up 整理 + 移行根拠) + sub-sprint table ε 完了 reflect + 状態 Draft → Accepted。 Codex layer 2 = ε review round 1 revise must-fix 1 (= G-2 欠落) → 追記 → round 2 approve、 user 判断 gate で Accepted 移行確定。 ADR-0050 Accepted = fade-out semantics 完了、 軸 B 全体は未完了 (= 「軸 B 完成」 表現不使用)。 follow-up = FM attack click / V0 SSG keyon literal trace gate / ADPCM-B literal value decay 未確認 |
