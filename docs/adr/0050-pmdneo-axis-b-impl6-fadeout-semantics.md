...
### 決定 3: 設計核心論点 A = fade 適用位置 (= 案 a / 案 b、 β 着手前 user 判断 gate)
...
- **ADPCM-A/ADPCM-B** は master volume register (= ADPCM-A port B reg `0x01` / ADPCM-B port B reg `0x1B`) を持ち、 note dispatch がこの master volume を per-note に再 write しないため、 IRQ 前半で fade 値を直接 write しても上書きされない (= 現 active fade が ADPCM-A で機能する理由)。
- **FM/SSG** は master volume register を持たず、 note dispatch が per-note に FM operator TL register / SSG channel volume register を再 write する。 そのため IRQ 前半で fade 値を直接 write しても、 同 tick 後半の song dispatch で上書きされる。
...
案 (a)/(b) の最終確定は β 着手前に Codex layer 2 壁打ち → user 判断 gate (= ADR-0041 §決定 4-2)。 ADPCM-A/B は採用案に関わらず master volume register 経路を維持する (= 論点 A は FM/SSG 限定)。
...
- chip 別 fade strategy (= ADPCM-A/B は master volume register 経路、 FM/SSG は決定 3 論点 A の採用案経路)
...
| 2 | per-tick 段階減衰 (= 採用案で分岐) | **ADPCM-A/B 共通** = master volume register (= ADPCM-A reg `0x01` / ADPCM-B reg `0x1B`) が IRQ tick 毎に減衰 write + 減衰値単調減少。 **FM/SSG** = 案 (a) 採用時 = dispatch 後の post-dispatch register write が IRQ tick 毎に発生 + 減衰値単調減少 / 案 (b) 採用時 = `pmdneo_v2_fade_*` attenuation factor field が IRQ tick 毎に単調減少 (= z80-mem-trace) + note dispatch 発生 tick の volume register write 値が factor 適用後値 |
| 3 | fade 値が dispatch raw 値に上書きされない (= 採用案で分岐) | 案 (a) = fade write が当該 tick の last write (= ymfm-trace write 順序確認) / 案 (b) = dispatch 時の volume register write 値が factor 適用後値で raw note volume ではない |
...
|---|---|---|---|
| α (= ground truth 調査 + ADR 起票) | **進行中** (= 38th session、 本 PR) | 本 PR | kickoff 計画 round 1-3 chain (= round 1 revise must-fix 4 + round 2 revise must-fix 2 → round 3 approve) + ADR-0050 起票 review (= 後続) |
| β (= fade decay path 実装 + driver-embedded fixture) | 未着手 | - | - |
...
2. ADPCM-A/B は master volume register を持つため減衰が単純 (= register 1 本を ramp)。 FM/SSG は master volume register がないため、 fade 適用位置が設計論点になる (= 決定 3 論点 A)。
