...
### timing 制約 (= 25th session PR #4 review finding 1)

`timing.timeMode` は `"absolute"` のみ受け付ける (= 省略時 default = "absolute")。 `"delta"` は spike では reject (exit 65)。 delta IR を絶対 tick 扱いで lowering すると timing が静かに破壊されるため、 importer 側で absolute に正規化してから入力する。

### pass-through 規律 (= 25th session PR #4 review finding 2)

ADPCMATrigger / RawRegisterWrite は元 event を残しつつ、 `order` は allocator で再採番する (= 同一 tick に semantic lowering 由来 event と pass-through event が混在しても order 衝突しないように)。

### 入力 sort 規律 (= 25th session PR #4 review finding 4 + finding 5)

入力 events 配列の array 順は **not authoritative** = IR の `(tick, trackId, order)` フィールドが semantic 真の順序。 lower_events() は処理前に `(tick, trackId, order)` で sort し直し、 array 順依存の silent semantic 順序反転を防止する。 入力に tick/order/layer/type の必須 field 欠落があれば sort 前に exit 65 reject。

同一 `(tick, trackId, order)` 重複は Python stable sort 経由で input array 順が silently authoritative 化するため、 sort 前に検出して exit 65 reject (= finding 5 反映、 schema 上 `order` は track 内で単調増加と規定されているため (tick, trackId, order) global unique を spike layer で enforce)。
...
def lower_events(input_events: list[dict]) -> tuple[list[dict], dict]:
    ...
    Note: 入力 array 順は **not authoritative**。 IR の (tick, trackId, order) が semantic 真の順序。
    spike は処理前に (tick, trackId, order) で sort し直す (= PR #4 review finding 4 反映、
    array 順依存 silent semantic 順序反転 防止)。
    同一 (tick, trackId, order) 重複は Python の stable sort で array 順が silently authoritative
    化するため、 sort 前に検出して exit 65 reject (= PR #4 review finding 5 反映)。
    """
    ...
    for ev in input_events:
        ...
        if key in seen_keys:
            raise ValueError(
                ...
                f"既存 event {seen_keys[key]} vs 新 event {ev}。 "
                f"schema は track 内で order 単調増加と規定しているため (tick, trackId, order) は global unique 必須。 "
                f"重複を許すと Python stable sort 経由で input array 順が silently authoritative 化する (= PR #4 review finding 5 反映)。"
            )
        seen_keys[key] = ev
...
def main() -> int:
    ...
    if time_mode != "absolute":
        print(
            f"[FAIL] lowering error: timing.timeMode = {time_mode!r} は spike scope 外 (= absolute tick のみ対応)。 "
            f"delta IR は importer 側で absolute に正規化してから lowering 入力にすること (= silent timing 破壊 防止)。",
            file=sys.stderr,
        )
        return EXIT_PARSE
