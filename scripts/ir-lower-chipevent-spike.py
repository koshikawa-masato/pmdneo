...
### 入力 sort 規律 (= 25th session PR #4 review finding 4)

入力 events 配列の array 順は **not authoritative** = IR の `(tick, order)` フィールドが semantic 真の順序。 lower_events() は処理前に `(tick, order)` で sort し直し、 array 順依存の silent semantic 順序反転を防止する。 入力に tick/order/layer/type の必須 field 欠落があれば sort 前に exit 65 reject。
...
def lower_events(input_events: list[dict]) -> tuple[list[dict], dict]:
    ...
    Note: 入力 array 順は **not authoritative**。 IR の (tick, order) が semantic 真の順序。
    spike は処理前に (tick, order) で sort し直す (= PR #4 review finding 4 反映、
    ...
    sorted_input = sorted(input_events, key=lambda e: (e["tick"], e["order"]))
