#!/usr/bin/env python3
"""PMDNEO IR FM3Mode chip event validation + identity pass-through spike (= ADR-0038 §決定 5-2 / 28th session γ).

v0.4 IR JSON file を入力に、 FM3Mode ChipEvent + 全 event を chip-to-chip identity で pass-through する spike。 raw lowering は ADR-0038 §決定 5 で v0.5 以降別 ADR (= RMW / mask event 新設判断含む) に defer のため、 本 spike は validation + identity pass-through に縮小。

「FM3Mode が chip layer で validation 可能で、 chip event として下流に流せる」 ことの proof。 compiler 本体 / WebApp / runtime / driver / `.mn` / `.PNE` は touch しない。

## scope (= ADR-0038 §決定 5-2 + §決定 7)

- 入力 v0.4 IR を sort + 重複 reject + timeMode check
- 全 event を chip-to-chip identity で pass-through (= output allocator 経由 order 再採番のみ)
- FM3Mode 固有 defense in depth (= operatorEnableMask 1-15 / operatorBlock 各 0-7 / operatorFnum 各 0-2047 / array length 4 / keyPolicy enum)
- 出力を (tick, order) sort 正規化

## scope-out (= ADR-0038 §scope-out)

- FM3Mode → RawRegisterWrite raw lowering (= v0.5 以降別 ADR、 RMW/mask event 新設判断含む)
- mode 切替直前 KeyOff 推奨 diagnostics 追加 (= 別 sprint、 spike では出さない)
- 他 event 種の chip → raw lowering (= 別 spike chain)
...
CREATED_BY = "ir-lower-fm3mode-spike v0.4 (= 28th session γ、 IR FM3Mode chip event validation + identity pass-through、 raw lowering は ADR-0038 §決定 5 で v0.5 以降別 ADR defer)"
...
def _validate_fm3mode(ev: dict) -> None:
    """FM3Mode 固有 defense in depth validation (= schema 表現外の range / type 厳密判定、 ADR-0038 §決定 3 + 5-2)."""
    ...
    if type(mask) is not int or not (1 <= mask <= 15):
        raise ValueError(
            f"FM3Mode.operatorEnableMask must be int 1-15 (= ADR-0035 §決定 6 規律、 no-op 防止): {mask!r}"
        )
...
def lower_events(input_events: list[dict]) -> tuple[list[dict], dict]:
    """v0.4 IR events を chip-to-chip identity で pass-through (= FM3Mode 固有 validation 込み)。
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="PMDNEO IR FM3Mode chip event validation + identity spike (= ADR-0038 §決定 5-2 / 28th session γ)",
    )
