#!/usr/bin/env bash
#
# ADR-0032 step 18 γ: K-0x03 vs R-0x03 (= BD+CYM multi-bit) byte-identical proof (= K と R が同 multi-bit dispatch path を経由 = latent semantics 証明下で K=R 成立)
#
# 目的:
#   ADR-0032 §決定 4/6/8 「dispatch path は drum 種拡張で増やさない」 + 「dispatch path は simultaneous trigger でも増やさない」
#   = 第 2 invariant の literal 証明。
#   K-0x03 fixture (= k05.mml) と R-0x03 fixture (= r-melody-05.mml) を両方 build + run して、
#   ADPCM-A L ch multi-bit register write sequence (= BD reg writes → CYM reg writes、 last-write-dominant CYM)
...
#     bit 0 BD trigger + bit 1 CYM trigger 順次)
#   - R command 0xEB 0x03 path (= pmdneo_part_main_parse → commandsp → commandsp_rhykey →
#     pmdneo_rhythm_event_trigger → bit 0 BD trigger + bit 1 CYM trigger 順次)
#
...
#
#   fixture naming = bitmap-centric hex pattern (= k05.mml / r-melody-05.mml = bitmap 0x03、
#   drum-centric semantics naming Step 12-17 からの転換点 = sprint chain 軸転換 milestone)。
#
#   PMDDotNET 側 emit path = mc.cs rs00() OR 蓄積 (= 連続 \b\c → 単一 0xEB 0x03 bytecode に collapse)。
#
...
#           _rhythm_event_bd_trigger / _rhythm_event_cym_trigger 同 addr (= multi-bit dispatch sub-routine 共通)
#   gate 4: K-0x03 fixture run last-write-dominant CYM literal (= last-write tail = CYM)
#   gate 5: R-0x03 fixture run last-write-dominant CYM literal (= K-0x03 と同 last-write)
#   gate 6: K-0x03 と R-0x03 の L ch register write sequence byte-identical (= **multi-bit differential proof**)
#   gate 7: K-0x03 と R-0x03 で L ch keyon mask 0x01 trigger count identical (= 2 件以上、 BD + CYM multi-bit dispatch)
#
# 検証範囲外 (= δ で別途):
...
#
# 使い方:
#   bash src/test-fixtures/step18/verify-step18-kr-05-byte-identical.sh
#
# 副作用:
#   /tmp/pmdneo-step18/k05.wav              (= K-0x05 multi-bit audible 試聴用、 4 秒、 CYM 支配)
#   /tmp/pmdneo-step18/r-melody-05.wav      (= R-0x05 multi-bit audible 試聴用、 4 秒、 CYM 支配)
#   /tmp/pmdneo-step18/k05-*.tsv            (= K-0x03 trace snapshot)
#   /tmp/pmdneo-step18/r-melody-05-*.tsv    (= R-0x03 trace snapshot)
#
# Exit code:
#   0 = PASS (= 全 7 gate 通過)
#   1 = verify fail
...
mkdir -p "$OUT_DIR"
TMPDIR=$(mktemp -d "/tmp/pmdneo-step18-gamma-kr-05-XXXXXX")
trap 'rm -rf "$TMPDIR"' EXIT

## Literal expected values (= ADR-0032 §決定 7 BD-anchor pair、 last-write-dominant = CYM)
## last-write tail = CYM literal (= bitmap 0x03 末尾 drum、 bit 1 = adpcma_sample_sd reuse)
EXPECTED_LAST_START_LSB="12"             # (= SD_START_LSB)
EXPECTED_LAST_START_MSB="00"             # (= SD_START_MSB)
EXPECTED_LAST_STOP_LSB="29"              # (= SD_STOP_LSB)
EXPECTED_LAST_STOP_MSB="00"              # (= SD_STOP_MSB)
EXPECTED_VOL_PAN="DF"
EXPECTED_KEYON_MASK="01"
EXPECTED_KEYON_COUNT_MIN=2               # (= BD + CYM 順次 multi-bit dispatch)
LAST_DRUM_LABEL="CYM"
BITMAP_HEX="05"
...
echo "  [INFO] K-0x${BITMAP_HEX} build: _rhythm_event_cym_trigger @ 0x${K_CYM_ADDR}"
...
echo "  [INFO] R-0x${BITMAP_HEX} build: _rhythm_event_cym_trigger @ 0x${R_CYM_ADDR}"
...
if [[ "$K_CYM_ADDR" != "$R_CYM_ADDR" ]]; then
    echo "  [FAIL] gate 3: CYM trigger addr differs (K=0x${K_CYM_ADDR} vs R=0x${R_CYM_ADDR})"
    exit 1
fi
echo "  [PASS] _rhythm_event_cym_trigger addr identical = 0x${K_CYM_ADDR}"
