#!/usr/bin/env bash
#
# ADR-0032 step 18 γ: K part 0x21 (= BD+RIM) multi-bit rhythm trigger proof (= K \b\i → 0xEB 0x21 → pmdneo_rhythm_event_trigger → bit 0 BD + bit 5 RIM 順次 call nz 連鎖 → ADPCM-A L ch 順次上書き = last-write-dominant RIM = simultaneous trigger semantics proof representative pair γ)
#
# 目的:
#   ADR-0032 §決定 1/2/3/4/5/6/7/11 整合の K part 0xEB 0x21 multi-bit rhythm dispatch path proof。
#   K-0x21 fixture (= src/test-fixtures/step18/k21.mml) の K part body `\b\i` が
...
#   rs01 OR 蓄積 work.al |= cch = 0x21 → 直前 0xEB arg を 0x21 で上書き)、 driver の rhythm_main K part
...
#
#   R-0x21 fixture (= melody part 内 \b\i inline = r-melody-21.mml) は γ scope で別 verify script。
#   K-0x21 vs R-0x21 byte-identical literal proof も γ で別 verify script。
#
# 検証: 5 段 gate (= ADR-0032 §verify gate Gate 1 + Gate 4 + Gate 5 + Gate 6 + Gate 7 統合)
#   gate 1: K-0x21 fixture build PASS (= PMDDotNET \b\i → rs00() OR 蓄積 → 0xEB 0x21 + driver compile)
...
#   gate 3: K-0x21 fixture run PC trace + ymfm-trace 取得
...
#   - K-0x21 と R-0x21 で同 routine addr hit literal 比較 (= byte-identical) → γ で differential verify
...
K_MML="$PROJECT_ROOT/src/test-fixtures/step18/k21.mml"
...
mkdir -p "$OUT_DIR"
TMPDIR=$(mktemp -d "/tmp/pmdneo-step18-gamma-21-XXXXXX")
...
echo "=== ADR-0032 step 18 γ: K part 0x21 (= BD+RIM) multi-bit rhythm trigger proof (= K \\b\\s → 0xEB 0x21 → ADPCM-A L ch 順次上書き = last-write-dominant RIM = simultaneous trigger semantics proof representative pair γ) ==="
echo

# ============================================================
# gate 1: K-0x21 fixture build
# ============================================================
echo "=== gate 1: K-0x21 fixture build PASS (= PMDDotNET \\b\\s → rs00() OR 蓄積 → 0xEB 0x21 + driver compile) ==="
PMDDOTNET_MML="$K_MML" PMDDOTNET_MODE=B PMDNEO_USE_PMDDOTNET=1 \
    bash scripts/build-poc.sh > "$TMPDIR/k21-build.log" 2>&1 || {
    echo "  [FAIL] gate 1 infra: K-0x21 fixture build failed (log: $TMPDIR/k21-build.log)"
    exit 2
}
echo "  [PASS] K-0x21 fixture build"
...
# ============================================================
# gate 3: K-0x21 fixture run + trace 取得
# ============================================================
echo
echo "=== gate 3: K-0x21 fixture run + PC trace + ymfm-trace 取得 ==="
bash scripts/run-mame.sh --headless --wavwrite --wavwrite-seconds 4 --trace \
    > "$TMPDIR/k21-run.log" 2>&1 || {
    echo "  [FAIL] gate 3 infra: MAME run failed (log: $TMPDIR/k21-run.log)"
    exit 2
}
K_MEM="$OUT_DIR/k21-mem.tsv"
K_YMFM="$OUT_DIR/k21-ymfm.tsv"
K_WAV="$OUT_DIR/k21.wav"
cp /tmp/pmdneo-trace/z80-mem-trace.tsv "$K_MEM"
cp /tmp/pmdneo-trace/ymfm-trace.tsv "$K_YMFM"
cp /tmp/pmdneo-trace/audio.wav "$K_WAV"
echo "  [PASS] trace + wav 取得 (= K-0x21 fixture run、 wav 4 sec @ ${K_WAV}、 RIM 支配の last-write-dominant)"
...
RIM_LINE=$(awk -F'\t' -v expect="$EXPECTED_RIM_START_LSB" 'BEGIN{IGNORECASE=1} $2 == "B" && $3 == "110" && toupper($4) == toupper(expect) {print NR; exit}' "$K_YMFM")
...
echo
echo "🎉 ADR-0032 step 18 γ K-0x21 multi-bit rhythm trigger proof PASS (= simultaneous trigger semantics proof representative pair γ)"
echo "   - gate 1: K-0x21 fixture build PASS"
